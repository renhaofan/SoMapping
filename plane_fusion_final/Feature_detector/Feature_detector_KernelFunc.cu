...
__global__ void convert_intensity_to_cvmat_KernelFunc(const float * src_intensity, My_Type::Vector2i image_size, 
													  ...
													  unsigned char * dst_intensity)
{
	...
	if (u >= image_size.width)	return;
	if (v >= image_size.height)	return;
	int index = v * image_size.width + u;
	// Load intensity value
	float intensity_value_f = src_intensity[index];
	// Normalization
	intensity_value_f = (intensity_value_f - intensity_range.x) / (intensity_range.y - intensity_range.x);
	// Scale to [0, 255]
	intensity_value_f = intensity_value_f * 256.0f;
	int intensity_value_i = (int)intensity_value_f;
	if (intensity_value_i < 0)		intensity_value_i = 0;
	if (intensity_value_i > 255)	intensity_value_i = 255;
										   ...
										   unsigned char * dst_intensity)
{
	...
	convert_intensity_to_cvmat_KernelFunc << <block_rect, thread_rect >> >(src_intensity, image_size, intensity_range,
																		   dst_intensity);
}

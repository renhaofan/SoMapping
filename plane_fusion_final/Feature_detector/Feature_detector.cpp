...
//
std::vector<float> compute_response(const cv::Mat &img,
...
Feature_detector::Feature_detector() {
  ...
  //
  this->orb_feature_extractor = new ORB_SLAM2::ORBextractor(
      this->aim_number_of_features, this->orb_scale_factor,
      this->number_of_feature_levels, this->iniThFAST, this->minThFAST);
  //
...
//
void Feature_detector::detect_orb_features(
    Hierarchy_image<float> &dev_current_intensity,
  ...
  // Prepare to detect features
  {
    ...
    //
    My_Type::Vector2i image_size(dev_current_intensity.size[0].width,
                                 dev_current_intensity.size[0].height);
    ...
    block_rect.z = 1;
    convert_intensity_to_cvmat_KernelFunc(
        block_rect, thread_rect, dev_current_intensity.data_ptrs[0], image_size,
        My_Type::Vector2f(0.0f, 1.0f), this->dev_gray_image_buffer);
    // CUDA_CKECK_KERNEL;

    checkCudaErrors(
        cudaMemcpy(this->gray_image.data, this->dev_gray_image_buffer,
  ...
  if (false) {
    ...
    this->current_match_to_model_id.reserve(
        this->previous_match_to_model_id.size());
    ...
    if (!this->previous_gray_image.empty()) {
      ...
      cv::calcOpticalFlowPyrLK(this->previous_gray_image, this->gray_image,
                               this->previous_keypoints_2d,
                               tracked_keypoints_2d, track_status,
                               cv::noArray());
      //
      for (int previous_id = 0; previous_id < tracked_keypoints_2d.size();
           previous_id++) {
        if (track_status[previous_id] == 1 &&
          ...
          // Match id
          this->current_match_to_model_id.push_back(
              this->previous_match_to_model_id[previous_id]);
          // Keypoint-2d
          this->current_keypoints_2d.push_back(
              tracked_keypoints_2d[previous_id]);
        }
      }
      ...
      std::vector<float> response_vector =
          compute_response(this->gray_image, this->current_keypoints_2d, 3);
      ...
      if (response_vector_to_sort.size() > this->aim_number_of_features) {
        response_threshold =
            response_vector_to_sort[response_vector_to_sort.size() -
                                    this->aim_number_of_features];
      ...
      for (int response_id = 0; response_id < response_vector.size();
           response_id++) {
        if (response_vector[response_id] >= response_threshold) {
          keypoint_match_id_buffer.push_back(
              this->current_match_to_model_id[response_id]);
      ...
      // Debug
      // printf("%f\n", (float)this->current_keypoints_2d.size() /
      // (float)tracked_keypoints_2d.size());
    }
    ...
    if (use_mask) {
      // Generate mask
      this->detected_mask =
          255 * cv::Mat::ones(this->gray_image.size(), this->gray_image.type());
      for (int tracked_kp_id = 0;
           tracked_kp_id < this->current_keypoints_2d.size(); tracked_kp_id++) {
        ...
        for (int u_offset = -this->keypoint_window_radius * multiple_value;
             u_offset < this->keypoint_window_radius * multiple_value;
             u_offset++)
          for (int v_offset = -this->keypoint_window_radius * multiple_value;
               v_offset < this->keypoint_window_radius * multiple_value;
               v_offset++) {
            int u_check = u_base + u_offset;
            int v_check = v_base + v_offset;
            if (u_check < 0 || u_check >= this->detected_mask.cols ||
                v_check < 0 || v_check >= this->detected_mask.rows)
              continue;
            //
            this->detected_mask
                .data[u_check + v_check * this->detected_mask.cols] = 0;
          }
      }
      // cv::imshow("this->detected_mask", this->detected_mask);
      cv::goodFeaturesToTrack(
          this->gray_image, new_detected_keypoints_2d,
          (int)this->aim_number_of_features / 3, (double)0.01,
          (double)this->keypoint_window_radius, this->detected_mask, 3);
    } else {
      std::vector<cv::Point2f> new_keypoint_swap_buffer;
      cv::goodFeaturesToTrack(
          this->gray_image, new_keypoint_swap_buffer,
          (int)this->aim_number_of_features / 3, (double)0.01,
          (double)this->keypoint_window_radius, cv::noArray(), 3);
      //
      this->detected_mask =
          255 * cv::Mat::ones(this->gray_image.size(), CV_8UC1);
      for (int tracked_kp_id = 0;
           tracked_kp_id < this->current_keypoints_2d.size(); tracked_kp_id++) {
        int u_base = (int)this->current_keypoints_2d[tracked_kp_id].x;
        int v_base = (int)this->current_keypoints_2d[tracked_kp_id].y;
      ...
      // cv::imshow("this->detected_mask", this->detected_mask);
      //
      for (int new_keypoint_id = 0;
           new_keypoint_id < new_keypoint_swap_buffer.size();
           new_keypoint_id++) {
        int mask_index = (int)new_keypoint_swap_buffer[new_keypoint_id].x +
                         (int)new_keypoint_swap_buffer[new_keypoint_id].y *
                             this->detected_mask.cols;
        if (this->detected_mask.data[mask_index] != 0 &&
            new_keypoint_swap_buffer[new_keypoint_id].x > 16 &&
            new_keypoint_swap_buffer[new_keypoint_id].x <=
                this->gray_image.cols - 16 &&
            new_keypoint_swap_buffer[new_keypoint_id].y > 16 &&
            new_keypoint_swap_buffer[new_keypoint_id].y <=
                this->gray_image.rows - 16) {
          new_detected_keypoints_2d.push_back(
              new_keypoint_swap_buffer[new_keypoint_id]);
        }
      }
    }
    //
    this->current_keypoints_2d.insert(this->current_keypoints_2d.end(),
                                      new_detected_keypoints_2d.begin(),
                                      new_detected_keypoints_2d.end());
    //
    ...
    this->current_match_to_model_id.insert(
        this->current_match_to_model_id.end(), temp_match_id.begin(),
        temp_match_id.end());

    // Compute ORB features
    this->current_keypoints.clear();
    for (int pt_id = 0; pt_id < this->current_keypoints_2d.size(); pt_id++)
      this->current_keypoints.push_back(
          cv::KeyPoint(this->current_keypoints_2d[pt_id], 3));
    cv::Ptr<cv::ORB> orb_detector_ptr =
        cv::ORB::create(this->aim_number_of_features);
    //
    std::vector<cv::KeyPoint> compare_buffer = this->current_keypoints;
    orb_detector_ptr->compute(this->gray_image, this->current_keypoints,
                              this->current_features);
    //
  ...
  if (false) {
    cv::Mat draw_buffer;
    drawKeypoints(this->gray_image, this->current_keypoints, draw_buffer,
  ...
  for (int current_id = 0; current_id < this->current_keypoints.size();
    ...
    for (int v_inc = -window_radius; v_inc <= window_radius; v_inc++)
      for (int u_inc = -window_radius; u_inc <= window_radius; u_inc++) {
        int u_check = u + v_inc;
        int v_check = v + v_inc;
        if (u_check < window_radius &&
            u_check >= this->gray_image.cols - window_radius &&
            v_check < window_radius &&
            v_check >= this->gray_image.rows - window_radius)
          continue;

        int point_index = u_check + v_check * image_width;
        if (current_points.data_ptrs[0][point_index].z > 0.1f)
          min_z_point =
              fminf(min_z_point, current_points.data_ptrs[0][point_index].z);
      }
    //
    ...
    for (int v_inc = -window_radius; v_inc <= window_radius; v_inc++)
      for (int u_inc = -window_radius; u_inc <= window_radius; u_inc++) {
        ...
        int point_index = u_check + v_check * image_width;
        if (fabsf(current_points.data_ptrs[0][point_index].z - min_z_point) >
            0.05f)
          continue;
...
void Feature_detector::match_orb_features(int number_of_model_keypoints) {
  ...
  for (int current_id = 0; current_id < this->current_keypoints.size();
    ...
    for (int model_id = 0;
         model_id < this->visible_point_model_index[current_id].size();
         model_id++) {
      cv::Mat model_orb_feature =
          this->visible_model_features[current_id].row(model_id);
      int orb_distance =
          DescriptorDistance(model_orb_feature, current_orb_feature);
    ...
    if (this->current_match_to_model_id[current_id] >= 0) {
      ...
      /* Matched by feature */
      this->current_match_to_model_id[current_id] =
          visible_point_model_index[current_id][min_feature_id];
      ...
      this->current_match_to_model_id[current_id] = -1;
      // printf("3-  %d, %d\n", current_id,
      // this->visible_point_model_index[current_id].size());
    }
  }
...
std::vector<float> compute_response(const cv::Mat &img,
  ...
  for (int ptidx = 0; ptidx < pts.size(); ptidx++) {
    ...
    if (is_valid_point) {
      ...
      response_vector.push_back(((float)a * b - (float)c * c -

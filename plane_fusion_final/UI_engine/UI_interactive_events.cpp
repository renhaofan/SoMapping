...
void UI_engine::fix_window_aspect()
{
	...
	double preset_ratio = (double)UI_parameters::instance()->main_viewport_size.height
		/ (double)(UI_parameters::instance()->main_viewport_size.width + UI_parameters::instance()->sub_viewport_size.width);

	// Resize window
	UI_ptr->window_height = (int)(0.5f + (double)(UI_ptr->window_width) * preset_ratio);
	...
	//My_Type::Vector2i viewport_size(UI_ptr->main_viewport_width, UI_ptr->main_viewport_height);
...
void UI_engine::interactive_events()
{
	...
	if (this->special_key[GLUT_KEY_LEFT] == 1)	
		this->GL_camera_Frame.rotate_inFrame(0.0, -UI_parameters::instance()->UI_camera_rotation_sensitivity, 0.0);
	if (this->special_key[GLUT_KEY_RIGHT] == 1)	
		this->GL_camera_Frame.rotate_inFrame(0.0, +UI_parameters::instance()->UI_camera_rotation_sensitivity, 0.0);
	if (this->special_key[GLUT_KEY_UP] == 1)		
		this->GL_camera_Frame.rotate_inFrame(-UI_parameters::instance()->UI_camera_rotation_sensitivity, 0.0, 0.0);
	if (this->special_key[GLUT_KEY_DOWN] == 1)	
		this->GL_camera_Frame.rotate_inFrame(+UI_parameters::instance()->UI_camera_rotation_sensitivity, 0.0, 0.0);
	// (Normal key)
	if (this->normal_key['d'] == 1 || this->normal_key['D'] == 1)
		this->GL_camera_Frame.transfer_inFrame(-UI_parameters::instance()->UI_camera_translation_sensitivity, 0, 0);
	if (this->normal_key['a'] == 1 || this->normal_key['A'] == 1)
		this->GL_camera_Frame.transfer_inFrame(+UI_parameters::instance()->UI_camera_translation_sensitivity, 0, 0);
	if (this->normal_key['w'] == 1 || this->normal_key['W'] == 1)
		this->GL_camera_Frame.transfer_inFrame(0, 0, +UI_parameters::instance()->UI_camera_translation_sensitivity);
	if (this->normal_key['s'] == 1 || this->normal_key['S'] == 1)
		this->GL_camera_Frame.transfer_inFrame(0, 0, -UI_parameters::instance()->UI_camera_translation_sensitivity);

	// Roll
	if (this->special_key[GLUT_KEY_PAGE_UP] == 1)		
		this->GL_camera_Frame.rotate_inFrame(0.0, 0.0, -UI_parameters::instance()->UI_camera_rotation_sensitivity);
	if (this->special_key[GLUT_KEY_PAGE_DOWN] == 1)	
		this->GL_camera_Frame.rotate_inFrame(0.0, 0.0, +UI_parameters::instance()->UI_camera_rotation_sensitivity);
	
	// zoom-in 
	if ((this->normal_key['e'] == 1 || this->normal_key['E'] == 1) && (this->GL_camera_Frame.view_distance > 0))
	{
		this->GL_camera_Frame.view_distance -= UI_parameters::instance()->UI_camera_translation_sensitivity;
		this->GL_camera_Frame.transfer_inFrame(0, 0, +UI_parameters::instance()->UI_camera_translation_sensitivity);
	}
	// zoom-out
	...
	{
		float d_x, d_y;
		d_x = (float)(this->mouse_LeftDown_X - this->mouse_X) * UI_parameters::instance()->UI_camera_mouse_rotation_sensitivity;
		d_y = (float)(this->mouse_LeftDown_Y - this->mouse_Y) * UI_parameters::instance()->UI_camera_mouse_rotation_sensitivity;
	...
	{
		float d_x, d_y;
		d_x = (float)(this->mouse_RightDown_X - this->mouse_X) * UI_parameters::instance()->UI_camera_mouse_translation_sensitivity;
		d_y = (float)(this->mouse_RightDown_Y - this->mouse_Y) * UI_parameters::instance()->UI_camera_mouse_translation_sensitivity;
	...
	if (this->normal_key['c'] == 1 || this->normal_key['C'] == 1)
		this->SLAM_system_ptr->processing_state = ProcessingState::PROCESS_CONTINUOUS_FRAME;
	// Single frame image processing
	if (this->normal_key['f'] == 1 || this->normal_key['F'] == 1)
		this->SLAM_system_ptr->processing_state = ProcessingState::PROCESS_SINGLE_FRAME;

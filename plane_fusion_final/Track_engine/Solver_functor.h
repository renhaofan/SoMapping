...
void compute_pose_mat_from_result(const Eigen::Matrix<float, 6, 1> & result_vector,
...
public:
							  ...
							  Eigen::Matrix<float, 4, 4> & incremental_pose)
	{
		...
		float condition_number = hessian.lpNorm<Eigen::Infinity>() * hessian.inverse().lpNorm<Eigen::Infinity>();
	...
	//! Compute pose matrix from result vector
	friend void compute_pose_mat_from_result(const Eigen::Matrix<float, 6, 1> & result_vector,
											 Eigen::Matrix4f & output_pose_mat);

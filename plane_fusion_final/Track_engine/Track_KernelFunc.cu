...
//
inline __device__ bool interpolate_bilinear(float u, float v,
											const My_Type::Vector2f * vec2_image,
											int layer_width, int layer_height,
											My_Type::Vector2f & interpolated_point);
//
inline __device__ bool interpolate_bilinear(float u, float v,
											const My_Type::Vector3f * vec3_image,
											int layer_width, int layer_height,
...
__global__ void compute_points_residual_KernelFunc(const My_Type::Vector3f * current_points,
												   const My_Type::Vector3f * model_points,
												   const My_Type::Vector3f * current_normals,
												   const My_Type::Vector3f * model_normals,
												   const float * points_weight,
												   ...
												   Accumulate_result * accumulate_result)
{
	...
	if (u_current < 0 || u_current >= layer_width || v_current < 0 || v_current >= layer_height)	valid_point = false;
	...
	if (valid_point)
	{
		...
		trans_point.x = incremental_pose.m00 * current_point.x + incremental_pose.m01 * current_point.y + incremental_pose.m02 * current_point.z + incremental_pose.m03;
		trans_point.y = incremental_pose.m10 * current_point.x + incremental_pose.m11 * current_point.y + incremental_pose.m12 * current_point.z + incremental_pose.m13;
		trans_point.z = incremental_pose.m20 * current_point.x + incremental_pose.m21 * current_point.y + incremental_pose.m22 * current_point.z + incremental_pose.m23;

		// Project current point to (u, v) coordinate (Find correspondence points)
		if (layer_id == 0)
		{
			u_model = roundf(trans_point.x / trans_point.z * sensor_param.sensor_fx + sensor_param.sensor_cx - 0.5f);
			v_model = roundf(trans_point.y / trans_point.z * sensor_param.sensor_fy + sensor_param.sensor_cy - 0.5f);
		}
		else
		{
			u_model = roundf(trans_point.x / trans_point.z * sensor_param.sensor_fx + sensor_param.sensor_cx);
			v_model = roundf(trans_point.y / trans_point.z * sensor_param.sensor_fy + sensor_param.sensor_cy);
			u_model = u_model / (float)(1 << layer_id);
			v_model = v_model / (float)(1 << layer_id);
		}
		// Validate memory access
		if (u_model <= FLT_EPSILON || u_model >= (float)(layer_width - (1 << layer_id))
			|| v_model <= FLT_EPSILON || v_model >= (float)(layer_height - (1 << layer_id)))	valid_point = false;
	}
	...
	if (valid_point)
	{
		...
		trans_normal.x = incremental_pose.m00 * current_normal.x + incremental_pose.m01 * current_normal.y + incremental_pose.m02 * current_normal.z;
		trans_normal.y = incremental_pose.m10 * current_normal.x + incremental_pose.m11 * current_normal.y + incremental_pose.m12 * current_normal.z;
		trans_normal.z = incremental_pose.m20 * current_normal.x + incremental_pose.m21 * current_normal.y + incremental_pose.m22 * current_normal.z;
		...
		valid_point = interpolate_bilinear(u_model, v_model, model_points, layer_width, layer_height, model_point, noise_radius);
		//valid_point = interpolate_bilinear(u_model, v_model, model_points, layer_width, layer_height, model_point, 0.01);
		float normal_continuous_threshold = 0.1;
		valid_point = interpolate_bilinear(u_model, v_model, model_normals, layer_width, layer_height, model_normal, normal_continuous_threshold);
#else
	...
	if (valid_point)
	{
...
#endif
		// p_{i} \times n_{i}
		A_array[0] = (trans_point.y * model_normal.z - trans_point.z * model_normal.y);
		A_array[1] = (trans_point.z * model_normal.x - trans_point.x * model_normal.z);
		A_array[2] = (trans_point.x * model_normal.y - trans_point.y * model_normal.x);
		// n_{i}
		...
		// (p_{i} - q_{i}) \cdot n_{i}
		b_value = diff_vec.x * model_normal.x + diff_vec.y * model_normal.y + diff_vec.z * model_normal.z;
	}
	else
	{
		A_array[0] = 0;		A_array[3] = 0;
		A_array[1] = 0;		A_array[4] = 0;
		A_array[2] = 0;		A_array[5] = 0;
		b_value = 0;
	}
	__syncthreads();
...
#pragma region(Reduce)
	...
	for (int i = 0; i < 6; i++)
	{
		for (int j = 0; j <= i; j++)
		{
			...
			if (tid == 0)
			{
				atomicAdd(&accumulate_result->hessian_upper[(i + 1) * i / 2 + j], cache_f[0]);
			}
		}
	}
	// ATb = Nabla / 2
...
//
void compute_points_residual_CUDA(dim3 block_rect, dim3 thread_rect,
								  ...
								  Accumulate_result * accumulate_result)
{
	compute_points_residual_KernelFunc << <block_rect, thread_rect >> >(current_points, model_points, 
																		current_normals, model_normals,
																		points_weight, 
																		sensor_param, 
																		incremental_pose, 
																		layer_id, 
...
//
__global__ void generate_correspondence_lines_KernelFunc(const My_Type::Vector3f * current_points,
														 const My_Type::Vector3f * model_points,
														 const My_Type::Vector3f * current_normals,
														 const My_Type::Vector3f * model_normals,
														 Sensor_params sensor_param,
														 ...
														 My_Type::Vector3f * correspondence_lines)
{
	...
	if (valid_point)
	{
		...
		if (u_model <= FLT_EPSILON || u_model >= (float)layer_width
			|| v_model <= FLT_EPSILON || v_model >= (float)layer_height)	valid_point = false;
	}
	...
	{
		correspondence_lines[current_index * 2 + 0] = My_Type::Vector3f(0.0, 0.0, 0.0);
		correspondence_lines[current_index * 2 + 1] = My_Type::Vector3f(0.0, 0.0, 0.0);
	}
}
//
void generate_correspondence_lines_CUDA(dim3 block_rect, dim3 thread_rect,
										const My_Type::Vector3f * current_points,
										const My_Type::Vector3f * model_points,
										const My_Type::Vector3f * current_normals,
										const My_Type::Vector3f * model_normals,
										...
										My_Type::Vector3f * correspondence_lines)
{
	generate_correspondence_lines_KernelFunc << <block_rect, thread_rect >> >(current_points, model_points, current_normals, model_normals,
...
__global__ void compute_photometric_residual_KernelFunc(const My_Type::Vector3f * current_points, 
														const float * current_intensity,
														const float * model_intensity,
														const My_Type::Vector2f * model_gradient,
														const float * points_weight,
														Sensor_params sensor_param,
														My_Type::Matrix44f incremental_pose,
														int layer_id,
														Accumulate_result * accumulate_result)
{
	...
	int current_index = u_current + v_current * layer_width;
	// Validate memory access
	if (u_current < 0 || u_current >= layer_width || v_current < 0 || v_current >= layer_height)	valid_pixel = false;
	...
	if (valid_pixel)
	{
		...
		// Validate memory access
		if (u_model <= FLT_EPSILON || u_model >= (float)(layer_width - (1 << layer_id))
			|| v_model <= FLT_EPSILON || v_model >= (float)(layer_height - (1 << layer_id)))	valid_pixel = false;
	}
	...
	if (valid_pixel)
	{
		...
		valid_pixel |= interpolate_bilinear(u_model, v_model, model_intensity, layer_width, layer_height, 
											model_pixel_intensity);
		valid_pixel |= interpolate_bilinear(u_model, v_model, model_gradient, layer_width, layer_height,
											model_gradient_vec);
		if (current_pixel_intensity == 0.0f || model_pixel_intensity == 0.0f)	valid_pixel = false;
		intensity_diff = model_pixel_intensity - current_pixel_intensity;
		if (fabsf(intensity_diff) < 0.10f || fabsf(intensity_diff) > 0.9f)		valid_pixel = false;
	}
	...
	if (valid_pixel)
	{
		...
		gradient_projection[2] = model_gradient_vec.x * sensor_param.sensor_cx + model_gradient_vec.y * sensor_param.sensor_cy;

		//
		nabla[0] = -gradient_projection[1] * current_point.z + gradient_projection[2] * current_point.y;
		nabla[1] = +gradient_projection[0] * current_point.z - gradient_projection[2] * current_point.x;
		nabla[2] = -gradient_projection[0] * current_point.y + gradient_projection[1] * current_point.x;
	...
	{
		nabla[0] = 0.0f;	nabla[1] = 0.0f;	nabla[2] = 0.0f;
		nabla[3] = 0.0f;	nabla[4] = 0.0f;	nabla[5] = 0.0f;
	}
...
// Photometric residual
void compute_photometric_residual_CUDA(dim3 block_rect, dim3 thread_rect,
									   const My_Type::Vector3f * current_points,
									   const float * current_intensity,
									   const float * model_intensity,
									   const My_Type::Vector2f * model_gradient,
									   ...
									   Accumulate_result * accumulate_result)
{
	compute_photometric_residual_KernelFunc << <block_rect, thread_rect >> >(current_points, current_intensity, model_intensity, model_gradient,
...
#if(1) /* for robust */
	if (u < FLT_EPSILON || u >= layer_width - 1.0f || v < FLT_EPSILON || v >= layer_height - 1.0f)	return false;
#endif
...
#define CHECK_COUNTINUOUS
	...
	if ((interpolated_point - point_00).norm() > continuous_threshold)	return false;
	if ((interpolated_point - point_01).norm() > continuous_threshold)	return false;
	if ((interpolated_point - point_10).norm() > continuous_threshold)	return false;
	if ((interpolated_point - point_11).norm() > continuous_threshold)	return false;
#endif

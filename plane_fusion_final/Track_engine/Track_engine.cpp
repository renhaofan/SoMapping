...
//
void copy_out_hessian_from_accumulate_data(
    Eigen::Matrix<float, 6, 6> &out_hessian,
    Eigen::Matrix<float, 6, 1> &out_nabla, const Accumulate_result &accu_data) {
  float *ptr_f = nullptr;
...
void Track_engine::init() {
  ...
  int buffer_size =
      ...
      sizeof(My_Type::Vector3f) * 2;
  this->correspondence_lines = (My_Type::Vector3f *)malloc(buffer_size);
  checkCudaErrors(
      cudaMalloc((void **)&(this->dev_correspondence_lines), buffer_size));
#endif
}
...
#ifdef COMPILE_DEBUG_CODE
void Track_engine::generate_icp_correspondence_lines(
  ...
  memcpy(cache_increment_pose.data, this->incremental_pose.data(),
         4 * 4 * sizeof(float));
  ...
  block_rect.z = 1;
  generate_correspondence_lines_CUDA(
      block_rect, thread_rect, dev_current_points_hierarchy.data_ptrs[0],
  ...
  int buffer_size =
      ...
      SLAM_system_settings::instance()->aligned_depth_size.height *
      sizeof(My_Type::Vector3f) * 2;
  checkCudaErrors(cudaMemcpy(this->correspondence_lines,
                             this->dev_correspondence_lines, buffer_size,
...
Basic_ICP_tracker::Basic_ICP_tracker() {
  checkCudaErrors(cudaMalloc((void **)&(this->dev_accumulation_buffer),
                             sizeof(Accumulate_result)));
}
Basic_ICP_tracker::~Basic_ICP_tracker() {
...
TrackingState Basic_ICP_tracker::track_camera_pose(
  ...
  // for (int layer_id = 0;
  for (int layer_id =
           SLAM_system_settings::instance()->number_of_hierarchy_layers - 1;
       layer_id >= 0; layer_id--) {
    //
    ...
    for (iterate_times[layer_id] = 0;
         ...
         SLAM_system_settings::instance()->max_iterate_times[layer_id];
         iterate_times[layer_id]++) {
      //
      ...
      // ----- Residual computation (Hierarchy_image --> Hessian, Nabla)
      {
        ...
        thread_rect.z = 1;
        block_rect.x =
            dev_current_points_hierarchy.size[layer_id].width / thread_rect.x;
        block_rect.y =
            dev_current_points_hierarchy.size[layer_id].height / thread_rect.y;
        block_rect.z = 1;
        // Lunch kernel function
        compute_points_residual_CUDA(
        ...
        checkCudaErrors(cudaMemcpy(
            (void *)&this->icp_accumulation, this->dev_accumulation_buffer,
            sizeof(Accumulate_result), cudaMemcpyDeviceToHost));
        copy_out_hessian_from_accumulate_data(icp_hessian, icp_nabla,
                                              this->icp_accumulation);
        // Normalize residual
      ...
      // ----- Residual computation (Hierarchy_image --> Hessian, Nabla)
      {
        ...
        block_rect.z = 1;
        // Lunch kernel function
        compute_photometric_residual_CUDA(
            block_rect, thread_rect,
            dev_current_points_hierarchy.data_ptrs[layer_id],
        ...
        checkCudaErrors(cudaMemcpy(
            (void *)&this->icp_accumulation, this->dev_accumulation_buffer,
            sizeof(Accumulate_result), cudaMemcpyDeviceToHost));
        copy_out_hessian_from_accumulate_data(
            photometric_hessian, photometric_nabla, this->icp_accumulation);
        // Normalize residual
      ...
      // ----- Solver (Hessian, Nabla --> X=[rx,ry,rz,tx,ty,tz] --> increment
      // pose matrix)
      //
      {
        float photometric_weight = 0.5;
        //
        this->total_hessian = icp_hessian + photometric_weight *
                                                photometric_weight *
                                                photometric_hessian;
        this->total_nabla =
            -(icp_nabla + photometric_weight * photometric_nabla);

        // Compute average point distance
        float average_distance = this->icp_accumulation.energy /
                                 (float)this->icp_accumulation.number_of_pairs;

        //
        // IterationState state = my_solver(this->total_hessian,
        // this->total_nabla, this->incremental_pose);
        IterationState state = my_solver(
            this->total_hessian, this->total_nabla, average_distance,
            this->icp_accumulation.number_of_pairs, this->incremental_pose);
        if (state == IterationState::FAILED ||
            state == IterationState::CONVERGENCED) {
          // printf("%f\n", average_energy);
          break;
        }
      }
...
void Basic_ICP_tracker::prepare_to_new_iteration() {
  ...
  //
  checkCudaErrors(cudaMemset(this->dev_accumulation_buffer, 0x00,
                             sizeof(Accumulate_result)));
...
//
TrackingState Keypoint_ICP_tracker::track_camera_pose(
    const Hierarchy_image<My_Type::Vector3f> &dev_current_points_hierarchy,
    const Hierarchy_image<My_Type::Vector3f> &dev_model_points_hierarchy,
    const Hierarchy_image<My_Type::Vector3f> &dev_current_normals_hierarchy,
    const Hierarchy_image<My_Type::Vector3f> &dev_model_normals_hierarchy,
  ...
  for (int layer_id =
    ...
    for (iterate_times[layer_id] = 0;
      ...
      // ----- Residual computation (Hierarchy_image --> Hessian, Nabla)
      {
        ...
        float hessian_norm_infinit_inv =
            icp_hessian.inverse().lpNorm<Eigen::Infinity>();
      ...
      // ----- Residual computation (Hierarchy_image --> Hessian, Nabla)
      {
        ...
        // Copy out result
        checkCudaErrors(cudaMemcpy((void *)&this->photometric_accumulation,
                                   this->dev_accumulation_buffer,
                                   sizeof(Accumulate_result),
                                   cudaMemcpyDeviceToHost));
        copy_out_hessian_from_accumulate_data(photometric_hessian,
                                              photometric_nabla,
                                              this->photometric_accumulation);
        // Normalize residual
      ...
      // ----- Residual computation (Keypoints --> Hessian, Nabla)
      {
        ...
        Eigen::Matrix4f cache_increment_pose_mat(
            this->cache_increment_pose.data);
        ...
        for (int point_id = 0; point_id < number_of_keypoints; point_id++) {
          ...
          current_point = cache_increment_pose_mat.block(0, 0, 3, 3) *
                              current_point.eval() +
                          cache_increment_pose_mat.block(0, 3, 3, 1);
          ...
          //
          J_mat.data()[1] = -current_point.z();
          J_mat.data()[2] = +current_point.y();
          J_mat.data()[6] = -current_point.y();
          J_mat.data()[3] = +current_point.z();
          J_mat.data()[5] = -current_point.x();
          J_mat.data()[7] = +current_point.x();
          //
          ...
          if (point_diff < huber_radius) {
            ...
            for (int i = 0; i < 3; i++)
              clamp_b_mat.data()[i] =
                  fmaxf(fminf(b_mat.data()[i], this->keypoint_huber),
                        -this->keypoint_huber);
    ...
    Solve_one_iteration : {
      this->total_hessian =
          icp_weight * icp_hessian +
          photometric_weight * photometric_weight * photometric_hessian +
          keypoint_weight * keypoint_hessian;
      this->total_nabla =
          -(icp_weight * icp_nabla + photometric_weight * photometric_nabla +
            keypoint_weight * keypoint_nabla);

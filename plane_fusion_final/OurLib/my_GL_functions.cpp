...
// OpenGL      CUDA
void GL_buffer_object_init_with_CUDA(GLuint * GL_BO, cudaGraphicsResource ** CUDA_GR, const void* data, \
	unsigned int size, GLenum GL_BO_catalogue, GLenum GL_copy_flag, unsigned int CUDA_flag)
{
	//      
...
void checkGL(const string& func, const char *const file, int const line)
{
	...
	while ((err = glGetError()) != GL_NO_ERROR){
		...
		fprintf(stderr, "GL error at %s:%d detected error: %s at %s\n",
			file, line, errInfo.c_str(), func.c_str());
		exit(0);
	}
}

...
My_pose::My_pose()
{
	...
	checkCudaErrors(cudaMalloc((void **)&(this->dev_mat_inv), 16 * sizeof(float)));
	//
...
void My_pose::synchronize_to_GPU()
{
	...
	//
	checkCudaErrors(cudaMemcpy(this->dev_mat, this->mat.data(), 16 * sizeof(float), cudaMemcpyHostToDevice));
	checkCudaErrors(cudaMemcpy(this->dev_mat_inv, mat_inv.data(), 16 * sizeof(float), cudaMemcpyHostToDevice));
}
...
{
	checkCudaErrors(cudaMemcpy(this->mat.data(), this->dev_mat, 16 * sizeof(float), cudaMemcpyDeviceToHost));
}
...
void My_pose::transformation_Rt_divided(My_pose & pose_transformation)
{
	...
	// 
	this->mat.block(0, 3, 3, 1) = this->mat.block(0, 3, 3, 1) - pose_transformation.mat.block(0, 3, 3, 1);
	// 

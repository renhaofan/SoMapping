...
//
Trajectory_node::Trajectory_node(double timestamp,
...
//
std::ostream & operator<< (std::ostream & out_stream, const Trajectory_node & _node)
{
	out_stream	<< _node.time << "\t" 
				<< _node.tx << "\t" << _node.ty << "\t" << _node.tz << "\t" 
				<< _node.quaternions;

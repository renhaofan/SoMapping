...
compute_2D_bounding_box(const My_Type::Vector3f &vertex,
  ...
  // x-1 y-1 z-1
  temp_vertex =
      vertex + My_Type::Vector3f(block_width, block_width, block_width);
  vertex_on_plane.x = temp_vertex.dot(plane_coordinate.x_vec);
  vertex_on_plane.y = temp_vertex.dot(plane_coordinate.y_vec);
  min_max_2D_bounding_box(vertex_on_plane, bounding_box);
}
//
__global__ void build_allocate_flag_KernelFunc(
    const HashEntry *allocated_entries, const Voxel_f *voxel_block_array,
    const PlaneHashEntry *plane_entries, Plane_coordinate plane_coordinate,
  ...
  if (is_valid_entry) {
    voxel_block_vertex.x =
        allocated_entries[voxel_entry_id].position[0] * block_width;
    voxel_block_vertex.y =
        allocated_entries[voxel_entry_id].position[1] * block_width;
    voxel_block_vertex.z =
        allocated_entries[voxel_entry_id].position[2] * block_width;
  }
  ...
  if (tid == 0 && is_valid_entry) {
    ...
    compute_2D_bounding_box(voxel_block_vertex, plane_coordinate,
                            pixel_bounding_box);
    //
    float pixel_block_width = PLANE_PIXEL_BLOCK_WIDTH * PLANE_PIXEL_SIZE;
    int x_start = floor_by_stride(pixel_bounding_box[0].x, pixel_block_width) /
                      pixel_block_width +
                  FLT_EPSILON;
    int x_end = floor_by_stride(pixel_bounding_box[1].x, pixel_block_width) /
                    pixel_block_width +
                FLT_EPSILON;
    int y_start = floor_by_stride(pixel_bounding_box[0].y, pixel_block_width) /
                      pixel_block_width +
                  FLT_EPSILON;
    int y_end = floor_by_stride(pixel_bounding_box[1].y, pixel_block_width) /
                    pixel_block_width +
                FLT_EPSILON;
    //
    for (int pixel_block_x = x_start; pixel_block_x <= x_end; pixel_block_x++)
      for (int pixel_block_y = y_start; pixel_block_y <= y_end;
           pixel_block_y++) {
        ...
        if (!is_allocated) {
          ...
          allocate_pixel_block_pos[hash_value] =
              My_Type::Vector2i(pixel_block_x, pixel_block_y);
        }
      }
  }
}
...
//
__global__ void allocate_plane_blocks_KernelFunc(
    bool *need_allocate, const My_Type::Vector2i *allocate_pixel_block_pos,
  ...
  if (entry_need_allocate) {
    ...
    // Allocate
    plane_entries[entry_index].position[0] =
        allocate_pixel_block_pos[hash_value].x;
    plane_entries[entry_index].position[1] =
        allocate_pixel_block_pos[hash_value].y;
    plane_entries[entry_index].ptr =
        (int)PLANE_PIXEL_BLOCK_SIZE * atomicAdd(number_of_blocks, 1);
  }
}
//
...
//
__global__ void find_allocated_planar_entries_KernelFunc(
    const PlaneHashEntry *entries, int *index_array, int *allocated_counter) {
  bool is_allocated = true;
  int entry_index = blockIdx.x * blockDim.x + threadIdx.x;
  //
  if (entries[entry_index].ptr >= 0)
    index_array[atomicAdd(allocated_counter, 1)] = entry_index;
}
//
...
//
__global__ void fusion_sdf_to_plane_KernelFunc(
    const HashEntry *voxel_entries, const Voxel_f *voxel_block_array,
    const int *entries_index_list, Plane_info model_plane,
    Plane_coordinate plane_coordinate, PlaneHashEntry *plane_entries,
  ...
  if (is_valid_pixel) {
    //
    float px = plane_entries[entry_id].position[0] * PLANE_PIXEL_BLOCK_WIDTH_M +
               threadIdx.x * PLANE_PIXEL_SIZE + HALF_PLANE_PIXEL_SIZE;
    float py = plane_entries[entry_id].position[1] * PLANE_PIXEL_BLOCK_WIDTH_M +
               threadIdx.y * PLANE_PIXEL_SIZE + HALF_PLANE_PIXEL_SIZE;
    ...
    //
    int voxel_index = get_voxel_index_neighbor(pixel_point.x, pixel_point.y,
                                               pixel_point.z, voxel_entries);
    if (voxel_index >= 0) {
      if (voxel_block_array[voxel_index].weight <= MIN_RAYCAST_WEIGHT)
        is_valid_pixel = false;
      if (voxel_block_array[voxel_index].plane_index != model_plane.plane_index)
        is_valid_pixel = false;
      //
      is_valid_pixel |=
  ...
  if (is_valid_pixel) {
    ...
    for (int i = 0; i < 8; i++) {
      ...
      is_valid_pixel |=
          get_sdf_interpolated(temp_point.x, temp_point.y, temp_point.z,
                               voxel_entries, voxel_block_array, sdf);
      //
...
//
__global__ void generate_block_vertex_KernelFunc(
    const PlaneHashEntry *entries, Plane_coordinate plane_coordinate,
    Plane_info model_plane, My_Type::Vector3f *block_vertexs,
  ...
  if (is_allocated) {
    ...
    // 00
    temp_vertex[0] = pixel_block_width * (float)temp_entry.position[0] *
                         plane_coordinate.x_vec +
                     pixel_block_width * (float)temp_entry.position[1] *
                         plane_coordinate.y_vec +
                     (float)(-model_plane.d) * plane_coordinate.z_vec;
    //// 10
    // temp_vertex[1] = temp_vertex[0] + pixel_block_width *
    // plane_coordinate.y_vec;
    //// 11
    // temp_vertex[2] = temp_vertex[0] + pixel_block_width *
    // (plane_coordinate.x_vec + plane_coordinate.y_vec);
    //// 01
    // temp_vertex[3] = temp_vertex[0] + pixel_block_width *
    // plane_coordinate.x_vec;
    // 10
    temp_vertex[1] =
        temp_vertex[0] + 0.8 * pixel_block_width * plane_coordinate.y_vec;
    // 11
    temp_vertex[2] =
        temp_vertex[0] + 0.8 * pixel_block_width *
                             (plane_coordinate.x_vec + plane_coordinate.y_vec);
    // 01
    temp_vertex[3] =
        temp_vertex[0] + 0.8 * pixel_block_width * plane_coordinate.x_vec;

    //
    vertex_buffer[buffer_index + 0] = temp_vertex[0];
    vertex_buffer[buffer_index + 1] = temp_vertex[1];
    vertex_buffer[buffer_index + 2] = temp_vertex[1];
    vertex_buffer[buffer_index + 3] = temp_vertex[2];
    vertex_buffer[buffer_index + 4] = temp_vertex[2];
    vertex_buffer[buffer_index + 5] = temp_vertex[3];
    vertex_buffer[buffer_index + 6] = temp_vertex[3];
    vertex_buffer[buffer_index + 7] = temp_vertex[0];
  }
  ...
  if (tid < cache_block_number) {
    ...
    block_vertexs[array_offset + tid * 8 + 0] = vertex_buffer[tid * 8 + 0];
    __threadfence_block();
    block_vertexs[array_offset + tid * 8 + 1] = vertex_buffer[tid * 8 + 1];
    __threadfence_block();
    block_vertexs[array_offset + tid * 8 + 2] = vertex_buffer[tid * 8 + 2];
    __threadfence_block();
    block_vertexs[array_offset + tid * 8 + 3] = vertex_buffer[tid * 8 + 3];
    __threadfence_block();
    block_vertexs[array_offset + tid * 8 + 4] = vertex_buffer[tid * 8 + 4];
    __threadfence_block();
    block_vertexs[array_offset + tid * 8 + 5] = vertex_buffer[tid * 8 + 5];
    __threadfence_block();
    block_vertexs[array_offset + tid * 8 + 6] = vertex_buffer[tid * 8 + 6];
    __threadfence_block();
    block_vertexs[array_offset + tid * 8 + 7] = vertex_buffer[tid * 8 + 7];
    __threadfence_block();
  }
}
//

...
Plane_map::Plane_map()
{
	...
	//
	checkCudaErrors(cudaMalloc((void **)&this->dev_plane_list, MAX_CURRENT_PLANES * sizeof(Plane_info)));
	...
	checkCudaErrors(cudaMalloc((void **)&(this->dev_number_of_pixel_block), sizeof(int)));
	//
	checkCudaErrors(cudaMalloc((void **)&(this->dev_pixel_block_need_allocate), ORDERED_TABLE_LENGTH * sizeof(bool)));
	checkCudaErrors(cudaMalloc((void **)&(this->dev_pixel_block_buffer), ORDERED_TABLE_LENGTH * sizeof(My_Type::Vector2i)));
	checkCudaErrors(cudaMalloc((void **)&(this->dev_plane_entry_excess_counter), sizeof(int)));
	//
	checkCudaErrors(cudaMalloc((void **)&(this->dev_allocated_counter), sizeof(int)));
	checkCudaErrors(cudaMalloc((void **)&(this->dev_allocated_entry_index),
		(ORDERED_PLANE_TABLE_LENGTH + EXCESS_PLANE_TABLE_LENGTH) * sizeof(PlaneHashEntry)));
...
Plane_map::~Plane_map()
{
	...
	for (int plane_id = 0; plane_id < this->dev_plane_entry_list.size(); plane_id++)
	{	checkCudaErrors(cudaFree(this->dev_plane_entry_list[plane_id]));	}
	for (int plane_id = 0; plane_id < this->dev_plane_pixel_array_list.size(); plane_id++)
								  ...
								  std::vector<My_Type::Vector2i> & matches)
{
	...
	for (int match_id = 1; match_id < matches.size(); match_id++)
	{
		...
		else if (model_plane_index > 0)
		{
			...
			model_params = model_params * model_weight + current_params * current_weight;
			...
			this->plane_list[model_plane_index].weight = fmin(model_weight + current_weight, FLT_MAX);
			//printf("current_weight = %f\n", current_weight);
			//printf("model_weight = %f\n", model_weight);
			//printf("%f, %f, %f, %f\n", current_params.x, current_params.y, current_params.z, current_params.w);
	...
	if (false)
	{
		...
		for (int plane_id = 0; plane_id < this->plane_counter; plane_id++)
		{
			printf("%d : (%f, %f, %f), %f ",
				   plane_id,
				   this->plane_list[plane_id].nx,
				   this->plane_list[plane_id].ny,
				   this->plane_list[plane_id].nz,
								   ...
								   const Voxel_f * dev_voxel_array)
{
	...
	// Update coordinate of model planes
	{
		...
		for (int plane_id = 1; plane_id < this->plane_coordinate_list.size(); plane_id++)
		{
			// Update axis-Z
			...
			//
			this->plane_coordinate_list[plane_id].x_vec = My_Type::Vector3f(base_x.x(), base_x.y(), base_x.z());
			this->plane_coordinate_list[plane_id].y_vec = My_Type::Vector3f(base_y.x(), base_y.y(), base_y.z());
			this->plane_coordinate_list[plane_id].z_vec = My_Type::Vector3f(base_z.x(), base_z.y(), base_z.z());
		}

		// New plane coordinate
		for (int plane_id = this->plane_coordinate_list.size(); plane_id < this->plane_counter; plane_id++)
		{
			// Random generate coordinate
			Eigen::Vector3f base_x, base_y, base_z;
			base_z.x() = this->plane_list[plane_id].nx;
			base_z.y() = this->plane_list[plane_id].ny;
			base_z.z() = this->plane_list[plane_id].nz;
			//std::cout << plane_id << " - base_z : " << base_z.transpose() << std::endl;
			// Base vector X
			do{
				base_x.setRandom();
				base_x.normalize();
			...
			temp_coordinate.x_vec.x = base_x.x();	temp_coordinate.x_vec.y = base_x.y();	temp_coordinate.x_vec.z = base_x.z();
			temp_coordinate.y_vec.x = base_y.x();	temp_coordinate.y_vec.y = base_y.y();	temp_coordinate.y_vec.z = base_y.z();
			temp_coordinate.z_vec.x = base_z.x();	temp_coordinate.z_vec.y = base_z.y();	temp_coordinate.z_vec.z = base_z.z();
	...
	// Find allocated entries
	{
		...
		find_alllocated_entries_CUDA(block_rect, thread_rect,
									 dev_entries, (ORDERED_TABLE_LENGTH + EXCESS_TABLE_LENGTH),
...
//		CUDA_CKECK_KERNEL;
	...
	// Generate 2D plane structure
	{
		...
		{
			checkCudaErrors(cudaMemset(this->dev_number_of_pixel_block, 0x00, sizeof(int)));
			checkCudaErrors(cudaMemset(this->dev_plane_entry_excess_counter, 0x00, sizeof(int)));
			checkCudaErrors(cudaMemset(temp_plane_pixel_array, 0x00,
				PIXEL_BLOCK_NUM * PLANE_PIXEL_BLOCK_SIZE * sizeof(Plane_pixel)));

			// Allocate memory
			PlaneHashEntry * temp_entry = nullptr;
			checkCudaErrors(cudaMalloc((void **)&temp_entry,
				(ORDERED_PLANE_TABLE_LENGTH + EXCESS_PLANE_TABLE_LENGTH) * sizeof(PlaneHashEntry)));
			checkCudaErrors(cudaMemset(temp_entry, 0xFF,
				(ORDERED_PLANE_TABLE_LENGTH + EXCESS_PLANE_TABLE_LENGTH) * sizeof(PlaneHashEntry)));
			this->dev_plane_entry_list.push_back(temp_entry);

			//PlaneHashEntry temp_buffer_1[ORDERED_PLANE_TABLE_LENGTH + EXCESS_PLANE_TABLE_LENGTH];
			//checkCudaErrors(cudaMemcpy(temp_buffer_1, temp_entry, 1024 * sizeof(PlaneHashEntry), cudaMemcpyDeviceToHost));
			...
			while (new_allocate_num != 0)
			{
				...
				checkCudaErrors(cudaMemset(this->dev_pixel_block_need_allocate, 0x00, ORDERED_TABLE_LENGTH * sizeof(bool)));
				checkCudaErrors(cudaMemset(this->dev_pixel_block_buffer, 0x00, ORDERED_TABLE_LENGTH * sizeof(My_Type::Vector2i)));
				...
				block_rect.y = 1;		block_rect.z = 1;
				build_allocate_flag_CUDA(block_rect, thread_rect,
										 this->dev_allocated_entries, dev_voxel_array, temp_entry,
										 this->plane_coordinate_list[plane_id], plane_id,
...
//				CUDA_CKECK_KERNEL;
				...
				allocate_plane_blocks_CUDA(block_rect, thread_rect,
										   this->dev_pixel_block_need_allocate, this->dev_pixel_block_buffer,
										   temp_entry, this->dev_plane_entry_excess_counter, this->dev_number_of_pixel_block);
//				CUDA_CKECK_KERNEL;

				//
				last_plane_block_num = plane_block_num;
				checkCudaErrors(cudaMemcpy(&plane_block_num, this->dev_number_of_pixel_block, sizeof(int), cudaMemcpyDeviceToHost));
				new_allocate_num = plane_block_num - last_plane_block_num;
			}
			//printf("plane_id : %d\tplane_block_num = %d\n", plane_id, plane_block_num);
			total_plane_block_num += plane_block_num;

			// 

			// Find allocated pixel block array
			checkCudaErrors(cudaMemset(this->dev_allocated_counter, 0x00, sizeof(int)));
			thread_rect.x = 256;	thread_rect.y = 1;	thread_rect.z = 1;
			block_rect.x = (ORDERED_PLANE_TABLE_LENGTH + EXCESS_PLANE_TABLE_LENGTH) / thread_rect.x;
			block_rect.y = 1;		block_rect.z = 1;
			find_allocated_planar_entries_CUDA(block_rect, thread_rect,
											   temp_entry,
											   this->dev_allocated_entry_index, this->dev_allocated_counter);
//			CUDA_CKECK_KERNEL;
			...
			block_rect.y = 1;		block_rect.z = 1;
			fusion_sdf_to_plane_CUDA(block_rect, thread_rect,
									 dev_entries, dev_voxel_array, this->dev_allocated_entry_index,
									 this->plane_list[plane_id], this->plane_coordinate_list[plane_id],
			...
			checkCudaErrors(cudaMemcpy(&number_of_pixel_block, this->dev_number_of_pixel_block, sizeof(int), cudaMemcpyDeviceToHost));
			this->plane_array_length_list.push_back(number_of_pixel_block);
			//
			Plane_pixel * buffer_plane_pixel_array = nullptr;
			checkCudaErrors(cudaMalloc((void **)&buffer_plane_pixel_array,
				number_of_pixel_block * PLANE_PIXEL_BLOCK_SIZE * sizeof(Plane_pixel)));
			checkCudaErrors(cudaMemcpy(buffer_plane_pixel_array, temp_plane_pixel_array,
				number_of_pixel_block * PLANE_PIXEL_BLOCK_SIZE * sizeof(Plane_pixel), cudaMemcpyDeviceToDevice));
...
void Plane_map::generate_planar_block_render_information()
{
	...
	for (int plane_id = 1; plane_id < this->plane_array_length_list.size(); plane_id++)
		number_of_blocks += this->plane_array_length_list[plane_id];

	// Allocate memory for block vertex array
	this->block_vertexs = (My_Type::Vector3f *)malloc(number_of_blocks * 8 * sizeof(My_Type::Vector3f));
	...
	for (int plane_id = 1; plane_id < this->plane_counter; plane_id++)
	{
		...
		block_rect.y = 1;		block_rect.z = 1;
		// Lunch kernel function
		generate_block_vertex_CUDA(block_rect, thread_rect,
								   this->dev_plane_entry_list[plane_id],
	...
	checkCudaErrors(cudaMemcpy(this->block_vertexs, this->dev_block_vertexs,
		number_of_blocks * 8 * sizeof(My_Type::Vector3f), cudaMemcpyDeviceToHost));

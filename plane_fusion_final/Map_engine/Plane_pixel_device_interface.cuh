...
{
	return (((((unsigned int)x) * PRIME_X) ^ (((unsigned int)y) * PRIME_Y)) & (unsigned int)ORDERED_PLANE_TABLE_MASK);
}
									  ...
									  const PlaneHashEntry * plane_entries)
{
	...
	if (find_pixel)
	{
		...
		return (temp_entry.ptr + offset_in_block_x + offset_in_block_y * PLANE_PIXEL_BLOCK_WIDTH);
	}
	else
	{
		return -1;

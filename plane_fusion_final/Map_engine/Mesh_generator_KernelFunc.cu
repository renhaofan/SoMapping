...
//
__global__ void find_alllocated_entries_KernelFunc(const HashEntry * entries,
												   ...
												   int * number_of_allocated_entries)
{
	...
	if (threadIdx.x == 0)
	{
		allocated_entry_offset = atomicAdd(number_of_allocated_entries, allocated_number);
	}
								  ...
								  int * number_of_allocated_entries)
{
	...
	find_alllocated_entries_KernelFunc << <block_rect, thread_rect >> >(entries, max_number_of_entries,
																		allocated_entries, number_of_allocated_entries);
}



//
__global__ void find_nonplanar_blocks_KernelFunc(const HashEntry * entries,
												 ...
												 int * number_of_nonplanar_block)
{
	...
	block_offset.x = (float)temp_entry.position[0] * VOXEL_BLOCK_WDITH * VOXEL_SIZE;
	block_offset.y = (float)temp_entry.position[1] * VOXEL_BLOCK_WDITH * VOXEL_SIZE;
	block_offset.z = (float)temp_entry.position[2] * VOXEL_BLOCK_WDITH * VOXEL_SIZE;
	//
	...
	int voxel_index = get_voxel_index_neighbor(point_vec.x, point_vec.y, point_vec.z, entries);
	...
	if (tid == 0)
	{
		...
		//
		float nonplanar_ratio = (float)nonplanar_voxel_number[0] / (float)surface_voxel_number[0];
...
void find_nonplanar_blocks_CUDA(dim3 block_rect, dim3 thread_rect,
								...
								int * number_of_nonplanar_block)
{
	find_nonplanar_blocks_KernelFunc << <block_rect, thread_rect >> >(entries, allocated_entries, voxel_block_array,
																	  nonplanar_entries, number_of_nonplanar_block);
}
...
__device__ inline bool get_sdf_from_voxel_block(const HashEntry & this_entry,
												const HashEntry * entries,
												const Voxel_f * voxel_block_array,
												My_Type::Vector3f * sdf_position,
												float * sdf_value)
{
	My_Type::Vector3f block_offset;
	block_offset.x = (float)this_entry.position[0] * VOXEL_BLOCK_WDITH * VOXEL_SIZE;
	block_offset.y = (float)this_entry.position[1] * VOXEL_BLOCK_WDITH * VOXEL_SIZE;
	block_offset.z = (float)this_entry.position[2] * VOXEL_BLOCK_WDITH * VOXEL_SIZE;
	...
	is_valid_cube = get_sdf_interpolated(sdf_position[0].x, sdf_position[0].y, sdf_position[0].z, entries, voxel_block_array, sdf_value[0]);
	...
	is_valid_cube = get_sdf_interpolated(sdf_position[1].x, sdf_position[1].y, sdf_position[1].z, entries, voxel_block_array, sdf_value[1]);
	if (!is_valid_cube)	return false;
	// 0 1 1
	...
	is_valid_cube = get_sdf_interpolated(sdf_position[2].x, sdf_position[2].y, sdf_position[2].z, entries, voxel_block_array, sdf_value[2]);
	if (!is_valid_cube)	return false;
	// 0 1 0
	...
	is_valid_cube = get_sdf_interpolated(sdf_position[3].x, sdf_position[3].y, sdf_position[3].z, entries, voxel_block_array, sdf_value[3]);
	if (!is_valid_cube)	return false;
	// 1 0 0
	...
	is_valid_cube = get_sdf_interpolated(sdf_position[4].x, sdf_position[4].y, sdf_position[4].z, entries, voxel_block_array, sdf_value[4]);
	if (!is_valid_cube)	return false;
	// 1 0 1
	...
	is_valid_cube = get_sdf_interpolated(sdf_position[5].x, sdf_position[5].y, sdf_position[5].z, entries, voxel_block_array, sdf_value[5]);
	if (!is_valid_cube)	return false;
	// 1 1 1
	...
	is_valid_cube = get_sdf_interpolated(sdf_position[6].x, sdf_position[6].y, sdf_position[6].z, entries, voxel_block_array, sdf_value[6]);
	if (!is_valid_cube)	return false;
	// 1 1 0
	...
	is_valid_cube = get_sdf_interpolated(sdf_position[7].x, sdf_position[7].y, sdf_position[7].z, entries, voxel_block_array, sdf_value[7]);
	if (!is_valid_cube)	return false;
...
//
__device__ inline My_Type::Vector3f bilinear_interpolate_by_sdf(const My_Type::Vector3f & vertex_1,
																const My_Type::Vector3f & vertex_2,
																float sdf_1,
																float sdf_2)
{
...
//
__global__ void generate_triangle_mesh_KernelFunc(const HashEntry * entries,
												  const HashEntry * allocated_entries,
												  const Voxel_f * voxel_block_array,
												  ...
												  int max_number_of_triangles)
{
	...
	bool is_valid_cube = get_sdf_from_voxel_block(temp_entry, entries, voxel_block_array, sdf_position, sdf_value);

	//
	int cube_index = 0;
	if (is_valid_cube)
	{
		if (sdf_value[0] < 0) cube_index |= 1;	if (sdf_value[1] < 0) cube_index |= 2;
		if (sdf_value[2] < 0) cube_index |= 4;	if (sdf_value[3] < 0) cube_index |= 8;
		if (sdf_value[4] < 0) cube_index |= 16;	if (sdf_value[5] < 0) cube_index |= 32;
		if (sdf_value[6] < 0) cube_index |= 64;	if (sdf_value[7] < 0) cube_index |= 128;
	...
	if (is_valid_cube)
	{
		if (edgeTable[cube_index] & 1)		vertex_list[0] = bilinear_interpolate_by_sdf(sdf_position[0], sdf_position[1], sdf_value[0], sdf_value[1]);
		if (edgeTable[cube_index] & 2)		vertex_list[1] = bilinear_interpolate_by_sdf(sdf_position[1], sdf_position[2], sdf_value[1], sdf_value[2]);
		if (edgeTable[cube_index] & 4)		vertex_list[2] = bilinear_interpolate_by_sdf(sdf_position[2], sdf_position[3], sdf_value[2], sdf_value[3]);
		if (edgeTable[cube_index] & 8)		vertex_list[3] = bilinear_interpolate_by_sdf(sdf_position[3], sdf_position[0], sdf_value[3], sdf_value[0]);
		if (edgeTable[cube_index] & 16)		vertex_list[4] = bilinear_interpolate_by_sdf(sdf_position[4], sdf_position[5], sdf_value[4], sdf_value[5]);
		if (edgeTable[cube_index] & 32)		vertex_list[5] = bilinear_interpolate_by_sdf(sdf_position[5], sdf_position[6], sdf_value[5], sdf_value[6]);
		if (edgeTable[cube_index] & 64)		vertex_list[6] = bilinear_interpolate_by_sdf(sdf_position[6], sdf_position[7], sdf_value[6], sdf_value[7]);
		if (edgeTable[cube_index] & 128)	vertex_list[7] = bilinear_interpolate_by_sdf(sdf_position[7], sdf_position[4], sdf_value[7], sdf_value[4]);
		if (edgeTable[cube_index] & 256)	vertex_list[8] = bilinear_interpolate_by_sdf(sdf_position[0], sdf_position[4], sdf_value[0], sdf_value[4]);
		if (edgeTable[cube_index] & 512)	vertex_list[9] = bilinear_interpolate_by_sdf(sdf_position[1], sdf_position[5], sdf_value[1], sdf_value[5]);
		if (edgeTable[cube_index] & 1024)	vertex_list[10] = bilinear_interpolate_by_sdf(sdf_position[2], sdf_position[6], sdf_value[2], sdf_value[6]);
		if (edgeTable[cube_index] & 2048)	vertex_list[11] = bilinear_interpolate_by_sdf(sdf_position[3], sdf_position[7], sdf_value[3], sdf_value[7]);
	}
	...
	if (is_valid_cube)
	{
		for (int i = 0; triangleTable[cube_index][i] != -1; i += 3)
		{
			...
			{
				triangles[triangleId * 3 + 0] = vertex_list[triangleTable[cube_index][i + 0]];
				triangles[triangleId * 3 + 1] = vertex_list[triangleTable[cube_index][i + 1]];
				triangles[triangleId * 3 + 2] = vertex_list[triangleTable[cube_index][i + 2]];
			}
		}
	}
								 ...
								 int max_number_of_triangles)
{
	...
	generate_triangle_mesh_KernelFunc << <block_rect, thread_rect >> >(entries, allocated_entries, voxel_block_array,
																	   triangles, number_of_triangles, max_number_of_triangles);
}
...
//
__global__ void generate_vertex_normals_KernelFunc(const My_Type::Vector3f * vertex_array,
												   const HashEntry * entries,
												   const Voxel_f * voxel_block_array,
												   My_Type::Vector3f * vertex_normals)
{
	...
	bool is_valid_normal = interpolate_normal_by_sdf(temp_vertex.x, temp_vertex.y, temp_vertex.z, entries, voxel_block_array, normal_vector);
								  ...
								  My_Type::Vector3f * vertex_normals)
{
	...
	generate_vertex_normals_KernelFunc << <block_rect, thread_rect >> >(vertex_array, entries, voxel_block_array, 
																		vertex_normals);
}
...
//
__global__ void generate_vertex_color_KernelFunc(const My_Type::Vector3f * vertex_array,
												 ...
												 My_Type::Vector4uc * vertex_color_array)
{
	...
	// compute normal vector
	int voxel_index = get_voxel_index_neighbor(temp_vertex.x, temp_vertex.y, temp_vertex.z, entries);
	//
								...
								My_Type::Vector4uc * vertex_color_array)
{
	...
	generate_vertex_color_KernelFunc << <block_rect, thread_rect >> >(vertex_array, number_of_vertex, entries, voxel_block_array,
																	  vertex_color_array);
}
//
__global__ void generate_vertex_color_KernelFunc(const My_Type::Vector3f * vertex_array,
												 ...
												 My_Type::Vector4uc * vertex_color_array)
{
	...
	if (is_planar_voxel)
	{
		...
		if (plane_label_mapper[plane_label].y < 0 || plane_label_mapper[plane_label].y > 127)
			printf("plane_label_mapper[plane_label].y = %d\n", plane_label_mapper[plane_label].y);
		if (plane_label_mapper[plane_label].y > 0)		plane_label = plane_label_mapper[plane_label].y;
								...
								My_Type::Vector4uc * vertex_color_array)
{
	...
	generate_vertex_color_KernelFunc << <block_rect, thread_rect >> >(vertex_array, number_of_vertex, entries, voxel_block_array, plane_label_mapper,
																	  vertex_color_array);
}
...
//
__global__ void generate_triangle_mesh_from_plane_KernelFunc(Plane_info model_plane,
															 Plane_coordinate plane_coordinate,
															 ...
															 int * triangle_counter)
{
	...
	int pixel_index_00, pixel_index_01, pixel_index_10, pixel_index_11, u_offset, v_offset;
	if (is_valid_pixel)
	{
	...
	if (is_valid_pixel)
	{
		...
		if (diff_00 == FLT_MAX || diff_01 == FLT_MAX || diff_10 == FLT_MAX || diff_11 == FLT_MAX)	is_valid_pixel = false;
	}
	...
	if (is_valid_pixel)
	{
		...
		compute_vertex_of_plane_pixel(u_offset, v_offset, model_plane.d, diff_00, plane_coordinate, point_00);
		compute_vertex_of_plane_pixel(u_offset + 1, v_offset, model_plane.d, diff_01, plane_coordinate, point_01);
		compute_vertex_of_plane_pixel(u_offset, v_offset + 1, model_plane.d, diff_10, plane_coordinate, point_10);
		compute_vertex_of_plane_pixel(u_offset + 1, v_offset + 1, model_plane.d, diff_11, plane_coordinate, point_11);
	...
	if (tid == 0)
	{
		//if (tid == 0 && number_of_cache_triangles> 0)printf("triangle_counter = %d\n", number_of_cache_triangles);
		vertex_index_offset_s = atomicAdd(triangle_counter, number_of_cache_triangles);
	}
	__syncthreads();
	...
	if (tid < copy_thread_number)
	{
		...
		//
		vertex_array[vertex_index_offset + tid + 0 * copy_thread_number] = vertex_cache[tid + 0 * copy_thread_number];	__threadfence();
		vertex_array[vertex_index_offset + tid + 1 * copy_thread_number] = vertex_cache[tid + 1 * copy_thread_number];	__threadfence();
		vertex_array[vertex_index_offset + tid + 2 * copy_thread_number] = vertex_cache[tid + 2 * copy_thread_number];	__threadfence();
		vertex_array[vertex_index_offset + tid + 3 * copy_thread_number] = vertex_cache[tid + 3 * copy_thread_number];	__threadfence();
		vertex_array[vertex_index_offset + tid + 4 * copy_thread_number] = vertex_cache[tid + 4 * copy_thread_number];	__threadfence();
		vertex_array[vertex_index_offset + tid + 5 * copy_thread_number] = vertex_cache[tid + 5 * copy_thread_number];	__threadfence();
											...
											int * triangle_counter)
{
	...
	generate_triangle_mesh_from_plane_KernelFunc << <block_rect, thread_rect >> >(model_plane, plane_coordinate, plane_entries, plane_pixel_array,
...
// 
__global__ void copy_mesh_to_global_map_KernelFunc(const My_Type::Vector3f * src_vertex_array, 
												   ...
												   My_Type::Vector3f * dst_normal_array)
{
	...
	dst_vertex.x = submap_pose[0] * src_vertex.x + submap_pose[4] * src_vertex.y + submap_pose[8] * src_vertex.z + submap_pose[12];
	dst_vertex.y = submap_pose[1] * src_vertex.x + submap_pose[5] * src_vertex.y + submap_pose[9] * src_vertex.z + submap_pose[13];
	dst_vertex.z = submap_pose[2] * src_vertex.x + submap_pose[6] * src_vertex.y + submap_pose[10] * src_vertex.z + submap_pose[14];
	dst_normal.x = submap_pose[0] * src_normal.x + submap_pose[4] * src_normal.y + submap_pose[8] * src_normal.z;
	dst_normal.y = submap_pose[1] * src_normal.x + submap_pose[5] * src_normal.y + submap_pose[9] * src_normal.z;
	dst_normal.z = submap_pose[2] * src_normal.x + submap_pose[6] * src_normal.y + submap_pose[10] * src_normal.z;
								  ...
								  My_Type::Vector3f * dst_normal_array)
{
	...
	copy_mesh_to_global_map_KernelFunc << <block_rect, thread_rect >> >(src_vertex_array, src_normal_array, submap_pose, number_of_vertex,

...
void Map_engine::init_base() {
  ...
  int ceil_depth_image_height = ceil_by_stride(
...
void Basic_Voxel_map::update_map_after_tracking(
  ...
  this->voxel_map_ptr->allocate_voxel_block(dev_current_points,
                                            camera_pose.mat);
  ...
  if (dev_plane_labels != nullptr) {
    this->voxel_map_ptr->fusion_plane_label_to_voxel(
        dev_current_points, dev_plane_labels, camera_pose.mat);
  }
}
...
void Basic_Voxel_map::raycast_points_from_map(Eigen::Matrix4f &camera_pose,
  ...
  if (raycast_mode == RaycastMode::RAYCAST_FOR_VIEW) {
    checkCudaErrors(cudaMemcpy(
        ...
        this->voxel_map_ptr->scene_depth_width *
            this->voxel_map_ptr->scene_depth_height * sizeof(My_Type::Vector3f),
        cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(
        this->scene_normals, this->voxel_map_ptr->dev_scene_normals,
        this->voxel_map_ptr->scene_depth_width *
            this->voxel_map_ptr->scene_depth_height * sizeof(My_Type::Vector3f),
        cudaMemcpyDeviceToHost));
    checkCudaErrors(
        cudaMemcpy(this->scene_weight, this->voxel_map_ptr->dev_scene_weight,
                   this->voxel_map_ptr->scene_depth_width *
                       this->voxel_map_ptr->scene_depth_height * sizeof(int),
                   cudaMemcpyDeviceToHost));
    checkCudaErrors(cudaMemcpy(
        this->scene_plane_labels, this->voxel_map_ptr->dev_scene_plane_label,
        this->voxel_map_ptr->scene_depth_width *
            this->voxel_map_ptr->scene_depth_height * sizeof(int),
        cudaMemcpyDeviceToHost));

  } else {
    checkCudaErrors(cudaMemcpy(this->dev_model_points,
                               this->voxel_map_ptr->dev_raycast_points,
                               this->voxel_map_ptr->raycast_depth_width *
                                   this->voxel_map_ptr->raycast_depth_height *
                                   sizeof(My_Type::Vector3f),
                               cudaMemcpyDeviceToDevice));
    checkCudaErrors(cudaMemcpy(this->dev_model_normals,
                               this->voxel_map_ptr->dev_raycast_normal,
                               this->voxel_map_ptr->raycast_depth_width *
                                   this->voxel_map_ptr->raycast_depth_height *
                                   sizeof(My_Type::Vector3f),
                               cudaMemcpyDeviceToDevice));
    checkCudaErrors(cudaMemcpy(this->dev_model_plane_labels,
                               this->voxel_map_ptr->dev_raycast_plane_label,
                               this->voxel_map_ptr->raycast_depth_width *
                                   this->voxel_map_ptr->raycast_depth_height *
                                   sizeof(int),
                               cudaMemcpyDeviceToDevice));
...
void Submap_Voxel_map::update_map_after_tracking(
  ...
  this->frame_counter++;
  // Allocation
  int number_of_blocks = this->voxel_map_ptr->allocate_voxel_block(
      dev_current_points, camera_pose.mat);
  // Update
  this->voxel_map_ptr->fusion_SDF_to_voxel(
      dev_current_points, dev_current_normals, camera_pose.mat);

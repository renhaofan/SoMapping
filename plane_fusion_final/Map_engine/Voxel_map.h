...
public:
	...
	//! Voxel block position corresponds to voxel block array(dev_voxel_block_array). 
...
#pragma region(Output, gengerated by raycast module)
	...
	//! CUDA device pointer. Raycast normal (caculate normal vector from SDF of voxel)
...
#pragma endregion
	...
	/*!
		...
		\return	void
	*/
	void init_Voxel_map(My_Type::Vector2i aligned_depth_size, int max_voxel_block_number);


	//! Allocate voxel block by current frame points.
	/*!
		\param	dev_current_points	CUDA device pointer. Current depth frame points cloud.

		\param	camera_pose			The camera pose matrix in map coordinate.

		\param	submap_pose			The submap pose matrix in world coordinate.

		\return	int					Number of voxel block allocated.
	*/
	int allocate_voxel_block(My_Type::Vector3f * dev_current_points, 
	...
	/*!
		...
		\param	dev_current_normal	CUDA device pointer. Current depth frame points cloud normal vectors.

		\param	camera_pose			The camera pose matrix in map coordinate.

		\return	void
	*/
	void fusion_SDF_to_voxel(My_Type::Vector3f * dev_current_points, 
	...
	/*!
		...
		\param	plane_img			CUDA device pointer. Current plane segmentation result.(A label mask image)

		\return	void
	*/
	void fusion_plane_label_to_voxel(My_Type::Vector3f * dev_current_points,
									 int * plane_img,
									 Eigen::Matrix4f camera_pose,
									 Eigen::Matrix4f sumap_pose = Eigen::Matrix4f::Identity());
	...
	/*!
		...
		\return	void
	*/
	void merge_with_voxel_map(const Voxel_map & fragment_map, int * dev_plane_global_index);
	...
	*/
	...
	/*!
		...
		\note	Raycast is the inverse operation of projection. See details on wiki 
	...
	*/
	...
	////! Raycast informations for (OpenGL) rendering Voxel map and caculate points color array. 
	///*!
	//	\param	pose				The camera pose matrix in map coordinate.
	...
	//void render_map_special(Eigen::Matrix4f trans_mat, int render_mode, int fragment_map_id);

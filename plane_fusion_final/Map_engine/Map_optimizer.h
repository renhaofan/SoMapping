...
struct Cost_function_of_feature
{
	...
	Cost_function_of_feature(Eigen::Vector3f p1, Eigen::Vector3f p2) : _p1(p1), _p2(p2){}

	// 
	template<typename T>
	bool operator()(const T * const extrinsic_1, const T * const extrinsic_2, T * residual) const
	{
		T p_buffer[3];
		T p_1[3];
		p_buffer[0] = T(_p1.x());
...
// TODO
//! Functor. Use adjacent fragment map's plane information to optimize fragment maps' pose.
struct Cost_function_of_adjacent_plane
{
	Eigen::Vector3f _n1_F, _n2_F;
	float _d1_F, _d2_F;
	// Assignment constructor
	Cost_function_of_adjacent_plane(Eigen::Vector3f n1_F, Eigen::Vector3f n2_F, float d1_F, float d2_F) : \
	...
	bool operator()(const T * const extrinsic_1, const T * const extrinsic_2, T * residual) const
	{
		...
		d1_W = ceres::abs(point_W[0] * n1_W[0] + point_W[1] * n1_W[1] + point_W[2] * n1_W[2]);
		...
		d2_W = ceres::abs(point_W[0] * n2_W[0] + point_W[1] * n2_W[1] + point_W[2] * n2_W[2]);
...
struct Cost_function_of_global_plane
{
	...
	float _d1_F, _d2_W;
	// Assignment constructor
	Cost_function_of_global_plane(Eigen::Vector3f n1_F, Eigen::Vector3f n2_W, float d1_F, float d2_W) : \
	...
	bool operator()(const T * const extrinsic_1, T * residual) const
	{
		...
		point_W[1] = -n1_W[1] * (T)_d1_F + extrinsic_1[4];
		point_W[2] = -n1_W[2] * (T)_d1_F + extrinsic_1[5];
		d1_W = -(point_W[0] * n1_W[0] + point_W[1] * n1_W[1] + point_W[2] * n1_W[2]);

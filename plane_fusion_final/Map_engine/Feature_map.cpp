...
//
void Feature_map::update_current_features(
    std::vector<My_Type::Vector3f> current_keypoints, cv::Mat current_features,
    std::vector<int> &current_match_to_model,
    std::vector<int> &previous_match_to_model, My_pose &camera_pose) {
  int number_of_model_keypoint = this->model_keypoints.size();

  // Update keypoint and feature
  for (int current_id = 0; current_id < current_keypoints.size();
       current_id++) {
    ...
    for (int scale_id = 0; scale_id < MAX_LAYER_NUMBER; scale_id++) {
      float scale_diff =
          fabsf(scale_layer_depth[scale_id] - current_keypoints[current_id].z);
      if (min_diff_value > scale_diff) {
        min_scale_id = scale_id;
        min_diff_value = scale_diff;
      }
    }
    //
    ...
    if (match_model_id >= number_of_model_keypoint) {
      ...
      new_keypoint.point =
          My_Type::Vector3f(current_vec_in_map.x(), current_vec_in_map.y(),
      ...
      new_multiscale_feature.create(
          cv::Size(new_feature.cols, MAX_LAYER_NUMBER), new_feature.type());
      ...
      // - Mapper
      My_Type::Vector3i keypoint_block_pos(
          new_keypoint.point.x / FEATRUE_BLOCK_WIDTH,
      ...
      My_Type::Vector3f model_keypoint_position(
          this->model_keypoints[match_model_id].point.data);
      // Check position
      if ((model_keypoint_position - My_Type::Vector3f(current_vec_in_map.x(),
                                                       current_vec_in_map.y(),
                                                       current_vec_in_map.z()))
              .norm() > 0.2) {
      ...
      this->model_keypoints[match_model_id].point =
          (model_keypoint_position * model_weight +
           My_Type::Vector3f(current_vec_in_map.x(), current_vec_in_map.y(),
                             current_vec_in_map.z())) /
          (model_weight + 1.0f);
      ...
      new_feature.copyTo(
          this->model_features[match_model_id].row(min_scale_id));
      if (min_scale_id > 0) {
        new_feature.copyTo(
            this->model_features[match_model_id].row(min_scale_id - 1));
        this->model_feature_scale_flag[match_model_id]
            .exist[min_scale_id - 1]++;
      }
      if (min_scale_id < MAX_LAYER_NUMBER - 1) {
        new_feature.copyTo(
            this->model_features[match_model_id].row(min_scale_id + 1));
  ...
  for (int model_id = 0; model_id < this->model_keypoints.size(); model_id++) {
    if (this->model_keypoints[model_id].is_valid &&
        this->model_keypoints[model_id].observe_counter <
            this->valid_observe_count) {
      this->model_keypoints[model_id].non_observe_counter++;
      if (this->model_keypoints[model_id].non_observe_counter >
          this->outlyer_non_observe_count)
...
//
void Feature_map::get_model_keypoints(
  ...
  for (int current_id = 0; current_id < current_keypoints.size();
    ...
    base_point =
        base_point -
        My_Type::Vector3f(search_radius_3D, search_radius_3D, search_radius_3D);
    ...
    for (int step_z = 0; step_z < number_of_step; step_z++)
      for (int step_y = 0; step_y < number_of_step; step_y++)
        for (int step_x = 0; step_x < number_of_step; step_x++) {
          My_Type::Vector3i check_vec(
              (int)(base_point.x / FEATRUE_BLOCK_WIDTH) + step_x,
              (int)(base_point.y / FEATRUE_BLOCK_WIDTH) + step_y,
              (int)(base_point.z / FEATRUE_BLOCK_WIDTH) + step_z);
          // Search point
          std::unordered_map<My_Type::Vector3i, int>::iterator
              point_index_iter = this->map_point_mapper.find(check_vec);
          if (point_index_iter != this->map_point_mapper.end()) {
            ...
            neighbor_feature.push_back(this->model_features[model_point_index]
                                           .row(min_scale_id)
                                           .clone());
          }
        }

...
void SLAM_system::init_parameters(Data_engine * data_engine_ptr)
{
	...
	if (data_engine_ptr != nullptr)
	{
		...
		SLAM_system_settings::instance()->set_color_image_size(raw_color_width, raw_color_height, true);
		this->data_engine->get_depth_image_size(raw_depth_width, raw_depth_height);
		SLAM_system_settings::instance()->set_depth_image_size(raw_depth_width, raw_depth_height, true);
	}
	else
	{
	...
	raw_color_size.width = raw_color_width;		raw_color_size.height = raw_color_height;
	raw_depth_size.width = raw_depth_width;		raw_depth_size.height = raw_depth_height;
	this->preprocess_engine->init(raw_color_size, raw_depth_size,
	...
	if (SLAM_system_settings::instance()->enable_plane_module)
	{
		...
		//Plane_super_pixel_detector * temp_ptr = dynamic_cast<Plane_super_pixel_detector *>(this->plane_detector);
		//temp_ptr->init();
...
{
	// Load one frame
	bool load_state = this->data_engine->load_next_frame(this->timestamp, this->color_mat, this->depth_mat, false);
	if (!load_state)
	{
		this->processing_state = ProcessingState::STOP_PROCESS;
		all_data_process_done = true;
	...
	this->map_engine->raycast_points_from_map(this->estimated_camera_pose.mat, RaycastMode::RAYCSAT_FOR_TRACKING);
...
{
	// Preprocess current image/points (generate hierarchical points/normals/weights etc.)
	...
	this->preprocess_engine->preprocess_model_points(this->map_engine->dev_model_points,
													 this->map_engine->dev_model_normals);
}
...
void Ground_truth_SLAM_system::track_camera_pose()
{
	...
	bool state = this->data_engine->get_next_ground_truth_camera_pose(ground_truth_trajectory_node);
...
{
	//
	this->map_engine->update_map_after_tracking(this->estimated_camera_pose,
...
void Ground_truth_SLAM_system::generate_render_info(Eigen::Matrix4f view_pose)
{
	...
	{
		this->map_engine->raycast_points_from_map(view_pose, RaycastMode::RAYCAST_FOR_VIEW);
	}
}
...
{
	Basic_Voxel_map * Voxel_map_ptr = dynamic_cast<Basic_Voxel_map *>(this->map_engine);
	// 
	this->mesh_of_total_map.generate_mesh_from_voxel(Voxel_map_ptr->voxel_map_ptr->dev_entrise,
...
{
	// Create modules
	this->preprocess_engine = dynamic_cast<Preprocess_RGBD *>(new Preprocess_RGBD());
...
void Basic_voxel_SLAM_system::process_one_frame()
{
	...
	if (SLAM_system_settings::instance()->enable_plane_module)
	{
		this->plane_detector->detect_plane(this->preprocess_engine->dev_hierarchy_points.data_ptrs[0],
										   this->preprocess_engine->dev_hierarchy_normals.data_ptrs[0],
		...
		this->plane_detector->match_planes(map_ptr->plane_map_ptr->plane_list.data(), map_ptr->plane_map_ptr->plane_counter,
	...
	if (this->frame_id % 20 == 19 && false)
	{
		...
		this->mesh_of_total_map.generate_mesh_from_plane_array(map_ptr->plane_map_ptr->plane_list,
...
void Basic_voxel_SLAM_system::track_camera_pose()
{
	...
	Basic_ICP_tracker * tracker_ptr = dynamic_cast<Basic_ICP_tracker *>(this->track_engine);
	tracker_ptr->track_camera_pose(this->preprocess_engine->dev_hierarchy_points,
	...
	tracker_ptr->generate_icp_correspondence_lines(this->preprocess_engine->dev_hierarchy_points,
												   this->preprocess_engine->dev_hierarchy_model_points,
												   this->preprocess_engine->dev_hierarchy_normals,
												   this->preprocess_engine->dev_hierarchy_model_normals);
#endif
...
void Basic_voxel_SLAM_system::generate_mesh()
{
	Basic_Voxel_map * map_ptr = dynamic_cast<Basic_Voxel_map *>(this->map_engine);
	// 
	this->mesh_of_total_map.generate_mesh_from_voxel(map_ptr->voxel_map_ptr->dev_entrise,
													 map_ptr->voxel_map_ptr->dev_voxel_block_array);
...
void Submap_SLAM_system::init_modules()
{
	...
	//
	//const string vocabulary_path = "C:/My_SLAM_project/DBoW_result/vocabulary_orb/voc_L3K5_T500N300.yml.gz";
...
void Submap_SLAM_system::process_one_frame()
{
	...
	{
	first_reach = false;
	load_state = this->data_engine->load_next_frame(this->timestamp, this->color_mat, this->depth_mat, false);
	}*/
	...
	// Raycast point from map
	this->submap_ptr_array.back()->raycast_points_from_map(this->estimated_camera_pose.mat, RaycastMode::RAYCSAT_FOR_TRACKING);
	...
	this->feature_detector->detect_orb_features(this->preprocess_engine->dev_hierarchy_intensity,
												this->preprocess_engine->hierarchy_points);
	this->feature_map_ptr_array.back()->get_model_keypoints(this->feature_detector->current_keypoint_position,
	...
	this->feature_detector->match_orb_features(this->feature_map_ptr_array.back()->model_keypoints.size());
	...
	if (SLAM_system_settings::instance()->enable_plane_module && true)
	{
		...
		Submap_Voxel_map * map_ptr = dynamic_cast<Submap_Voxel_map *>(this->submap_ptr_array.back());
		this->plane_detector->match_planes(map_ptr->plane_map_ptr->plane_list.data(), map_ptr->plane_map_ptr->plane_counter,
										   this->plane_detector->dev_current_plane_labels,
	...
	for (int submap_id = 0; submap_id < this->estimated_trajectory_array.size(); submap_id++)
	{
		Eigen::Matrix4f submap_pose = this->submap_pose_array[submap_id]->mat;
		for (int trajectory_node_id = 0; trajectory_node_id < this->estimated_trajectory_array[submap_id].size(); trajectory_node_id++)
		{
			Trajectory_node temp_node = this->estimated_trajectory_array[submap_id][trajectory_node_id];
			// TODO : rotation
			//Eigen::Quaternionf rot_q_src(temp_node.quaternions.qr, temp_node.quaternions.qx, temp_node.quaternions.qy, temp_node.quaternions.qz);
			//Eigen::Matrix3f rot_mat = rot_q_src.toRotationMatrix();
			
			Eigen::Vector3f trans_vec(temp_node.tx, temp_node.ty, temp_node.tz);
			trans_vec = submap_pose.block(0, 0, 3, 3) * trans_vec.eval() + submap_pose.block(0, 3, 3, 1);
			temp_node.tx = trans_vec.x();	temp_node.ty = trans_vec.y();	temp_node.tz = trans_vec.z();
...
void Submap_SLAM_system::preprocess()
{
	...
	this->preprocess_engine->preprocess_image(this->color_mat, this->depth_mat);
	// Preprocess model points/normal (generate hierarchical points/normals)
	this->preprocess_engine->preprocess_model_points(this->submap_ptr_array.back()->dev_model_points,
...
void Submap_SLAM_system::track_camera_pose()
{
	...
	current_keypoints.resize(this->feature_detector->current_keypoint_position.size());
	model_keypoints.resize(this->feature_detector->current_keypoint_position.size());
	//
	int max_model_number = this->feature_map_ptr_array.back()->model_keypoints.size();
	Eigen::Matrix4f pose_inv = this->estimated_camera_pose.mat.inverse();
	for (int current_point_id = 0; current_point_id < this->feature_detector->current_keypoint_position.size(); current_point_id++)
	{
		int model_point_id = this->feature_detector->current_match_to_model_id[current_point_id];
		if (model_point_id >= 0 && model_point_id < max_model_number)
		{
			current_keypoints[current_point_id] = Eigen::Vector3f(this->feature_detector->current_keypoint_position[current_point_id].x,
																  this->feature_detector->current_keypoint_position[current_point_id].y,
																  this->feature_detector->current_keypoint_position[current_point_id].z);
			Eigen::Vector3f model_point(this->feature_map_ptr_array.back()->model_keypoints[model_point_id].point.x,
										this->feature_map_ptr_array.back()->model_keypoints[model_point_id].point.y,
										this->feature_map_ptr_array.back()->model_keypoints[model_point_id].point.z);
			model_point = pose_inv.block(0, 0, 3, 3) * model_point.eval() + pose_inv.block(0, 3, 3, 1);
	...
	Keypoint_ICP_tracker * tracker_ptr = dynamic_cast<Keypoint_ICP_tracker *>(this->track_engine);
	tracker_ptr->track_camera_pose(this->preprocess_engine->dev_hierarchy_points,
								   this->preprocess_engine->dev_hierarchy_model_points,
								   this->preprocess_engine->dev_hierarchy_normals,
								   this->preprocess_engine->dev_hierarchy_model_normals,
...
void Submap_SLAM_system::update_to_map()
{
	...
	// Voxel
	this->submap_ptr_array.back()->update_map_after_tracking(this->estimated_camera_pose,
															 this->preprocess_engine->dev_hierarchy_points.data_ptrs[0],
															 this->preprocess_engine->dev_hierarchy_normals.data_ptrs[0],
															 this->plane_detector->dev_current_plane_labels);
	// Plane
	this->submap_ptr_array.back()->update_plane_map(this->plane_detector->current_planes,
													this->plane_detector->matches);
	// Keypoint
	this->feature_map_ptr_array.back()->update_current_features(this->feature_detector->current_keypoint_position, 
	...
	if (this->submap_ptr_array.back()->consider_to_create_new_submap() && this->enable_submap_creation)
	{
		//this->processing_state = ProcessingState::STOP_PROCESS;
		...
		this->mesh_ptr_array.back()->generate_mesh_from_voxel(this->submap_ptr_array.back()->voxel_map_ptr->dev_entrise,
		...
		if (this->enable_plane_map)
		{
			...
			this->mesh_ptr_array.back()->generate_nonplanar_mesh_from_voxel(last_map_ptr->voxel_map_ptr->dev_entrise,
																			last_map_ptr->voxel_map_ptr->dev_voxel_block_array);

			this->mesh_of_total_map.generate_mesh_from_plane_array(last_map_ptr->plane_map_ptr->plane_list,
		...
		// Relabel plane id
		this->plane_detector->match_planes_to_new_map(last_map_ptr->plane_map_ptr->plane_list.data(),
													  last_map_ptr->plane_map_ptr->plane_counter,
													  this->plane_detector->dev_current_plane_labels,
													  last_map_ptr->dev_model_plane_labels,
													  plane_matches);
		...
		this->plane_associator->update_matches(plane_matches, std::pair<int, int>(last_submap_id, current_submap_id), Matches_type::Tracked_Matches);
		...
		std::vector<int> init_map_keypoint_id(this->feature_detector->current_match_to_model_id.size());
		for (int i = 0, valid_counter = 0; i < init_map_keypoint_id.size(); i++)
		{
			...
			{	
				...
				valid_counter++; 
				feature_intermap_matches.push_back(std::pair<int, int>(this->feature_detector->current_match_to_model_id[i],
		...
		this->keypoint_associator->update_matches(feature_intermap_matches, std::pair<int, int>(last_submap_id, current_submap_id), Matches_type::Tracked_Matches);
		...
		for (int submap_id = 0; submap_id < this->submap_pose_array.size(); submap_id++)
		{
			std::vector<My_Type::Vector2i> relabel_list;
			this->generate_submap_to_global_plane_relabel_list(submap_map_to_global_plane, submap_id,
															   relabel_list);
			// generate mesh
			this->mesh_of_total_map.copy_triangle_mesh_from(this->mesh_ptr_array[submap_id], static_triangle_offset, 
															...
															relabel_list);
			static_triangle_offset += this->mesh_ptr_array[submap_id]->number_of_triangles;
		}
		//
		Mesh_generator * mesh_ptr = new Mesh_generator();
		this->mesh_ptr_array.push_back(mesh_ptr);
	...
	if (this->need_generate_mesh)
	{
		...
		this->generate_submap_to_global_plane_mapper(submap_map_to_global_plane);
		std::vector<My_Type::Vector2i> relabel_list;
		this->generate_submap_to_global_plane_relabel_list(submap_map_to_global_plane, this->submap_ptr_array.size() - 1,
														   relabel_list);
		...
		this->mesh_ptr_array.back()->generate_mesh_from_voxel(this->submap_ptr_array.back()->voxel_map_ptr->dev_entrise,
															  this->submap_ptr_array.back()->voxel_map_ptr->dev_voxel_block_array,
															  relabel_list);
		this->mesh_of_total_map.copy_triangle_mesh_from(this->mesh_ptr_array.back(), static_triangle_offset);
	}
...
void Submap_SLAM_system::optimize_map()
{
	...
	for (int match_list_index = 0; match_list_index < this->keypoint_associator->all_submap_id_pair.size(); match_list_index++)
	{
		// index of matched fragment map
		int map_1_index = this->keypoint_associator->all_submap_id_pair[match_list_index].first;
		int map_2_index = this->keypoint_associator->all_submap_id_pair[match_list_index].second;
		// 
		double * pose_1_ptr = (double*)(pose_array.data() + map_1_index * 6);
		double * pose_2_ptr = (double*)(pose_array.data() + map_2_index * 6);

		// for each point pair
		for (int point_pair_id = 0; point_pair_id < this->keypoint_associator->all_matches[match_list_index].size(); point_pair_id++)
		{
			int map_1_point_id = this->keypoint_associator->all_matches[match_list_index][point_pair_id].first;
			int map_2_point_id = this->keypoint_associator->all_matches[match_list_index][point_pair_id].second;
			//
			My_Type::Vector3f point_1_t, point_2_t;
			point_1_t = this->feature_map_ptr_array[map_1_index]->model_keypoints[map_1_point_id].point;
			point_2_t = this->feature_map_ptr_array[map_2_index]->model_keypoints[map_2_point_id].point;
			Eigen::Vector3f point_1(point_1_t.x, point_1_t.y, point_1_t.z);
			Eigen::Vector3f point_2(point_2_t.x, point_2_t.y, point_2_t.z);

			// Create cost function
			ceres::CostFunction * cost_function_ptr = new ceres::AutoDiffCostFunction<Cost_function_of_feature, 3, 6, 6>\
				(new Cost_function_of_feature(point_1, point_2));
			// Add residual block
			problem.AddResidualBlock(cost_function_ptr, new ceres::CauchyLoss(0.05), pose_1_ptr, pose_2_ptr);
		}
	}
	// for each submap plane matches list
	for (int match_list_index = 0; match_list_index < this->plane_associator->all_submap_id_pair.size(); match_list_index++)
	{
		// index of matched fragment map
		int map_1_index = this->plane_associator->all_submap_id_pair[match_list_index].first;
		int map_2_index = this->plane_associator->all_submap_id_pair[match_list_index].second;
		// 
		double * pose_1_ptr = (double *)(pose_array.data() + map_1_index * 6);
		double * pose_2_ptr = (double *)(pose_array.data() + map_2_index * 6);

		// for each plane pair
		for (int plane_paire_id = 0; plane_paire_id < this->plane_associator->all_matches[match_list_index].size(); plane_paire_id++)
		{
			// Read the matched planes' ID in their fragment map 
			int map_1_plane_id = this->plane_associator->all_matches[match_list_index][plane_paire_id].first;
			int map_2_plane_id = this->plane_associator->all_matches[match_list_index][plane_paire_id].second;

			//printf("(%d, %d) -> (%d, %d)\n", map_1_index, map_1_plane_id, map_2_index, map_2_plane_id);

			// Read planes' parameters (normal vector and distance)
			Eigen::Vector3f normal_1, normal_2;
			normal_1.x() = this->submap_ptr_array[map_1_index]->plane_map_ptr->plane_list[map_1_plane_id].nx;
			normal_1.y() = this->submap_ptr_array[map_1_index]->plane_map_ptr->plane_list[map_1_plane_id].ny;
			normal_1.z() = this->submap_ptr_array[map_1_index]->plane_map_ptr->plane_list[map_1_plane_id].nz;
			float distance_1 = this->submap_ptr_array[map_1_index]->plane_map_ptr->plane_list[map_1_plane_id].d;
			normal_2.x() = this->submap_ptr_array[map_2_index]->plane_map_ptr->plane_list[map_2_plane_id].nx;
			normal_2.y() = this->submap_ptr_array[map_2_index]->plane_map_ptr->plane_list[map_2_plane_id].ny;
			normal_2.z() = this->submap_ptr_array[map_2_index]->plane_map_ptr->plane_list[map_2_plane_id].nz;
			float distance_2 = this->submap_ptr_array[map_2_index]->plane_map_ptr->plane_list[map_2_plane_id].d;
			...
			// Create cost function
			ceres::CostFunction * cost_function_ptr = new ceres::AutoDiffCostFunction<Cost_function_of_adjacent_plane, 4, 6, 6>\
				(new Cost_function_of_adjacent_plane(normal_1, normal_2, distance_1, distance_2));
			// Add residual block
	...
	for (int submap_id = 0; submap_id < this->submap_pose_array.size(); submap_id++)
	{
		...
		ceres::CostFunction * cost_function_ptr = new ceres::AutoDiffCostFunction<Cost_function_of_regular_constrain, 6, 6>\
	...
	this->estimated_camera_pose.mat = this->submap_pose_array.back()->mat * this->estimated_camera_pose.mat.eval();
	this->estimated_camera_pose.synchronize_to_GPU();
...
void Submap_SLAM_system::end_of_process_data()
{
	...
	for (int submap_id = 0; submap_id < this->submap_pose_array.size(); submap_id++)
	{
		...
		if (true)
		{
			...
			this->mesh_ptr_array.back()->generate_nonplanar_mesh_from_voxel(this->submap_ptr_array[submap_id]->voxel_map_ptr->dev_entrise,
																			this->submap_ptr_array[submap_id]->voxel_map_ptr->dev_voxel_block_array);

			this->mesh_of_total_map.generate_mesh_from_plane_array(this->submap_ptr_array[submap_id]->plane_map_ptr->plane_list,
																   this->submap_ptr_array[submap_id]->plane_map_ptr->plane_coordinate_list,
																   this->submap_ptr_array[submap_id]->plane_map_ptr->dev_plane_entry_list,
																   this->submap_ptr_array[submap_id]->plane_map_ptr->dev_plane_pixel_array_list,
																   this->submap_ptr_array[submap_id]->voxel_map_ptr->dev_entrise,
																   this->submap_ptr_array[submap_id]->voxel_map_ptr->dev_voxel_block_array);
			//
			this->submap_ptr_array[submap_id]->plane_map_ptr->generate_planar_block_render_information();
		}
	}
...
{
	// 
	this->submap_ptr_array.back()->raycast_points_from_map(view_pose, RaycastMode::RAYCAST_FOR_VIEW);
	
	//
	My_Type::Vector2i main_viewport_size = UI_parameters::instance()->main_viewport_size;
	//
	memcpy(this->map_engine->scene_points, this->submap_ptr_array.back()->scene_points,
		   main_viewport_size.width * main_viewport_size.height * sizeof(My_Type::Vector3f));
	memcpy(this->map_engine->scene_normals, this->submap_ptr_array.back()->scene_normals,
		   main_viewport_size.width * main_viewport_size.height * sizeof(My_Type::Vector3f));
}
...
void Submap_SLAM_system::generate_mesh()
{
	...
	this->mesh_ptr_array.back()->generate_mesh_from_voxel(map_ptr->voxel_map_ptr->dev_entrise,
														  map_ptr->voxel_map_ptr->dev_voxel_block_array);
}
...
{
	// Save keyframe
	this->feature_map_ptr_array.back()->save_keyframe(this->feature_detector->current_features,
													  this->feature_detector->current_match_to_model_id, 
													  this->ORB_vocabulary);

	//this->keypoint_buffer_1.clear();
	//std::vector<int> & test_mapper = this->feature_map_ptr_array.back()->keyframe_feature_mapper_list.back();
	...
	//		this->feature_map_ptr_array.back()->model_keypoints[kp_id].point.y, this->feature_map_ptr_array.back()->model_keypoints[kp_id].point.z));
	//}
	...
	int this_keyframe_id = this->feature_map_ptr_array[this_submap_id]->keyframe_feature_mapper_list.size() - 1;
	My_Type::Vector3f this_weight_center = this->feature_map_ptr_array[this_submap_id]->keyframe_weigth_centers[this_keyframe_id];
	DBoW3::BowVector current_bowvec = this->feature_map_ptr_array[this_submap_id]->dbow_vec_list[this_keyframe_id];
	...
	for (int pair_id = 0; pair_id < this->keypoint_associator->all_submap_id_pair.size(); pair_id++)
	{
		if (this->keypoint_associator->all_submap_id_pair[pair_id].first == this_submap_id)
		{	associated_flag[this->keypoint_associator->all_submap_id_pair[pair_id].second] = true;	}
		if (this->keypoint_associator->all_submap_id_pair[pair_id].second == this_submap_id)
		{	associated_flag[this->keypoint_associator->all_submap_id_pair[pair_id].first] = true;	}
	}
	// 
	int matched_keyframe_id = -1, matched_submap_id = -1, similarity_counter = 0;
	float max_similarity = 0;
	for (int submap_id = 0; submap_id < this->feature_map_ptr_array.size(); submap_id++)
	{
		if (associated_flag[submap_id])		continue;

		for (int keyframe_id = 0; keyframe_id < this->feature_map_ptr_array[submap_id]->keyframe_feature_mapper_list.size(); keyframe_id++)
		{
			My_Type::Vector3f check_weight_center = this->feature_map_ptr_array[submap_id]->keyframe_weigth_centers[keyframe_id];
			//if ((check_weight_center - this_weight_center).norm() > 1.0f)	continue;
			if ((check_weight_center - this_weight_center).norm() > 2.0f)	continue;
			if (this->feature_map_ptr_array[submap_id]->keyframe_feature_mapper_list[keyframe_id].size() < 10)	continue;

			// Check for DBoW3 ORB feature similarity
			float similar_ratio = 0.0f;
			DBoW3::BowVector keyframe_bowvec = this->feature_map_ptr_array[submap_id]->dbow_vec_list[keyframe_id];
			
			similar_ratio = (float)this->ORB_vocabulary.score(current_bowvec, keyframe_bowvec);
			...
			if (similar_ratio > 0.40f)
			{
				...
				//printf("%d, %d -> %d, %d\n", this_submap_id, this_keyframe_id, matched_submap_id, matched_keyframe_id);
				//std::cout << "similar_ratio = " << similar_ratio << std::endl;
			}
		}
	}
	...
	if (similarity_counter > 5)
	{
		...
		// Current info
		for (int current_id = 0; current_id < this->feature_detector->current_match_to_model_id.size(); current_id++)
		{
			current_keypoint_mapper.push_back(std::pair<int, int>(this->submap_ptr_array.size() - 1,
				this->feature_detector->current_match_to_model_id[current_id]));
		}
		// Looped info 
		std::vector<int> & matched_feature_mapper = this->feature_map_ptr_array[matched_submap_id]->keyframe_feature_mapper_list[matched_keyframe_id];
		for (int mapper_id = 0; mapper_id < matched_feature_mapper.size(); mapper_id++)
		{
			...
			{
				if (this->feature_map_ptr_array[matched_submap_id]->model_feature_scale_flag[model_keypoint_id].exist[scale_id] != 0)
				{
			...
			{
				loop_features.push_back(this->feature_map_ptr_array[matched_submap_id]->model_features[model_keypoint_id].row(valid_scale_id));
				loop_keypoint_position.push_back(this->feature_map_ptr_array[matched_submap_id]->model_keypoints[model_keypoint_id].point);
				loop_keypoint_mapper.push_back(std::pair<int, int>(matched_submap_id, model_keypoint_id));
			}
		}
		...
		std::vector<std::pair<int, int>> keypoint_matches; /* (current keypoint -> looped keypoint) */
		std::vector<bool> is_valid_keypoint_match;
		for (int current_id = 0; current_id < this->feature_detector->current_match_to_model_id.size(); current_id++)
		{
			int current_keypoint_index = this->feature_detector->current_match_to_model_id[current_id];
			if (current_keypoint_index < 0)	continue;

			My_Type::Vector3f current_position = this->feature_map_ptr_array[this_submap_id]->model_keypoints[current_keypoint_index].point;
			cv::Mat current_feature_row = this->feature_detector->current_features.row(current_id);
			//cv::Mat current_feature_row = this->feature_map_ptr_array[this_submap_id]->model_features[current_keypoint_index].row;

			for (int loop_id = 0; loop_id < loop_keypoint_position.size(); loop_id++)
			{
				...
				if (DescriptorDistance(current_feature_row, loop_feature_row) > 64)	continue;

				//
				current_position_buffer.push_back(Eigen::Vector3f(current_position.x, current_position.y, current_position.z));
				loop_position_buffer.push_back(Eigen::Vector3f(loop_position.x, loop_position.y, loop_position.z));
				keypoint_matches.push_back(std::pair<int, int>(current_keypoint_index, loop_id));
				//printf("%f, %d -> %d\n", (current_position - loop_position).norm(), current_keypoint_index, loop_id);
				is_valid_keypoint_match.push_back(true);
			}
		}
		// -plane
		std::vector<Plane_info> current_plane_buffer, loop_plane_buffer;
		std::vector<std::pair<int, int>> plane_matches; /* (current plane -> looped plane) */
		std::vector<bool> is_valid_plane_match;
		for (int current_id = 0; current_id < this->submap_ptr_array.back()->plane_map_ptr->plane_list.size(); current_id++)
		{
			if (!this->submap_ptr_array.back()->plane_map_ptr->plane_list[current_id].is_valid)	continue;
			My_Type::Vector3f current_normal;
			current_normal.nx = this->submap_ptr_array.back()->plane_map_ptr->plane_list[current_id].nx;
			current_normal.ny = this->submap_ptr_array.back()->plane_map_ptr->plane_list[current_id].ny;
			current_normal.nz = this->submap_ptr_array.back()->plane_map_ptr->plane_list[current_id].nz;
			float current_distance = this->submap_ptr_array.back()->plane_map_ptr->plane_list[current_id].d;

			for (int loop_id = 0; loop_id < this->submap_ptr_array[matched_submap_id]->plane_map_ptr->plane_list.size(); loop_id++)
			{
				if (!this->submap_ptr_array[matched_submap_id]->plane_map_ptr->plane_list[loop_id].is_valid)	continue;
				My_Type::Vector3f loop_normal;
				loop_normal.nx = this->submap_ptr_array[matched_submap_id]->plane_map_ptr->plane_list[loop_id].nx;
				loop_normal.ny = this->submap_ptr_array[matched_submap_id]->plane_map_ptr->plane_list[loop_id].ny;
				loop_normal.nz = this->submap_ptr_array[matched_submap_id]->plane_map_ptr->plane_list[loop_id].nz;
				float loop_distance = this->submap_ptr_array[matched_submap_id]->plane_map_ptr->plane_list[loop_id].d;


				if (current_normal.dot(loop_normal) < 0.90f)			continue;
				if (fabsf(current_distance - loop_distance) > 0.15f)	continue;

				current_plane_buffer.push_back(this->submap_ptr_array.back()->plane_map_ptr->plane_list[current_id]);
				loop_plane_buffer.push_back(this->submap_ptr_array[matched_submap_id]->plane_map_ptr->plane_list[loop_id]);
		...
		//						  current_plane_buffer, loop_plane_buffer,
		//						  is_valid_keypoint_match, is_valid_plane_match);

		keypoint_buffer_1.clear();
		for (int i = 0; i < this->feature_detector->current_match_to_model_id.size(); i++)
		{
			int point_id = this->feature_detector->current_match_to_model_id[i];
			if (point_id < 0)	continue;
			
			keypoint_buffer_1.push_back(Eigen::Vector3f(this->feature_map_ptr_array[matched_submap_id]->model_keypoints[point_id].point.x,
				this->feature_map_ptr_array[matched_submap_id]->model_keypoints[point_id].point.y,
				this->feature_map_ptr_array[matched_submap_id]->model_keypoints[point_id].point.z));
				
		}

		keypoint_buffer_2.clear();
		for (int i = 0; i < this->feature_map_ptr_array[matched_submap_id]->keyframe_feature_mapper_list[matched_keyframe_id].size(); i++)
		{
			std::vector<int> & temp_mapper = this->feature_map_ptr_array[matched_submap_id]->keyframe_feature_mapper_list[matched_keyframe_id];
			if (temp_mapper[i] < 0)	continue;

			keypoint_buffer_2.push_back(Eigen::Vector3f(this->feature_map_ptr_array[matched_submap_id]->model_keypoints[temp_mapper[i]].point.x,
				this->feature_map_ptr_array[matched_submap_id]->model_keypoints[temp_mapper[i]].point.y,
				this->feature_map_ptr_array[matched_submap_id]->model_keypoints[temp_mapper[i]].point.z));
		...
		// Associate planes
		for (int plane_pair_id = 0; plane_pair_id < plane_matches.size(); plane_pair_id++)
		{
			Plane_info current_plane = this->submap_ptr_array[this_submap_id]->plane_map_ptr->plane_list[plane_matches[plane_pair_id].first];
			Plane_info loop_plane = this->submap_ptr_array[matched_submap_id]->plane_map_ptr->plane_list[plane_matches[plane_pair_id].second];

			My_Type::Vector3f current_normal(current_plane.nx, current_plane.ny, current_plane.nz);
			float current_distance = current_plane.d;
			My_Type::Vector3f loop_normal(loop_plane.nx, loop_plane.ny, loop_plane.nz);
			float loop_distance = loop_plane.d;

			//printf("%f, %f\n", current_normal.dot(loop_normal), fabsf(current_distance - loop_distance));
		}


		//
		this->keypoint_associator->update_matches(std::vector<std::pair<int, int>>(),
												  std::pair<int, int>(this_submap_id, matched_submap_id), Matches_type::Relocalization_Matches);
		this->plane_associator->update_matches(plane_matches,
											   std::pair<int, int>(this_submap_id, matched_submap_id), Matches_type::Relocalization_Matches);
	}
...
//
int find_submap_plane_in_list(std::vector<std::vector<My_Type::Vector2i>> & global_plane_container,
							  const My_Type::Vector2i & submap_plane)
{
	for (int plane_id = 0; plane_id < global_plane_container.size(); plane_id++)
	{
		for (int list_id = 0; list_id < global_plane_container[plane_id].size(); list_id++)
		{
...
//
void Submap_SLAM_system::generate_submap_to_global_plane_mapper(std::vector<std::vector<My_Type::Vector2i>> & global_plane_container)
{
	...
	// Associated planes
	for (int map_match_id = 0; map_match_id < this->plane_associator->all_submap_id_pair.size(); map_match_id++)
	{
		for (int pair_id = 0; pair_id < this->plane_associator->all_matches[map_match_id].size(); pair_id++)
		{
			My_Type::Vector2i first_plane(this->plane_associator->all_submap_id_pair[map_match_id].first,
										  this->plane_associator->all_matches[map_match_id][pair_id].first);
			My_Type::Vector2i second_plane(this->plane_associator->all_submap_id_pair[map_match_id].second,
										   this->plane_associator->all_matches[map_match_id][pair_id].second);

			int first_global_id = find_submap_plane_in_list(global_plane_container, first_plane);
			int second_global_id = find_submap_plane_in_list(global_plane_container, second_plane);
					 ...
					 first_global_id != second_global_id)
			{
				...
				for (int global_plane_id = 0; global_plane_id < global_plane_container.size(); global_plane_id++)
				{
					if (first_global_id != global_plane_id && second_global_id != global_plane_id)
				...
				buffer_container.back().insert(buffer_container.back().begin(), 
			...
			//	int global_plane_id = std::max(first_global_id, second_global_id);
			//	if (first_global_id != -1)	global_plane_container[global_plane_id].push_back(second_plane);
			//	if (second_global_id != -1)	global_plane_container[global_plane_id].push_back(first_plane);
			//}
		}
	}

	// Independent planes
	for (int submap_id = 0; submap_id < this->submap_ptr_array.size(); submap_id++)
	{
		for (int plane_id = 1; plane_id < this->submap_ptr_array[submap_id]->plane_map_ptr->plane_list.size(); plane_id++)
		{
			if (!this->submap_ptr_array[submap_id]->plane_map_ptr->plane_list[plane_id].is_valid)	continue;
			if (find_submap_plane_in_list(global_plane_container, My_Type::
			Vector2i(submap_id, plane_id)) == -1)
			{
	...
	if (false)
	{
		for (int global_id = 0; global_id < global_plane_container.size(); global_id++)
		{
			printf("%d: ", global_id);
			for (int pair_id = 0; pair_id < global_plane_container[global_id].size(); pair_id++)
			{
				printf("(%d, %d)", global_plane_container[global_id][pair_id].x, global_plane_container[global_id][pair_id].y);
			}
			printf("\n");
		}
		printf("\n");
...
//
void Submap_SLAM_system::generate_submap_to_global_plane_relabel_list(std::vector<std::vector<My_Type::Vector2i>> & global_plane_container,
																	  int submap_id,
																	  std::vector<My_Type::Vector2i> & relabel_list)
{
	// 
	...
	for (int global_id = 0; global_id < global_plane_container.size(); global_id++)
		for (int pair_id = 0; pair_id < global_plane_container[global_id].size(); pair_id++)
			if (global_plane_container[global_id][pair_id].x == submap_id)
				temp_list.push_back(My_Type::Vector2i(global_id, global_plane_container[global_id][pair_id].y));
	...
	memset(relabel_list.data(), 0x00, relabel_list.size() * sizeof(My_Type::Vector2i));
...
void Submap_SLAM_system::filter_loop_matches(std::vector<Eigen::Vector3f> & current_points, std::vector<Eigen::Vector3f> & loop_points,
											 std::vector<Plane_info> & current_planes, std::vector<Plane_info> & loop_planes,
											 std::vector<bool> & is_valid_keypoint_match, std::vector<bool> & is_valid_plane_match)
{
	//!
	...
	for (int iterate_i = 0; iterate_i < 20; iterate_i++)
	{
		...
		{
			Eigen::Vector3f current_point_W = pose.block(0, 0, 3, 3) * current_points[point_id] + pose.block(0, 3, 3, 1);
			...
			jacobian_mat.data()[1] = -current_point_W.z();	jacobian_mat.data()[2] = +current_point_W.y();	jacobian_mat.data()[6] = -current_point_W.y();
			jacobian_mat.data()[3] = +current_point_W.z();	jacobian_mat.data()[5] = -current_point_W.x();	jacobian_mat.data()[7] = +current_point_W.x();
			...
			else if (point_diff < valid_radius)
			{
				...
				clamp_b_mat = b_mat;
				for (int i = 0; i < 3; i++)
					clamp_b_mat.data()[i] = fmaxf(fminf(clamp_b_mat.data()[i], huber_radius), -huber_radius);
		...
		my_solver(hessian_total, nabla_total, 
				  loss_total, valid_number, pose);
...
// 
void Submap_SLAM_system::match_plane_by_parameter(std::vector<Plane_info> & previous_map_planes,
												  std::vector<Plane_info> & current_map_planes,
												  std::vector<std::pair<int, int>> & plane_matches)
{
	//
	for (int pre_plane_id = 0; pre_plane_id < previous_map_planes.size(); pre_plane_id++)
	{
		...
		for (int matched_id = 0; matched_id < plane_matches.size(); matched_id++)
		{
			...
			{	already_matched = true;	break;	}
		}
		if (already_matched)	continue;


		for (int current_plane_id = 0; current_plane_id < current_map_planes.size(); current_plane_id++)
		{
			if (!current_map_planes[current_plane_id].is_valid)	continue;

			bool is_similar_plane = compute_plane_similarity(previous_map_planes[pre_plane_id], current_map_planes[current_plane_id]);

			if (is_similar_plane)
			{
				plane_matches.push_back(std::pair<int, int>(pre_plane_id, current_plane_id));

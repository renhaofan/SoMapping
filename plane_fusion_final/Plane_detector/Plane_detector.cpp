...
{
	this->aligned_depth_size = SLAM_system_settings::instance()->aligned_depth_size;
	this->cell_mat_size = this->aligned_depth_size / SLAM_system_settings::instance()->presegment_cell_width;
	
	//
	this->cell_info_mat = (Cell_info *)malloc(this->cell_mat_size.x * this->cell_mat_size.y * sizeof(Cell_info));
	this->current_planes = (Plane_info *)malloc(MAX_CURRENT_PLANES * sizeof(Plane_info));
	this->model_planes = (Plane_info *)malloc(MAX_MODEL_PLANES * sizeof(Plane_info));
	this->relative_matrix = (int *)malloc(MAX_CURRENT_PLANES * MAX_MODEL_PLANES * sizeof(int));
	...
	checkCudaErrors(cudaMalloc((void **)&(this->dev_current_plane_counter), sizeof(int)));
	checkCudaErrors(cudaMalloc((void **)&(this->dev_relative_matrix),
		MAX_CURRENT_PLANES * MAX_MODEL_PLANES * sizeof(int)));
	checkCudaErrors(cudaMalloc((void **)&(this->dev_matches), MAX_CURRENT_PLANES * sizeof(My_Type::Vector2i)));

	// Buffer coordinate
	checkCudaErrors(cudaMalloc((void **)&(this->dev_buffer_coordinate), MAX_CURRENT_PLANES * sizeof(Plane_coordinate)));
								  ...
								  bool with_continuous_frame_tracking)
{
	...
	this->fit_plane_for_each_cell(dev_current_points, dev_current_normals, this->dev_cell_info_mat);

	// 3. Cluster cells to detect plane
	this->cluster_cells(this->dev_cell_info_mat, dev_model_planes,
						this->dev_current_planes,
						with_continuous_frame_tracking);
...
//
void Plane_detector::transfer_plane_coordinate(const Eigen::Matrix4f & camera_pose)
{
	for (int plane_index = 1; plane_index < this->current_plane_counter; plane_index++)
	{
		...
		current_plane_W = -current_normal_W * current_d_C + camera_pose.block(0, 3, 3, 1);
	...
	if (1)
	{
		...
		for (int plane_id = 1; plane_id < this->current_plane_counter; plane_id++)
		{
			printf("%d : (%f, %f, %f), %f , %f , %d\t",
				   ...
				   this->current_planes[plane_id].nz,
				   this->current_planes[plane_id].d,
								  ...
								  const int * dev_model_plane_labels)
{
	...
	if (this->current_plane_counter == 0 || this->current_plane_counter >= MAX_CURRENT_PLANES)	return;
	...
	// 0. init data
	{
		...
		checkCudaErrors(cudaMemcpy(this->dev_current_planes, this->current_planes,
			MAX_CURRENT_PLANES * sizeof(Plane_info), cudaMemcpyHostToDevice));
		checkCudaErrors(cudaMemcpy(this->dev_model_planes, this->model_planes,
			MAX_MODEL_PLANES * sizeof(Plane_info), cudaMemcpyHostToDevice));


		checkCudaErrors(cudaMemset(this->dev_relative_matrix, 0x00, MAX_CURRENT_PLANES * MAX_MODEL_PLANES * sizeof(int)));
	}
	...
	// 1. Count pixel number of each plane
	{
		...
		count_planar_pixel_number_CUDA(block_rect, thread_rect,
									   dev_current_plane_labels,
									   this->dev_current_planes,
									   this->current_plane_counter);
		//CUDA_CKECK_KERNEL;

		// Lunch kernel function
		count_planar_pixel_number_CUDA(block_rect, thread_rect,
									   dev_model_plane_labels,
									   this->dev_model_planes,
									   model_plane_number);
		//CUDA_CKECK_KERNEL;


		checkCudaErrors(cudaMemcpy(this->current_planes, this->dev_current_planes,
			MAX_CURRENT_PLANES * sizeof(Plane_info), cudaMemcpyDeviceToHost));
		checkCudaErrors(cudaMemcpy(this->model_planes, this->dev_model_planes,
			MAX_MODEL_PLANES * sizeof(Plane_info), cudaMemcpyDeviceToHost));
	...
	// 2. Count overlap pixel number
	{
		...
		// Lunch kernel function
		count_overlap_pixel_number_CUDA(block_rect, thread_rect,
										dev_current_plane_labels,
										dev_model_plane_labels,
										this->current_plane_counter,
		...
		checkCudaErrors(cudaMemcpy(this->relative_matrix, this->dev_relative_matrix,
			MAX_CURRENT_PLANES * MAX_MODEL_PLANES * sizeof(int), cudaMemcpyDeviceToHost));
	}
	...
	// 3. Match current planes to model planes
	{
		...
		//for (int current_plane_id = 0; current_plane_id < this->current_plane_counter; current_plane_id++)
		//{
		//	//Plane_info current_plane_info = this->current_planes[current_plane_id];
		//	for (int model_plane_id = 0; model_plane_id < std::min(model_plane_number, MAX_MODEL_PLANES); model_plane_id++)
		//	{
		//		//Plane_info model_plane_info = this->model_planes[model_plane_id];
		//		printf("%d | ", this->relative_matrix[current_plane_id * MAX_MODEL_PLANES + model_plane_id]);
		//	}
		//	printf("\n");
		//}
		...
		for (int current_plane_id = 1; current_plane_id < this->current_plane_counter; current_plane_id++)
		{
			//
			...
			else
			{
				...
				std::vector<std::pair<int, int>> overlap_list = find_most_overlap_model_plane(current_plane_id, model_plane_number);
				int model_plane_id;
				...
				else if (overlap_list.size() > 1)
				{
					if (overlap_list.front().first == 0 &&
						overlap_list[1].second > MIN_CELLS_OF_PLANE * PLANE_PIXEL_BLOCK_SIZE * TRACK_PLANE_THRESHOLD)
					{
						// Tracked plane
						...
						overlap_pixel_number = overlap_list[1].second;
							 ...
							 overlap_list.front().second > MIN_CELLS_OF_PLANE * PLANE_PIXEL_BLOCK_SIZE * TRACK_PLANE_THRESHOLD)
					{
						// Tracked plane
						model_plane_id = overlap_list.front().first;
						overlap_pixel_number = overlap_list.front().second;
							 ...
							 overlap_list.front().second > MIN_CELLS_OF_PLANE * PLANE_PIXEL_BLOCK_SIZE)
					{
						// New plane
						model_plane_id = overlap_list.front().first;
						overlap_pixel_number = overlap_list.front().second;
				...
				else if (is_valid_match)
				{
					...
					My_Type::Vector3f current_normal(this->current_planes[current_plane_id].nx,
					...
					if (fabsf(model_planes[model_plane_id].d - this->current_planes[current_plane_id].d) > 0.1)	is_valid_match = false;
			...
			//printf("%d : %d, %d\n", current_plane_id, this->matches[current_plane_id].x, this->matches[current_plane_id].y);
		}
	}
	...
	// 4. Re-label current plane labels to model plane order
	{
		checkCudaErrors(cudaMemcpy(this->dev_matches, this->matches.data(),
			this->current_plane_counter * sizeof(My_Type::Vector2i), cudaMemcpyHostToDevice));
		...
		block_rect.y = this->cell_mat_size.height;
		block_rect.z = 1;
		relabel_plane_labels_CUDA(block_rect, thread_rect,
								  this->dev_matches,
...
void Plane_detector::match_planes_to_new_map(const Plane_info * model_planes,
											 int model_plane_number,
											 const int * dev_current_plane_labels,
											 const int * dev_model_plane_labels,
											 std::vector<std::pair<int, int>> & plane_matches)
{
	...
	for (int current_plane_id = 0, new_id = 1; current_plane_id < this->matches.size(); current_plane_id++)
	{
		printf("%d -> %d", this->matches[current_plane_id].x, this->matches[current_plane_id].y);
		if (this->matches[current_plane_id].y != 0)
		{
			plane_matches.push_back(std::pair<int, int>(this->matches[current_plane_id].y, new_id));
	...
	memset(this->matches.data(), 0x00, this->matches.size() * sizeof(My_Type::Vector2i));
	for (int i = 0; i < this->matches.size(); i++)	this->matches[i] = My_Type::Vector2i(i, i);
	//
	...
	{
		checkCudaErrors(cudaMemcpy(this->dev_matches, this->matches.data(),
			this->matches.size() * sizeof(My_Type::Vector2i), cudaMemcpyHostToDevice));
	...
	for (int i = 0; i < this->matches.size(); i++)
	{
		...
		//this->matches[i] = My_Type::Vector2i(this->matches[i].x, this->matches[i].y);
...
//
std::vector<std::pair<int, int>> Plane_detector::find_most_overlap_model_plane(int current_plane_label, int model_plane_counter)
{
	...
	{
		int tmep_pixel_number = this->relative_matrix[current_plane_label * MAX_MODEL_PLANES + model_index];
		if (tmep_pixel_number > 0)
		{
			overlap_list.push_back(std::pair<int, int>(model_index, tmep_pixel_number));
		}
	}
	// Sort
	std::qsort(overlap_list.data(), overlap_list.size(), sizeof(std::pair<int, int>), cmp_function);
...
{
	// 
	memset(this->cell_info_mat, 0x00, this->cell_mat_size.x * this->cell_mat_size.y * sizeof(Cell_info));
	...
	checkCudaErrors(cudaMemset(this->dev_current_plane_counter, 0x00, sizeof(int)));
	checkCudaErrors(cudaMemset(this->dev_matches, 0x00, (int)MAX_CURRENT_PLANES * sizeof(My_Type::Vector2i)));
}
...
void Plane_stereoprojection_detector::init()
{
	...
	//
	this->hist_mat = (float *)malloc(HISTOGRAM_WIDTH * HISTOGRAM_WIDTH * sizeof(float));

	//
	checkCudaErrors(cudaMalloc((void **)&(this->dev_hist_mat),
	...
	checkCudaErrors(cudaMalloc((void **)&(this->dev_hist_normal_counter), sizeof(int)));
	checkCudaErrors(cudaMalloc((void **)&(this->dev_prj_distance_hist),
		ceil_by_stride((int)(MAX_VALID_DEPTH_M / MIN_PLANE_DISTANCE), 256) * sizeof(int)));
	// Buffers for K-means iteration
	checkCudaErrors(cudaMalloc((void **)&(this->dev_plane_mean_parameters), MAX_CURRENT_PLANES * sizeof(Cell_info)));
	checkCudaErrors(cudaMalloc((void **)&(this->dev_ATA_upper_buffer), 3 * MAX_CURRENT_PLANES * sizeof(float)));
	checkCudaErrors(cudaMalloc((void **)&(this->dev_ATb_buffer), 2 * MAX_CURRENT_PLANES * sizeof(float)));
...
void Plane_stereoprojection_detector::prepare_to_detect()
{
	...
	//
	memset(this->hist_mat, 0x00, HISTOGRAM_WIDTH * HISTOGRAM_WIDTH * sizeof(float));
...
//
void Plane_stereoprojection_detector::fit_plane_for_each_cell(const My_Type::Vector3f * dev_current_points,
															  const My_Type::Vector3f * dev_current_normals,
															  Cell_info * dev_cell_info_mat)
{
	...
	// 1. fit plane for each cells
	{
		...
		block_rect.z = 1;
		// Lunch kernel function
		fit_plane_for_cells_CUDA(block_rect, thread_rect,
								 dev_current_points, dev_current_normals, 
...
//
void Plane_stereoprojection_detector::cluster_cells(Cell_info * dev_cell_info_mat,
													const Plane_info * dev_model_planes,
...
#pragma region(Generate plane paramter seed)
	// 1. Histogram normal direction of cells on PxPy(use stereoproject [nx, ny, nz] to [px, py])
	{
		thread_rect.x = 256;	thread_rect.y = 1;		thread_rect.z = 1;
		block_rect.x = this->cell_mat_size.width * this->cell_mat_size.height / thread_rect.x;
		block_rect.y = 1;		block_rect.z = 1;
		// Lunch kernel function
		histogram_PxPy_CUDA(block_rect, thread_rect, dev_cell_info_mat, this->dev_hist_mat);
		//CUDA_CKECK_KERNEL;

		// Copy out histogram
		checkCudaErrors(cudaMemcpy(this->hist_mat, this->dev_hist_mat,
			HISTOGRAM_WIDTH * HISTOGRAM_WIDTH * sizeof(float), cudaMemcpyDeviceToHost));
	}
	...
	// 2. Search peaks on PxPy as plane normal direction
	{
		...
		checkCudaErrors(cudaMemset(this->dev_hist_mat, 0x00, MAX_HIST_NORMALS * sizeof(Hist_normal)));
		checkCudaErrors(cudaMemset(this->dev_hist_normal_counter, 0x00, sizeof(int)));
		//
		thread_rect.x = SLAM_system_settings::instance()->presegment_cell_width;
		thread_rect.y = SLAM_system_settings::instance()->presegment_cell_width;
		...
		block_rect.z = 1;
		// Lunch kernel function
		find_PxPy_peaks_CUDA(block_rect, thread_rect,
							 this->dev_hist_mat,
							 this->dev_hist_normals, this->dev_hist_normal_counter);
		//CUDA_CKECK_KERNEL;
		// Copy out peaks
		checkCudaErrors(cudaMemcpy(&this->hist_normal_counter, this->dev_hist_normal_counter, sizeof(int), cudaMemcpyDeviceToHost));
	...
	{
		int init_counter = 1;
		checkCudaErrors(cudaMemcpy(this->dev_current_plane_counter, &init_counter, sizeof(int), cudaMemcpyHostToDevice));
		checkCudaErrors(cudaMemset(this->dev_current_planes, 0x00, MAX_CURRENT_PLANES * sizeof(Plane_info)));
		// 
		...
		for (int i = 0; i < this->hist_normal_counter; i++)
		{
			...
			checkCudaErrors(cudaMemset(this->dev_prj_distance_hist, 0x00,
				ceil_by_stride((int)(MAX_VALID_DEPTH_M / MIN_PLANE_DISTANCE), 256) * sizeof(float)));
			// Set CUDA grid size
			thread_rect.x = 256;		thread_rect.y = 1;		thread_rect.z = 1;
			block_rect.x = ceil_by_stride(this->cell_mat_size.width * this->cell_mat_size.height, 256) / thread_rect.x;
			block_rect.y = 1;		block_rect.z = 1;
			// Lunch kernel function
			histogram_prj_dist_CUDA(block_rect, thread_rect,
									this->dev_cell_info_mat, dev_hist_normal_ptr,
									this->dev_prj_distance_hist);
			//CUDA_CKECK_KERNEL;

			// Set CUDA grid size
			thread_rect.x = 256;		thread_rect.y = 1;		thread_rect.z = 1;
			block_rect.x = ceil_by_stride((int)(MAX_VALID_DEPTH_M / MIN_PLANE_DISTANCE), 256) / thread_rect.x;
			block_rect.y = 1;		block_rect.z = 1;
			// Lunch kernel function
			find_prj_dist_peaks_CUDA(block_rect, thread_rect,
									 this->dev_prj_distance_hist, dev_hist_normal_ptr,
									 this->dev_current_plane_counter, this->dev_current_planes);
			//CUDA_CKECK_KERNEL;
		...
		checkCudaErrors(cudaMemcpy(&this->current_plane_counter, this->dev_current_plane_counter, sizeof(int), cudaMemcpyDeviceToHost));
...
#pragma region(K-means iteration)
	// 4. K-means iteration
	{
		...
		for (int i = 0; i < kmeans_iteration_times; i++)
		{
			...
			checkCudaErrors(cudaMemset(this->dev_plane_mean_parameters, 0x00, MAX_CURRENT_PLANES * sizeof(Cell_info)));
			checkCudaErrors(cudaMemset(this->dev_ATA_upper_buffer, 0x00, 3 * MAX_CURRENT_PLANES * sizeof(float)));
			checkCudaErrors(cudaMemset(this->dev_ATb_buffer, 0x00, 2 * MAX_CURRENT_PLANES * sizeof(float)));

			// Set CUDA grid size
			thread_rect.x = 256;		thread_rect.y = 1;		thread_rect.z = 1;
			block_rect.x = ceil_by_stride(this->cell_mat_size.width * this->cell_mat_size.height, thread_rect.x) / thread_rect.x;
			block_rect.y = 1;			block_rect.z = 1;
			// Lunch kernel function
			K_mean_iterate_CUDA(block_rect, thread_rect,
								this->dev_current_planes,
								this->dev_cell_info_mat,
								this->dev_plane_mean_parameters,
								this->dev_buffer_coordinate,
								this->dev_ATA_upper_buffer, this->dev_ATb_buffer,
								this->current_plane_counter);
			//CUDA_CKECK_KERNEL;
		}
	}
#pragma endregion
	...
	// 5. Label current plane
	{
		...
		block_rect.z = 1;
		//
		label_current_planes_CUDA(block_rect, thread_rect,
								  this->dev_cell_info_mat, 
								  this->dev_current_plane_labels);
		//CUDA_CKECK_KERNEL;
	}
	...
	checkCudaErrors(cudaMemcpy(&this->current_plane_counter, this->dev_current_plane_counter, sizeof(int), cudaMemcpyDeviceToHost));
	checkCudaErrors(cudaMemcpy(this->current_planes, this->dev_current_planes,
		this->current_plane_counter * sizeof(Plane_info), cudaMemcpyDeviceToHost));


	// For render
	{
		checkCudaErrors(cudaMemcpy(this->cell_info_mat, this->dev_cell_info_mat,
			this->cell_mat_size.width * this->cell_mat_size.height * sizeof(Cell_info), cudaMemcpyDeviceToHost));
	}
...
void Plane_super_pixel_detector::init()
{
	...
	//
	this->super_pixel_mat_size = this->aligned_depth_size / SLAM_system_settings::instance()->presegment_cell_width;

	//
	this->number_of_CUDA_block_per_line = ceilf((float)SLAM_system_settings::instance()->presegment_cell_width * 3 /
												(float)SUPER_PIXEL_BLOCK_WIDTH + FLT_EPSILON);
	this->number_of_CUDA_block_per_cell = number_of_CUDA_block_per_line * number_of_CUDA_block_per_line;

	//
	//this->super_pixel_adjacent_mat = (bool *)malloc(this->super_pixel_mat_size.width * this->super_pixel_mat_size.height * sizeof(bool));
	this->super_pixel_adjacent_mat = (bool *)malloc(this->super_pixel_mat_size.width * this->super_pixel_mat_size.height * 
													this->super_pixel_mat_size.width * this->super_pixel_mat_size.height * sizeof(bool));

	//
	checkCudaErrors(cudaMalloc((void **)&(this->dev_super_pixel_id_image),
		this->aligned_depth_size.x * this->aligned_depth_size.y * sizeof(int)));
	checkCudaErrors(cudaMalloc((void **)&(this->dev_super_pixel_mat),
		this->super_pixel_mat_size.x * this->super_pixel_mat_size.y * sizeof(Super_pixel)));
	checkCudaErrors(cudaMalloc((void **)&(this->dev_super_pixel_accumulate_mat),
		this->super_pixel_mat_size.x * this->super_pixel_mat_size.y * sizeof(Super_pixel)));
	checkCudaErrors(cudaMalloc((void **)&(this->dev_cell_hessain_uppers),
		this->super_pixel_mat_size.x * this->super_pixel_mat_size.y * sizeof(My_Type::Vector3f)));
	checkCudaErrors(cudaMalloc((void **)&(this->dev_cell_nabla),
		this->super_pixel_mat_size.x * this->super_pixel_mat_size.y * sizeof(My_Type::Vector2f)));
	checkCudaErrors(cudaMalloc((void **)&(this->dev_base_vectors),
		this->super_pixel_mat_size.x * this->super_pixel_mat_size.y * sizeof(Plane_coordinate)));
...
void Plane_super_pixel_detector::prepare_to_detect()
{
	...
	//
	memset(this->super_pixel_adjacent_mat, 0x00, this->super_pixel_mat_size.width * this->super_pixel_mat_size.height * 
		   this->super_pixel_mat_size.width * this->super_pixel_mat_size.height * sizeof(bool));
	//
	checkCudaErrors(cudaMemset(this->dev_super_pixel_mat, 0x00,
		this->super_pixel_mat_size.x * this->super_pixel_mat_size.y * sizeof(Super_pixel)));
...
//
void Plane_super_pixel_detector::presegment_to_cell(const My_Type::Vector3f * dev_current_points,
													const My_Type::Vector3f * dev_current_normals)
{
	dim3 block_rect(1, 1, 1), thread_rect(1, 1, 1);
	...
	// Init super dev_super_pixel_id_image
	{
		...
		init_super_pixel_image_CUDA(block_rect, thread_rect,
									dev_current_points, dev_current_normals,
	...
	for (int i = 0; i < 5; i++)
	{
		checkCudaErrors(cudaMemset(this->dev_super_pixel_accumulate_mat, 0x00,
			this->super_pixel_mat_size.x * this->super_pixel_mat_size.y * sizeof(Super_pixel)));
		...
		block_rect.y = this->aligned_depth_size.y / SUPER_PIXEL_BLOCK_WIDTH;
		block_rect.z = this->number_of_CUDA_block_per_cell;
		update_cluster_center_CUDA(block_rect, thread_rect,
								   dev_current_points, dev_current_normals, 
								   this->dev_super_pixel_id_image,
								   this->dev_super_pixel_accumulate_mat, this->dev_super_pixel_mat,
								   SLAM_system_settings::instance()->presegment_cell_width, 
								   this->number_of_CUDA_block_per_line);
		//CUDA_CKECK_KERNEL;
		...
		block_rect.z = 1;
		pixel_find_associate_center_CUDA(block_rect, thread_rect,
										 dev_current_points, dev_current_normals,
	...
	// for render
	checkCudaErrors(cudaMemcpy(this->dev_current_cell_labels, this->dev_super_pixel_id_image,
		this->aligned_depth_size.width * this->aligned_depth_size.height * sizeof(int), cudaMemcpyDeviceToDevice));
}



//
void Plane_super_pixel_detector::fit_plane_for_each_cell(const My_Type::Vector3f * dev_current_points,
														 const My_Type::Vector3f * dev_current_normals,
														 Cell_info * dev_cell_info_mat)
{
	dim3 block_rect(1, 1, 1), thread_rect(1, 1, 1);
	...
	// Fit plane for each cell
	{
		checkCudaErrors(cudaMemset(this->dev_super_pixel_accumulate_mat, 0x00,
			this->super_pixel_mat_size.x * this->super_pixel_mat_size.y * sizeof(Super_pixel)));
		checkCudaErrors(cudaMemset(this->dev_cell_hessain_uppers, 0x00,
			this->super_pixel_mat_size.x * this->super_pixel_mat_size.y * sizeof(My_Type::Vector3f)));
		checkCudaErrors(cudaMemset(this->dev_cell_nabla, 0x00,
			this->super_pixel_mat_size.x * this->super_pixel_mat_size.y * sizeof(My_Type::Vector2f)));
		...
		block_rect.z = this->number_of_CUDA_block_per_cell;
		fit_plane_for_cells_CUDA(block_rect, thread_rect,
								 dev_current_points, this->dev_super_pixel_id_image,
								 this->dev_base_vectors, this->dev_super_pixel_mat, this->dev_super_pixel_accumulate_mat,
	...
	// Generate cell info
	{
		// dev_cell_info_mat already initiated in Plane_detector::prepare_to_detect()

		// Update each super pixel center data
		thread_rect.x = 1;	thread_rect.y = 1;	thread_rect.z = 1;
		block_rect.x = this->aligned_depth_size.x / SUPER_PIXEL_BLOCK_WIDTH;
		block_rect.y = this->aligned_depth_size.y / SUPER_PIXEL_BLOCK_WIDTH;
		block_rect.z = 1;
		generate_cells_info_CUDA(block_rect, thread_rect,
								 this->dev_super_pixel_mat, SLAM_system_settings::instance()->sensor_params,
								 SLAM_system_settings::instance()->presegment_cell_width,
								 this->dev_cell_info_mat);
		//CUDA_CKECK_KERNEL;
	}
...
//
inline void find_next_seed_point(Cell_info * cell_mat, const My_Type::Vector2i & cell_mat_size,
...
//
inline bool flood_fill_search_pixel(Cell_info * cell_mat, const My_Type::Vector2i & cell_mat_size,
									...
									My_Type::Vector2i & searched_pixel)
{
	...
	for (int offset_index = 0; offset_index < 8; offset_index++)
	{
		...
		// Validate index
		if (u_cell < 0 || u_cell >= cell_mat_size.width ||
			v_cell < 0 || v_cell >= cell_mat_size.height)	continue;
		int index_cell = u_cell + v_cell * cell_mat_size.width;

		// Load cell info
		Cell_info temp_cell = cell_mat[index_cell];
		if (temp_cell.plane_index != 0 || 
			temp_cell.counter < 50 ||
			!temp_cell.is_valid_cell)	continue;

		// Check normal 
		My_Type::Vector3f cell_normal(temp_cell.nx, temp_cell.ny, temp_cell.nz);
		My_Type::Vector3f plane_normal(plane_params.nx, plane_params.ny, plane_params.nz);
		if (cell_normal.dot(plane_normal) < 0.95)	continue;

		// Check position
		My_Type::Vector3f cell_position(temp_cell.x, temp_cell.y, temp_cell.z);
		if (fabsf(fabsf(cell_position.dot(plane_normal)) - plane_params.d) > 0.04)	continue;
								...
								Plane_info & temp_plane)
{
	// Normal
	{
		...
		My_Type::Vector3f cell_normal(cell_info_mat[cell_index].nx, cell_info_mat[cell_index].ny, cell_info_mat[cell_index].nz);
		//
		float weight = (float)cell_info_mat[cell_index].counter / ((float)cell_info_mat[cell_index].counter + (float)temp_plane.weight);
	...
	for (int index_list_id = 0; index_list_id < cell_index_list.size(); index_list_id++)
	{
...
void build_adjacent_matrix_for_regions(Cell_info * cell_info_mat, My_Type::Vector2i cell_mat_size,
									   bool * adjacent_mat, int number_of_regions)
{
	...
	memset(adjacent_mat, 0x00, number_of_regions * number_of_regions * sizeof(bool));
	//
		...
		for (int u_cell = 0; u_cell < cell_mat_size.width; u_cell++)
	{
		...
		for (int check_i = 0; check_i < search_window_size; check_i++)
		{
			...
			if (u_check < 0 || u_check >= cell_mat_size.width ||
				v_check < 0 || v_check >= cell_mat_size.height)	continue;
			//
			...
			//
			adjacent_mat[center_plane_id + check_plane_id * number_of_regions] = true;
			adjacent_mat[check_plane_id + center_plane_id * number_of_regions] = true;
		}
	}
}
...
void generate_local_coordinate_host(Plane_coordinate & local_coordinate)
{
	...
	// Orthogonalization
	local_coordinate.x_vec = local_coordinate.x_vec - local_coordinate.x_vec.dot(local_coordinate.z_vec) * local_coordinate.z_vec;
	// Normalize base_x
...
//
inline void transform_to_local_coordinate_host(My_Type::Vector3f & src_point,
...
//
inline void transform_from_local_coordinate_host(My_Type::Vector3f & src_point,
												 My_Type::Vector3f & dst_point,
												 Plane_coordinate & local_coordinate)
{
								  ...
								  std::vector<int> & planar_cell_index_list)
{
	...
	plane_coordinate.z_vec = My_Type::Vector3f(plane_region_params.nx, plane_region_params.ny, plane_region_params.nz);
	...
	for (int index_id = 0; index_id < number_of_region_cells; index_id++)
	{
		...
		{
			mean_position += My_Type::Vector3f(cell_info_mat[cell_index].x, cell_info_mat[cell_index].y, cell_info_mat[cell_index].z);
	...
	if (valid_cell_counter >= 10)
	{
		...
		hessain_upper[0] = 0;	hessain_upper[1] = 0;	hessain_upper[2] = 0;
		nabla[0] = 0;	nabla[1] = 0;
		for (int index_id = 0; index_id < number_of_region_cells; index_id++)
		{
			int cell_index = planar_cell_index_list[index_id];
			...
			My_Type::Vector3f cell_position(cell_info_mat[cell_index].x, cell_info_mat[cell_index].y, cell_info_mat[cell_index].z);
			cell_position -= mean_position;
			My_Type::Vector3f local_cell_position;
			transform_to_local_coordinate_host(cell_position, local_cell_position, plane_coordinate);
		...
		// Compute Crammer
		D = hessain_upper[0] * hessain_upper[2] - hessain_upper[1] * hessain_upper[1];
		if (D > 1.0f)
		{
			...
			//
			transform_from_local_coordinate_host(normal_vec_local, plane_normal_vec, plane_coordinate);
...
#pragma endregion
	...
	if (valid_cell_counter > 0)
	{
		...
		for (int index_id = 0; index_id < number_of_region_cells; index_id++)
		{
			int cell_index = planar_cell_index_list[index_id];
			if (cell_info_mat[cell_index].is_valid_cell)
				mean_normal += My_Type::Vector3f(cell_info_mat[cell_index].nx, cell_info_mat[cell_index].ny, cell_info_mat[cell_index].nz);
		}
						 ...
						 bool * adjacent_mat)
{
	...
	for (search_id = 0; search_id < number_of_regions; search_id++)
	{
		if (adjacent_mat[base_id * number_of_regions + search_id] == true &&
			merge_flag[search_id] == false &&
			search_id != base_id &&
			plane_region_params[search_id].is_valid)
		{
			My_Type::Vector3f normal_base(plane_region_params[base_id].nx, plane_region_params[base_id].ny, plane_region_params[base_id].nz);
			My_Type::Vector3f normal_search(plane_region_params[search_id].nx, plane_region_params[search_id].ny, plane_region_params[search_id].nz);
			...
			My_Type::Vector3f plane_center(plane_region_params[search_id].x, plane_region_params[search_id].y, plane_region_params[search_id].z);
			My_Type::Vector3f base_center(plane_region_params[base_id].x, plane_region_params[base_id].y, plane_region_params[base_id].z);
...
// 
void merge_planar_regions(Cell_info * cell_info_mat, My_Type::Vector2i cell_mat_size,
						  bool * adjacent_mat, 
						  std::vector<Plane_info> & plane_region_params,
						  std::vector<std::vector<int>> & planar_cell_index_list)
{
	...
	while (seed_id < plane_region_params.size())
	{
		...
		do
		{
			...
			bool is_find = search_merge_region(base_id, searched_id, merge_flag, plane_region_params, adjacent_mat);
			if (is_find)
			{
				// 
	...
	for (int cell_index = 0; cell_index < cell_mat_size.width * cell_mat_size.height ; cell_index++)
	...
	for (int new_plane_id = 1; new_plane_id < merge_list.size(); new_plane_id++)
	{
		...
		for (int j = 0; j < merge_list[new_plane_id].size(); j++)
		{
			...
			for (int cell_index_id = 0; cell_index_id < planar_cell_index_list[list_id].size(); cell_index_id++)
			{
			...
			temp_cell_index_list.insert(temp_cell_index_list.end(), 
										planar_cell_index_list[list_id].begin(), planar_cell_index_list[list_id].end());
		}
		// Update merged plane info
		Plane_info temp_plane_info;
		fit_params_for_planar_region(cell_info_mat, temp_plane_info, temp_cell_index_list);
		plane_param_buffer.push_back(temp_plane_info);
		//printf("%d : %f, %f, %f, %f", new_plane_id , temp_plane_info.nx, temp_plane_info.ny, temp_plane_info.nz, temp_plane_info.d);
...
void relabel_cells_by_large_regions(Cell_info * cell_info_mat, 
									std::vector<Plane_info> & plane_region_params,
									std::vector<std::vector<int>> & planar_cell_index_list)
{
	std::vector<Plane_info> plane_param_buffer;
	...
	//
	int new_plane_id = 1;
	for (int plane_id = 1, new_plane_id = 1; plane_id < plane_region_params.size(); plane_id++)
	{
		...
		{
			for (int cell_index_id = 0; cell_index_id < planar_cell_index_list[plane_id].size(); cell_index_id++)
			{
...
//
void Plane_super_pixel_detector::cluster_cells(Cell_info * dev_cell_info_mat,
											   const Plane_info * dev_model_planes,
											   Plane_info * dev_current_planes,
											   bool with_continuous_frame_tracking)
{
...
#pragma region(Flood fill)
	...
	{
		//// Init 
		//for (int cell_index = 0; cell_index < this->cell_mat_size.width * this->cell_mat_size.height; cell_index++)
		//	this->cell_info_mat[cell_index].plane_index = 0;

		// Merge SuperPixels ------------------
		//
		int seed_point_index = 0, temp_plane_index = 0;
		int cell_mat_length = this->cell_mat_size.width * this->cell_mat_size.height;
		//
		...
		while (seed_point_index < cell_mat_length)
		{
			...
			// Init plane params
			find_next_seed_point(this->cell_info_mat, this->cell_mat_size, seed_point_index);
			cell_index_list.push_back(seed_point_index);
			update_plane_params(this->cell_info_mat, cell_index_list, temp_plane);
			find_next_seed_point(this->cell_info_mat, this->cell_mat_size, seed_point_index);

			//
			My_Type::Vector2i center_pixel(seed_point_index % this->cell_mat_size.width,
			...
			{
				My_Type::Vector2i searched_pixel;
				bool find_next = flood_fill_search_pixel(this->cell_info_mat, this->cell_mat_size, temp_plane,
														 center_pixel,
														 searched_pixel);
				...
				if (find_next)
				{
					...
					int center_index = center_pixel.x + center_pixel.y * this->cell_mat_size.width;
		...
		{
			for (int cell_index_id = 0; cell_index_id < this->planar_cell_index_list[list_id].size(); cell_index_id++)
			{
		...
		// Merge regions ------------------
		// Build adjacent matrix
		build_adjacent_matrix_for_regions(this->cell_info_mat, this->cell_mat_size, this->super_pixel_adjacent_mat, number_of_regions);

		//
		for (int i = 1; i < number_of_regions; i++)
			fit_params_for_planar_region(this->cell_info_mat, this->plane_region_params[i], this->planar_cell_index_list[i]);

		// Merge
		merge_planar_regions(this->cell_info_mat, this->cell_mat_size, this->super_pixel_adjacent_mat, this->plane_region_params, this->planar_cell_index_list);

		// Region to plane 
		relabel_cells_by_large_regions(this->cell_info_mat, this->plane_region_params, this->planar_cell_index_list);
		...
		checkCudaErrors(cudaMemcpy(this->dev_cell_info_mat, this->cell_info_mat,
			this->cell_mat_size.width * this->cell_mat_size.height * sizeof(Cell_info), cudaMemcpyHostToDevice));
...
#pragma endregion
	...
	//for (int plane_id = 1; plane_id < this->current_plane_counter; plane_id++)
	//{
	//	My_Type::Vector3f normal_vec(this->current_planes[plane_id].nx, this->current_planes[plane_id].ny,
	//								 this->current_planes[plane_id].nz);
	//	printf("%d : %f, %f, %f, %f\n", plane_id,
	...
	// Re-label
	{
		...
		block_rect.y = this->aligned_depth_size.y / SUPER_PIXEL_BLOCK_WIDTH;
		block_rect.z = this->number_of_CUDA_block_per_cell;
		relabel_super_pixels_CUDA(block_rect, thread_rect,
								  this->dev_cell_info_mat, this->dev_super_pixel_id_image,
								  this->dev_current_plane_labels, 
								  SLAM_system_settings::instance()->presegment_cell_width,
								  this->number_of_CUDA_block_per_line);
		//CUDA_CKECK_KERNEL;

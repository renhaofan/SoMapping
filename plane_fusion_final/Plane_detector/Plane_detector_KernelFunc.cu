...
//
__device__ inline void transform_to_local_coordinate(My_Type::Vector3f & src_point,
													 My_Type::Vector3f & dst_point,
													 Plane_coordinate & local_coordinate);
//
__device__ inline void transform_from_local_coordinate(My_Type::Vector3f & src_point,
													   My_Type::Vector3f & dst_point,
													   Plane_coordinate & local_coordinate);
...
#pragma region(Compute position of points in this cell)
	...
	see : https://stackoverflow.com/questions/27230621/cuda-shared-memory-inconsistent-results
	*/
...
#pragma region(plane area)
		...
		block_256_reduce(cache_f0, tid);
		// 
		if (tid == 0)	atomicAdd(&plane_mean_paramters[plane_index].area, cache_f0[0]);
...
__global__ void build_local_plane_coordinate_KernelFunc(const Plane_info * current_plane,
														Plane_coordinate * local_coordinate)
{
														...
														Plane_info * current_plane, int plane_num)
{
	...
	for (int plane_index = 1; plane_index < plane_num; plane_index++)
	{
		...
		{
			// Load position
			cell_point.x = cell_info_mat[cell_index].x - plane_mean_paramters[plane_index].x;
			cell_point.y = cell_info_mat[cell_index].y - plane_mean_paramters[plane_index].y;
			cell_point.z = cell_info_mat[cell_index].z - plane_mean_paramters[plane_index].z;
			transform_to_local_coordinate(cell_point, local_cell_point, local_coordinate[plane_index]);
		}
		...
		if (tid == 0)	current_plane[plane_index].area = plane_mean_paramters[plane_index].area;
	}
														...
														Plane_coordinate * local_coordinate)
{
	...
	transform_from_local_coordinate(local_normal_vec, normal_vec, local_coordinate[plane_index]);
						 ...
						 float * ATA_upper_buffer, float * ATb_buffer, int plane_num)
{
	...
	build_local_plane_coordinate_KernelFunc << <plane_num, 1 >> >(current_plane, local_coordinate);
	// 
	...
	recompute_plane_normal_step2_KernelFunc << <plane_num, 1 >> >(current_plane, plane_mean_paramters,
																  ATA_upper_buffer, ATb_buffer, local_coordinate);
...
//
__global__ void relabel_plane_labels_KernelFunc(const My_Type::Vector2i * matches,
												int * current_plane_labels)
{
	// Coordinate/index of pixel
	int u = threadIdx.x + blockIdx.x * blockDim.x;
							   ...
							   int * current_plane_labels)
{
	...
	relabel_plane_labels_KernelFunc << <block_rect, thread_rect >> >(matches,
																	 current_plane_labels);
}
...
//
__global__ void count_overlap_pixel_number_KernelFunc(const int * current_plane_labels,
									 ...
									 int * relative_matrix)
{
	...
	count_overlap_pixel_number_KernelFunc << <block_rect, thread_rect >> >(current_plane_labels, model_plane_labels, current_plane_counter,
																		   relative_matrix);
}
...
//
__global__ void init_super_pixel_image_KernelFunc(const My_Type::Vector3f * points,
												  ...
												  int number_of_block_per_line)
{
	...
	if (point_vec.z > FLT_EPSILON &&
		normal_vec.norm() > FLT_EPSILON)
	{	is_valid_pixel = true;	}
	else
...
void init_super_pixel_image_CUDA(dim3 block_rect, dim3 thread_rect, 
								 ...
								 int number_of_block_per_line)
{
	init_super_pixel_image_KernelFunc << <block_rect, thread_rect >> >(points, normals, super_pixel_id_image,
																	   super_pixel_width, number_of_block_per_line);
}


//
__global__ void update_cluster_center_KernelFunc(const My_Type::Vector3f * points,
												 const My_Type::Vector3f * normals, 
												 const int * super_pixel_id_image, 
												 Super_pixel * accumulate_super_pixels,
												 int super_pixel_width,
												 int number_of_block_per_line)
{
	...
	if (offset_x < super_pixel_width * 3 &&
		offset_y < super_pixel_width * 3)
	{
		// compute the start of the search window
	...
	for (int i = 0; i < 3; i++)
	{
		...
		block_256_reduce(cache_f, tid);
		//
		if (tid == 0)	atomicAdd(&accumulate_super_pixels[super_pixel_id].normal_data[i], cache_f[0]);
	}
	// Reduce position
	for (int i = 0; i < 3; i++)
	{
		...
		block_256_reduce(cache_f, tid);
		//
		if (tid == 0)	atomicAdd(&accumulate_super_pixels[super_pixel_id].position_data[i], cache_f[0]);
	}
	// Reduce center
	for (int i = 0; i < 2; i++)
	{
		...
		block_256_reduce(cache_f, tid);
		//
		if (tid == 0)	atomicAdd((float *)&accumulate_super_pixels[super_pixel_id].center_data[i], cache_f[0]);
	}
	__syncthreads();
	// Reduce valid counter
	{
		...
		//
		if (tid == 0)	atomicAdd(&accumulate_super_pixels[super_pixel_id].valid_pixel_number, cache_i[0]);
	}
}
//
__global__ void process_accumulate_super_pixel_KernelFunc(const Super_pixel * accumulate_super_pixels,
														  int number_of_block_per_line,
														  Super_pixel * super_pixels)
{
	...
	center_vec.x += temp_sp.cx;		center_vec.y += temp_sp.cy;
	normal_vec.x += temp_sp.nx;		normal_vec.y += temp_sp.ny;		normal_vec.z += temp_sp.nz;
	point_vec.x +=  temp_sp.px;		point_vec.y +=  temp_sp.py;		point_vec.z +=  temp_sp.pz;
	...
	//
	temp_sp.cx = center_vec.x;	temp_sp.cy = center_vec.y;
	temp_sp.nx = normal_vec.x;	temp_sp.ny = normal_vec.y;	temp_sp.nz = normal_vec.z;
	temp_sp.px = point_vec.x;	temp_sp.py = point_vec.y;	temp_sp.pz = point_vec.z;
...
//
void update_cluster_center_CUDA(dim3 block_rect, dim3 thread_rect, 
								const My_Type::Vector3f * points,
								const My_Type::Vector3f * normals,
								const int * super_pixel_id_image,
								Super_pixel * accumulate_super_pixels,
								...
								int number_of_block_per_line)
{
	...
	update_cluster_center_KernelFunc << <block_rect, thread_rect >> >(points, normals, super_pixel_id_image,
																	  accumulate_super_pixels, super_pixel_width, number_of_block_per_line);

	//
	dim3 block_rect_2 = block_rect;
	dim3 thread_rect_2(1,1,1);
	block_rect_2.z = 1;
	process_accumulate_super_pixel_KernelFunc << <block_rect_2, thread_rect_2 >> >(accumulate_super_pixels, number_of_block_per_line,
																				   super_pixels);
}


//
__device__ inline float compute_super_pixel_distance(const Super_pixel & temp_sp, 
													 const My_Type::Vector3f & normal_vec,
													 const My_Type::Vector3f & point_vec,
													 const My_Type::Vector2f & center_vec,
													 ...
													 Sensor_params sensor_params, int & super_pixel_width)
{
	...
	float outlayer_distance = sp_point.z * super_pixel_width * 3.0 / sensor_params.sensor_fx;
	...
	//
	//float distance_point = point_distance / (sensor_params.sensor_noise_ratio * point_vec.z);
	float distance_laplacian = fabsf(diff_vec.dot(sp_normal)) / (sensor_params.sensor_noise_ratio * point_vec.z);
	//
	...
	distance_laplacian *= (1.0f - weight_pixel_data) * (1.0f - weight_normal_position);
	
	return (distance_normal + distance_laplacian + distance_center);
}
//
__global__ void pixel_find_associate_center_KernelFunc(const My_Type::Vector3f * points,
													   const My_Type::Vector3f * normals, 
													   ...
													   Sensor_params sensor_params)
{
	...
	int pixel_index = u + v * image_width;
	if (super_pixel_id_image[pixel_index] == INVALID_PXIEL)	is_valid_pixel = false;
	...
	if (is_valid_pixel)
	{
		...
		for (int v_offset = -1; v_offset <= 1; v_offset++)	for (int u_offset = -1; u_offset <= 1; u_offset++)
		{
				...
				v_sp_check >= 0 && v_sp_check < super_pixel_mat_height)
			{
				...
				float distance = compute_super_pixel_distance(sp_check, normal_vec, point_vec, center_vec,
...
//
void pixel_find_associate_center_CUDA(dim3 block_rect, dim3 thread_rect,
									  const My_Type::Vector3f * points,
									  const My_Type::Vector3f * normals,
									  const Super_pixel * super_pixels,
									  int * super_pixel_id_image,
									  ...
									  Sensor_params sensor_params)
{
	pixel_find_associate_center_KernelFunc << <block_rect, thread_rect >> >(points, normals, super_pixels,
																			super_pixel_id_image, super_pixel_width,
																			weight_pixel_data, weight_normal_position, 
																			sensor_params);
}
...
// 
__global__ void generate_base_vector_for_each_cell_KernelFunc(const Super_pixel * super_pixels, 
															  Plane_coordinate * base_vectors)
{
	// Coordinate/index of cell
...
// 
__global__ void fit_plane_for_cells_KernelFunc(const My_Type::Vector3f * points,
											   const int * super_pixel_id_image,
											   const Plane_coordinate * base_vectors,
											   const Super_pixel * super_pixels,
											   Super_pixel * accumulate_super_pixels,
											   My_Type::Vector3f * cell_hessain,
											   My_Type::Vector2f * cell_nabla,
											   int super_pixel_width,
											   int number_of_block_per_line)
{
	// Important! this reduce many usefuless atomic operations (speed up 1.5x)
	...
	{	jacobian[0] = 0;	jacobian[1] = 0;	residual = 0;	}
...
//
__global__ void update_super_pixel_params_KenrelFunc(const Super_pixel * accumulate_super_pixels,
													 const My_Type::Vector3f * cell_hessain_upper,
													 const My_Type::Vector2f * cell_nabla,
													 const Plane_coordinate * cell_base_vectors,
													 Super_pixel * super_pixels)
{
	int super_pixel_id = blockIdx.x + blockIdx.y * gridDim.x;
	...
	if (temp_sp.valid_pixel_number > 0)
	{
		...
		My_Type::Vector3f hessain_upper = cell_hessain_upper[super_pixel_id] / (float)temp_sp.valid_pixel_number;
		My_Type::Vector2f nabla = cell_nabla[super_pixel_id] / temp_sp.valid_pixel_number;
		float hessain_inv_upper[3], inc_coeffs[2];
		hessain_inv_upper[0] = 1 / hessain_upper.data[0];
		hessain_inv_upper[1] = - hessain_upper.data[1] / (hessain_upper.data[0] * hessain_upper.data[2]);
		hessain_inv_upper[2] = 1 / hessain_upper.data[2];
		// G-N solver : X = inv(J'J) * J'r; nabla = J'r
		inc_coeffs[0] = hessain_inv_upper[0] * nabla[0] + hessain_inv_upper[1] * nabla[1];
		inc_coeffs[1] = hessain_inv_upper[1] * nabla[0] + hessain_inv_upper[2] * nabla[1];
		// Update normal vector
		Plane_coordinate base_coordinate = cell_base_vectors[super_pixel_id];
		My_Type::Vector3f new_normal;
		new_normal = inc_coeffs[0] * base_coordinate.x_vec + inc_coeffs[1] * base_coordinate.y_vec + base_coordinate.z_vec;
		new_normal.normlize();
		// 
		temp_sp.nx = new_normal.x;	temp_sp.ny = new_normal.y;	temp_sp.nz = new_normal.z;
...
// 
__global__ void fit_plane_for_cells_KernelFunc_2(const My_Type::Vector3f * points,
												 const int * super_pixel_id_image,
												 const Plane_coordinate * base_vectors,
												 const Super_pixel * super_pixels,
												 Super_pixel * accumulate_super_pixels,
												 ...
												 int number_of_block_per_line)
{
	...
	if (point_need_add)
	{
		...
		transform_to_local_coordinate(point_transfer, point_local, cell_base_coordinate);
...
//
__global__ void update_super_pixel_params_KenrelFunc_2(const Super_pixel * accumulate_super_pixels,
													   const My_Type::Vector3f * cell_hessain_upper,
													   const My_Type::Vector2f * cell_nabla,
													   const Plane_coordinate * cell_base_vectors,
													   Super_pixel * super_pixels)
{
	...
	if (temp_sp.valid_pixel_number > 0)
	{
		...
		transform_from_local_coordinate(normal_vec_local, normal_vec, cell_local_coordinate);
		// Check direction
...
void fit_plane_for_cells_CUDA(dim3 block_rect, dim3 thread_rect,
							  ...
							  Super_pixel * super_pixels,
							  Super_pixel * accumulate_super_pixels,
							  My_Type::Vector3f * cell_hessain_upper,
							  My_Type::Vector2f * cell_nabla,
							  int super_pixel_width,
							  int number_of_block_per_line)
{
	// Generate tangent plane base vectors
	generate_base_vector_for_each_cell_KernelFunc << <block_rect, 1 >> >(super_pixels,
																		 base_vectors);
	if (false)
	{
		// Fit plane for each super pixel
		fit_plane_for_cells_KernelFunc << < block_rect, thread_rect >> > (points, super_pixel_id_image, base_vectors, super_pixels,
																		  accumulate_super_pixels, cell_hessain_upper, cell_nabla,
																		  super_pixel_width, number_of_block_per_line);

		// Solve x = (J'J)^-1 J'r and update super pixel params
		update_super_pixel_params_KenrelFunc << < block_rect, 1 >> >(accumulate_super_pixels, cell_hessain_upper, cell_nabla, base_vectors,
																	 super_pixels);
	}
	else
	{
		// Fit plane for each super pixel
		fit_plane_for_cells_KernelFunc_2 << < block_rect, thread_rect >> > (points, super_pixel_id_image, base_vectors, super_pixels,
																		  accumulate_super_pixels, cell_hessain_upper, cell_nabla,
																		  super_pixel_width, number_of_block_per_line);

		// Solve x = (J'J)^-1 J'r and update super pixel params
		update_super_pixel_params_KenrelFunc_2 << < block_rect, 1 >> >(accumulate_super_pixels, cell_hessain_upper, cell_nabla, base_vectors,
																	 super_pixels);
	}
}
...
__global__ void eliminate_planar_cell_outlayers_KernelFunc(const My_Type::Vector3f * points,
														   Sensor_params sensor_params,
														   int * super_pixel_id_image,
														   Super_pixel * super_pixels,
														   int super_pixel_width,
														   int number_of_block_per_line)
{
	...
	//
	float outlyer_band = OUTLAYER_BAND_COEFF * sensor_params.sensor_noise_ratio * point_vec.z;
...
//
__global__ void generate_cells_info_KernelFunc(const Super_pixel * super_pixels,
											   ...
											   Cell_info * cell_info_mat)
{
	...
	if (temp_sp.valid_pixel_number >= 32 && 
		temp_sp.is_planar_cell)
	{
		Cell_info temp_cell;
		temp_cell.nx = temp_sp.nx;	temp_cell.ny = temp_sp.ny;	temp_cell.nz = temp_sp.nz;
		temp_cell.x = temp_sp.px;	temp_cell.y = temp_sp.py;	temp_cell.z = temp_sp.pz;
		temp_cell.counter = temp_sp.valid_pixel_number;

		// Ray direction
		My_Type::Vector3f ray_direction, normal_vec;
		normal_vec.x = temp_sp.nx;	normal_vec.y = temp_sp.ny;	normal_vec.z = temp_sp.nz;
		ray_direction.x = temp_sp.px;	ray_direction.y = temp_sp.py;	ray_direction.z = temp_sp.pz;
							  ...
							  Cell_info * cell_info_mat)
{
	...
	generate_cells_info_KernelFunc << <block_rect, thread_rect >> >(super_pixels, sensor_params, super_pixel_width,
																	cell_info_mat);
}
...
void relabel_super_pixels_CUDA(dim3 block_rect, dim3 thread_rect,
							   ...
							   int number_of_block_per_line)
{
	relabel_super_pixels_KernelFunc << <block_rect, thread_rect >> >(cell_info_mat, super_pixel_id_image, plane_id_image,
																	 super_pixel_width, number_of_block_per_line);
}

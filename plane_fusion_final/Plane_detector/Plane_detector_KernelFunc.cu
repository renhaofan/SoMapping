...
transform_to_local_coordinate(My_Type::Vector3f &src_point,
                              My_Type::Vector3f &dst_point,
                              Plane_coordinate &local_coordinate);
//
__device__ inline void
transform_from_local_coordinate(My_Type::Vector3f &src_point,
                                My_Type::Vector3f &dst_point,
                                Plane_coordinate &local_coordinate);
...
__global__ void find_PxPy_peaks_KernelFunc(const float *hist_mat,
  ...
  if (peak_value < MIN_AREA_OF_DIRECTION / 4.0)
    return;
...
__global__ void find_prj_dist_peaks_KernelFunc(const float *prj_dist_hist,
  ...
  // if (number_of_cells[1] < MIN_CELLS_OF_PLANE * 0.2659)		is_valid_dist =
  // false;
  if (number_of_cells[1] < MIN_AREA_OF_DIRECTION)
    is_valid_dist = false;
...
__global__ void
build_local_plane_coordinate_KernelFunc(const Plane_info *current_plane,
                                        Plane_coordinate *local_coordinate) {
...
__global__ void recompute_plane_normal_step2_KernelFunc(
  ...
  int plane_index = blockIdx.x;
  // Validate
  if (!current_plane[plane_index].is_valid)
    return;
...
__global__ void merge_similar_planes_KernelFunc(Plane_info *current_plane,
  ...
  for (int plane_id = 1; plane_id < plane_number; plane_id++) {
    if (!current_plane[plane_id].is_valid)
      continue;
...
//
__global__ void
relabel_plane_labels_KernelFunc(const My_Type::Vector2i *matches,
                                int *current_plane_labels) {
  // Coordinate/index of pixel
  int u = threadIdx.x + blockIdx.x * blockDim.x;
  int v = threadIdx.y + blockIdx.y * blockDim.y;
...
__global__ void init_super_pixel_image_KernelFunc(
  ...
  if (offset_x >= super_pixel_width || offset_y >= super_pixel_width)
    return;
  ...
  if (is_valid_pixel)
    super_pixel_id = blockIdx.x + blockIdx.y * gridDim.x;
...
__device__ inline float compute_super_pixel_distance(
  ...
  if (point_distance > outlayer_distance)
    return FLT_MAX;
...
//
__global__ void
generate_base_vector_for_each_cell_KernelFunc(const Super_pixel *super_pixels,
...
__global__ void fit_plane_for_cells_KernelFunc(
  ...
  for (int i = 0; i < 3; i++) {
    ...
    block_256_reduce(cache_f, tid);
    if (tid == 0)
      atomicAdd(&cell_hessain[super_pixel_id].data[i], cache_f[0]);
  }
  // Reduce nabla
  for (int i = 0; i < 2; i++) {
    if (point_need_add) {
    ...
    block_256_reduce(cache_f, tid);
    if (tid == 0)
      atomicAdd(&cell_nabla[super_pixel_id].data[i], cache_f[0]);
  }
  // Reduce position
  for (int i = 0; i < 3; i++) {
    if (point_need_add) {

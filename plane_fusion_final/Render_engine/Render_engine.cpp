						 ...
						 My_Type::Vector2i scene_depth_size)
{
	...
	this->range_map_size.width = 1 + (int)ceilf((float)depth_size.width
		/ (float)SLAM_system_settings::instance()->raycast_range_patch_width);
	this->range_map_size.height = 1 + (int)ceilf((float)depth_size.height
		/ (float)SLAM_system_settings::instance()->raycast_range_patch_width);

	//
	checkCudaErrors(cudaMalloc((void **)&(this->dev_scene_points),
		this->scene_depth_size.width * this->scene_depth_size.height * sizeof(My_Type::Vector3f)));
	checkCudaErrors(cudaMalloc((void **)&(this->dev_scene_normals),
		this->scene_depth_size.width * this->scene_depth_size.height * sizeof(My_Type::Vector3f)));
	checkCudaErrors(cudaMalloc((void **)&(this->dev_scene_points_weight),
		this->scene_depth_size.width * this->scene_depth_size.height * sizeof(int)));
	checkCudaErrors(cudaMalloc((void **)&(this->dev_scene_plane_label),
		this->scene_depth_size.width * this->scene_depth_size.height * sizeof(int)));
	checkCudaErrors(cudaMalloc((void **)&(this->dev_scene_points_color),
		this->scene_depth_size.width * this->scene_depth_size.height * sizeof(My_Type::Vector4uc)));

	// 
	this->range_map = (My_Type::Vector2f *)malloc(this->range_map_size.width * this->range_map_size.height
												  * sizeof(My_Type::Vector2f));
	//
	this->current_points = (My_Type::Vector3f *)malloc(this->depth_size.width * this->depth_size.height
													   * sizeof(My_Type::Vector3f));
	this->model_points = (My_Type::Vector3f *)malloc(this->depth_size.width * this->depth_size.height
													 * sizeof(My_Type::Vector3f));
	//
	this->scene_points = (My_Type::Vector3f *)malloc(this->scene_depth_size.width * this->scene_depth_size.height
													 * sizeof(My_Type::Vector3f));
	this->scene_normals = (My_Type::Vector3f *)malloc(this->scene_depth_size.width * this->scene_depth_size.height
													  * sizeof(My_Type::Vector3f));
	this->scene_points_color = (My_Type::Vector4uc *)malloc(this->scene_depth_size.width * this->scene_depth_size.height
															* sizeof(My_Type::Vector4uc));

	//
	this->current_hierarchy_normal_to_draw.init_parameters(this->depth_size, SLAM_system_settings::instance()->image_alginment_patch_width, 3);
	this->dev_current_hierarchy_normal_to_draw.init_parameters(this->depth_size, SLAM_system_settings::instance()->image_alginment_patch_width, 3);
	this->model_hierarchy_normal_to_draw.init_parameters(this->depth_size, SLAM_system_settings::instance()->image_alginment_patch_width, 3);
	this->dev_model_hierarchy_normal_to_draw.init_parameters(this->depth_size, SLAM_system_settings::instance()->image_alginment_patch_width, 3);

	//
	allocate_CUDA_memory_for_hierarchy(this->dev_current_hierarchy_normal_to_draw);
	...
	checkCudaErrors(cudaMalloc((void **)&(this->dev_viewport_2_color), 
		this->depth_size.width * this->depth_size.height * sizeof(My_Type::Vector4uc)));
	...
	//
	this->viewport_2_color = (My_Type::Vector4uc *)malloc(this->depth_size.width * this->depth_size.height
														  * sizeof(My_Type::Vector4uc));
	...
	checkCudaErrors(cudaMalloc((void **)&(this->dev_pseudo_plane_color), 
		this->depth_size.width * this->depth_size.height * sizeof(My_Type::Vector4uc)));
	this->pseudo_plane_color = (My_Type::Vector4uc *)malloc(this->depth_size.width * this->depth_size.height
															* sizeof(My_Type::Vector4uc));

	//
	this->enties_buffer = (HashEntry *)malloc((ORDERED_TABLE_LENGTH + EXCESS_TABLE_LENGTH) * sizeof(HashEntry));
...
void Render_engine::render_scene_points(MainViewportRenderMode render_mode)
{
	...
	//
	checkCudaErrors(cudaMemset(this->dev_scene_points_color, 0x00, 
		this->scene_depth_size.width * this->scene_depth_size.height * sizeof(My_Type::Vector4uc)));
	//
	switch (render_mode)
	{
		case PHONG_RENDER:
		{
			...
			checkCudaErrors(cudaMemcpy(this->dev_scene_normals, this->scene_normals,
				this->scene_depth_size.width * this->scene_depth_size.height * sizeof(My_Type::Vector3f), cudaMemcpyHostToDevice));
			...
			block_rect.z = 1;
			//
			render_gypsum_CUDA(block_rect, thread_rect, this->dev_scene_normals, this->dev_scene_points_color);
			//CUDA_CKECK_KERNEL;
	...
	// Copy color buffer
	checkCudaErrors(cudaMemcpy(this->scene_points_color, this->dev_scene_points_color, 
		this->scene_depth_size.width * this->scene_depth_size.height * sizeof(My_Type::Vector4uc), cudaMemcpyDeviceToHost));
...
//
void Render_engine::pseudo_render_depth(My_Type::Vector3f * dev_raw_aligned_points)
{
	dim3 block_rect(1, 1, 1), thread_rect(1, 1, 1);
	...
	// Pseudo render depth image
	{
		...
		block_rect.y = this->scene_depth_size.height / thread_rect.y;
		block_rect.z = 1;
		pseudo_render_depth_CUDA(block_rect, thread_rect,
								 dev_raw_aligned_points, this->dev_min_depth, this->dev_max_depth,
	...
	checkCudaErrors(cudaMemcpy(this->viewport_2_color, this->dev_viewport_2_color,
		this->depth_size.width * this->depth_size.height * sizeof(My_Type::Vector4uc), cudaMemcpyDeviceToHost));
...
void Render_engine::pseudo_render_plane_labels(int * dev_plane_labels)
{
	...
	// Render plane labels
	{
		...
		checkCudaErrors(cudaMemcpy(this->pseudo_plane_color, this->dev_pseudo_plane_color,
			this->depth_size.width * this->depth_size.height * sizeof(My_Type::Vector4uc), cudaMemcpyDeviceToHost));
	}
...
void Render_engine::generate_normal_segment_line(My_Type::Vector3f * dev_raw_aligned_points,
												 ...
												 NormalsSource normals_source)
{
	...
	// Generate normal vector segment lines (for visualization)
	{
		...
		switch (normals_source)
		{
			...
			{
				generate_line_segment_CUDA(block_rect, thread_rect,
										   dev_raw_aligned_points,
										   dev_normals,
										   this->dev_current_hierarchy_normal_to_draw.data_ptrs[0]);
				//CUDA_CKECK_KERNEL;
				//
				checkCudaErrors(cudaMemcpy(this->current_hierarchy_normal_to_draw.data_ptrs[0], 
					this->dev_current_hierarchy_normal_to_draw.data_ptrs[0],
					this->depth_size.width * this->depth_size.height * sizeof(My_Type::Line_segment), cudaMemcpyDeviceToHost));
				break;
			}
			case NormalsSource::MODEL_NORMAL:
			{
				...
				//CUDA_CKECK_KERNEL;
				//
				checkCudaErrors(cudaMemcpy(this->model_hierarchy_normal_to_draw.data_ptrs[0],
					this->dev_model_hierarchy_normal_to_draw.data_ptrs[0],
					this->depth_size.width * this->depth_size.height * sizeof(My_Type::Line_segment), cudaMemcpyDeviceToHost));
				break;
			}
...
//
void Render_engine::generate_voxel_block_lines(HashEntry * dev_entries, int number_of_entries)
{
	//
	checkCudaErrors(cudaMemcpy(this->enties_buffer, dev_entries, 
		number_of_entries * sizeof(HashEntry), cudaMemcpyDeviceToHost));
	...
	// Allocate memory
	this->voxel_block_lines = (My_Type::Vector3f*)malloc(this->number_of_blocks * sizeof(My_Type::Vector3f) * 24);
	...
	for (int block_id = 0; block_id < allocated_entries.size(); block_id++)
	{
		...
		point_list[0] = My_Type::Vector3f(temp_entry.position[0], temp_entry.position[1], temp_entry.position[2]);
		...
		point_list[3] = point_list[0] + My_Type::Vector3f(block_width,	block_width,	0);
		point_list[4] = point_list[0] + My_Type::Vector3f(0,			0,				block_width);
		point_list[5] = point_list[0] + My_Type::Vector3f(block_width,	0,				block_width);
		point_list[6] = point_list[0] + My_Type::Vector3f(0,			block_width,	block_width);
		point_list[7] = point_list[0] + My_Type::Vector3f(block_width,	block_width,	block_width);

		//
		int vertex_index = block_id * 24;
		// X direction
		this->voxel_block_lines[vertex_index + 0] = point_list[0];	this->voxel_block_lines[vertex_index + 1] = point_list[1];
		this->voxel_block_lines[vertex_index + 2] = point_list[2];	this->voxel_block_lines[vertex_index + 3] = point_list[3];
		this->voxel_block_lines[vertex_index + 4] = point_list[4];	this->voxel_block_lines[vertex_index + 5] = point_list[5];
		this->voxel_block_lines[vertex_index + 6] = point_list[6];	this->voxel_block_lines[vertex_index + 7] = point_list[7];
		// Y direction
		this->voxel_block_lines[vertex_index + 8] = point_list[0];	this->voxel_block_lines[vertex_index + 9] = point_list[2];
		this->voxel_block_lines[vertex_index + 10] = point_list[1];	this->voxel_block_lines[vertex_index + 11] = point_list[3];
		this->voxel_block_lines[vertex_index + 12] = point_list[4];	this->voxel_block_lines[vertex_index + 13] = point_list[6];
		this->voxel_block_lines[vertex_index + 14] = point_list[5];	this->voxel_block_lines[vertex_index + 15] = point_list[7];
		// Z direction
		this->voxel_block_lines[vertex_index + 16] = point_list[0];	this->voxel_block_lines[vertex_index + 17] = point_list[4];
		this->voxel_block_lines[vertex_index + 18] = point_list[1];	this->voxel_block_lines[vertex_index + 19] = point_list[5];
		this->voxel_block_lines[vertex_index + 20] = point_list[2];	this->voxel_block_lines[vertex_index + 21] = point_list[6];
		this->voxel_block_lines[vertex_index + 22] = point_list[3];	this->voxel_block_lines[vertex_index + 23] = point_list[7];
	}

										 ...
										 My_Type::Vector4uc * points_color)
{
	...
	if (normal.norm() <= 0.5)	valid_normal = false;
...
void render_gypsum_CUDA(dim3 block_rect, dim3 thread_rect, 
						My_Type::Vector3f * points_normal, My_Type::Vector4uc * points_color)
{
	render_gypsum_KernelFunc << <block_rect, thread_rect >> >(points_normal, points_color);
}


//      
__global__ void render_weight_KernelFunc(My_Type::Vector3f * points_normal, int * points_weight, 
										 My_Type::Vector4uc * points_color)
{
	//       
	int u, v, image_width;
	...
	// R G B
	if (normal.z > 0)	normal.z = -normal.z;
	color.r = (unsigned char)max(128.0 * (1.0 - weight / MAX_SDF_WEIGHT) + (1.0 - normal.z) * 64.0, 0.0f);
	color.g = (unsigned char)max(128.0 * weight / MAX_SDF_WEIGHT + (1.0 - normal.z) * 64.0, 0.0f);
	color.b = 0;
	// Alpha
	color.a = 0xFF;
						...
						My_Type::Vector4uc * points_color)
{
	...
	render_weight_KernelFunc << < block_rect, thread_rect >> > (points_normal, points_weight, points_color);
}



//    Label  
__global__ void render_plane_label_KernelFunc(My_Type::Vector3f * points_normal, int * plane_labels, 
											  My_Type::Vector4uc * points_color)
{
	//       
	int u, v, image_width;
	...
	if (plane_label > 0)
	{
		...
		color.a = 0xFF;
		// B G R
		color.b = (unsigned char)((color_i + color_table_Blue[plane_color_index]) >> 1);
		color.g = (unsigned char)((color_i + color_table_Green[plane_color_index]) >> 1);
		color.r = (unsigned char)((color_i + color_table_Red[plane_color_index]) >> 1);
	}
	else
	{
		// R G B
...
void render_plane_label_CUDA(dim3 block_rect, dim3 thread_rect, 
							 My_Type::Vector3f * points_normal, int * plane_labels, 
							 My_Type::Vector4uc * points_color)
{

	render_plane_label_KernelFunc << <block_rect, thread_rect >> >(points_normal, plane_labels, points_color);
}


//
__global__ void render_plane_label_KernelFunc(int * plane_labels,
											  My_Type::Vector4uc * points_color)
{
...
//#define PRIME_Y		19349669u
	...
	{
		int plane_color_index = (plane_label * PLANE_COLOR_STEP) % COLOR_NUM;
		//int plane_color_index = (plane_label ^ 73856093u * plane_label ^ 19349669u) % COLOR_NUM;
							 ...
							 My_Type::Vector4uc * points_color)
{
	...
	render_plane_label_KernelFunc << <block_rect, thread_rect >> >(plane_labels, points_color);
}




__global__ void reduce_range_KernelFunc(const My_Type::Vector3f * dev_raw_aligned_points,
										int * min_depth, int * max_depth)
{
	...
	if (depth_value <= FLT_EPSILON)	is_valid_depth = false;
					   ...
					   int * min_depth, int * max_depth)
{
	...
	reduce_range_KernelFunc << <block_rect, thread_rect >> >(dev_raw_aligned_points, min_depth, max_depth);
}


//
__global__ void pseudo_render_depth_KernelFunc(const My_Type::Vector3f * dev_raw_aligned_points,
											   ...
											   My_Type::Vector4uc * color_buffer)
{
	...
	// Read depth value
	float depth_value = dev_raw_aligned_points[index].z * 1000.0f;	/* millimeter */
	if (depth_value == 0.0f)	is_valid_depth = false;

	//
	My_Type::Vector4uc temp_color((unsigned char)0x00);
	if (is_valid_depth)
	{
		int color_index = (int)((depth_value - (float)min_depth[0]) / (float)(max_depth[0] - min_depth[0]) * (float)(COLOR_NUM));
...
void pseudo_render_depth_CUDA(dim3 block_rect, dim3 thread_rect,
							  ...
							  My_Type::Vector4uc * color_buffer)
{
	pseudo_render_depth_KernelFunc << <block_rect, thread_rect >> >(dev_raw_aligned_points, min_depth, max_depth,
																	color_buffer);
}
...
//
__global__ void generate_line_segment_KernelFunc(const My_Type::Vector3f * dev_raw_aligned_points,
												 ...
												 My_Type::Line_segment * dev_line_segments)
{
	...
	if (temp_point.z == 0.0f)	is_valid_depth = false;
	...
	if (is_valid_depth)
	{
		...
		temp_line_segment.dst = temp_point + NORMAL_LINE_SEGMENT_LENGTH * dev_normals[index];
	}
								...
								My_Type::Line_segment * dev_line_segments)
{
	...
	generate_line_segment_KernelFunc << <block_rect, thread_rect >> >(dev_raw_aligned_points, dev_normals,
																	  dev_line_segments);
}

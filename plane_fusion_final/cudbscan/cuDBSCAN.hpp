...
struct KernelObject {
  ...
  __device__ inline bool isValid(int tid) { return valid[tid] == 1; }
  ...
  __host__ void allocate() {
    ...
    cuda_call(
        cudaMalloc(&clusterMatrix, pointCount * pointCount * sizeof(int)));
    ...
    cuda_call(
        cudaMemset(clusterMatrix, 0, pointCount * pointCount * sizeof(int)));
  ...
  __host__ void free() {
    if (data != nullptr) cuda_call(cudaFree(data));

    if (clusterMatrix != nullptr) cuda_call(cudaFree(clusterMatrix));

    if (valid != nullptr) cuda_call(cudaFree(valid));

    if (reroute != nullptr) cuda_call(cudaFree(reroute));

    if (mergeHappened != nullptr) cuda_call(cudaFree(mergeHappened));

...
class DBSCANCPU {
 public:
  ...
  DBSCANCPU(std::vector<Point2>& _data, double _eps, int _mnpts)
      : data(_data), eps(_eps), mnpts(_mnpts) {
  ...
  int run() {
    ...
    for (int i = 0; i < pointCount; i++)
      while (merge(i))
        ;  // Means merge everybody into this one
    ...
    for (int col = 0; col < pointCount; col++) {
      ...
      for (int row = 0; row < pointCount; row++)
        if (clusterMatrix[row][col]) assignedIndices.push_back(row);

      if (assignedIndices.size() > static_cast<size_t>(mnpts)) {

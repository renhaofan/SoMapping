...
cuDBSCAN::cuDBSCAN(std::vector<Point2> &points, float eps, int mnpts) {
  ...
  for (size_t i = 0; i < points.size(); i++) labels[i] = -99;

  setData(points);
}

void cuDBSCAN::setData(std::vector<Point2> points) {
  cuda_call(cudaMemcpy(ko.data, &points[0], points.size() * sizeof(Point2),
                       cudaMemcpyHostToDevice));
  cuda_call(
      cudaMemcpy(ko.devPtr, &ko, sizeof(KernelObject), cudaMemcpyHostToDevice));
}
...
int cuDBSCAN::run() {
  ...
  for (int i = 0; i < ko.pointCount; i++)
    while (merge(i))
      ;  // Keep merging all clusters into this one

  // Copy back the results
  int *clusterMatrix = new int[ko.pointCount * ko.pointCount];
  int *valid = new int[ko.pointCount];

  cuda_call(cudaMemcpy(clusterMatrix, ko.clusterMatrix,
                       ko.pointCount * ko.pointCount * sizeof(int),
                       cudaMemcpyDeviceToHost));
  cuda_call(cudaMemcpy(valid, ko.valid, ko.pointCount * sizeof(int),
                       cudaMemcpyDeviceToHost));
  ...
  for (int col = 0; col < ko.pointCount; col++) {
    ...
    if (assignedIndices.size() > static_cast<size_t>(ko.mnpts))
      clusters[runningClusterIndex++] = assignedIndices;
  }

  for (auto &c : clusters)
    for (auto &index : c.second) labels[index] = c.first;
...
bool cuDBSCAN::merge(int clusterIndex) {
  ...
  // Did a merge happen?
  int merged = -1;
  cuda_call(cudaMemcpy(&merged, ko.mergeHappened, sizeof(int),
                       cudaMemcpyDeviceToHost));
...
__global__ void assignInitialClusters(KernelObject *cudbscan) {
  ...
  if (tid >= cudbscan->pointCount) return;
...
__global__ void mergeInto(KernelObject *cudbscan, int clusterIndex) {
  ...
  if (tid >= cudbscan->pointCount || !cudbscan->isValid(tid) ||  // Am I valid?
  ...
  if (doMerge) {
    ...
    *cudbscan->mergeHappened = 1;
    ...
    // Do the merge
    for (int i = 0; i < cudbscan->pointCount; i++) {
      if (cudbscan->get(i, tid)) cudbscan->set(i, targetCluster);
    }
  }
}

...
void Data_loader::load_ground_truth(string ground_truth_path,
  ...
  if (ifile.is_open()) {
    ...
    while (getline(ifile, current_read_line)) {
      ...
      sscanf(current_read_line.c_str(), "%lf%f%f%f%f%f%f%f", &(temp_GT.time),
             &(temp_GT.tx), &(temp_GT.ty), &(temp_GT.tz),
             &(temp_GT.quaternions.qx), &(temp_GT.quaternions.qy),
             &(temp_GT.quaternions.qz), &(temp_GT.quaternions.qr));
  ...
  if (this->ground_truth_trajectory.size() > 0) {
    ...
    for (size_t node_id = 0; node_id < this->ground_truth_trajectory.size();
         node_id++) {
      // ------------ Translation ------------
      //
      ...
      //
      if (is_ICL_NUIM_data) {
        this->ground_truth_trajectory[node_id].tx =
            +this->ground_truth_trajectory[node_id].tx;
        this->ground_truth_trajectory[node_id].ty =
            -this->ground_truth_trajectory[node_id].ty;
        this->ground_truth_trajectory[node_id].tz =
            +this->ground_truth_trajectory[node_id].tz;
      }

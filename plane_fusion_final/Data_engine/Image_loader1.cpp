...
bool Offline_image_loader::load_next_frame(double & timestamp, cv::Mat & color_mat, cv::Mat & depth_mat)
{
	...
	switch (this->image_loader_mode)
	{
		...
		case ImageLoaderMode::WITH_COLOR_AND_DEPTH:
		{
			...
			depth_mat = cv::imread(this->depth_path_vector[depth_index].c_str(), CV_LOAD_IMAGE_UNCHANGED);
			color_mat = cv::imread(this->color_path_vector[color_index].c_str(), CV_LOAD_IMAGE_UNCHANGED);
			break;
		}
...
//	{
//		//
//		if ((hFile = _findfirst(path.assign(color_folder).append("/*").c_str(), &fileinfo)) != -1)
//		{
//			do
//			{
...
//				else
//				{
//					this->color_path_vector.push_back(color_folder + fileinfo.name);
//					// save time stamp
//					double time_stamp = 0.0;
//					sscanf(fileinfo.name, "%lf", &time_stamp);
...
//	{
//		//
//		if ((hFile = _findfirst(path.assign(depth_folder).append("/*").c_str(), &fileinfo)) != -1)
//		{
//			do
//			{
//				if ((fileinfo.attrib & _A_SUBDIR)){ /* space holder */ }
//				else
//				{
//					this->depth_path_vector.push_back(depth_folder + fileinfo.name);
//					// save time stamp
//					double time_stamp = 0.0;
//					sscanf(fileinfo.name, "%lf", &time_stamp);
...
//			 depth_image_id < this->depth_timestamp_vector.size() && color_image_id < this->color_timestamp_vector.size();
//			 depth_image_id++)
//		{
//			double depth_timestamp = this->depth_timestamp_vector[depth_image_id];
//
//			while (color_image_id < this->color_timestamp_vector.size())
//			{
//				if (this->color_timestamp_vector[color_image_id] < depth_timestamp)
//				{	color_image_id++;}
//				else
//				{

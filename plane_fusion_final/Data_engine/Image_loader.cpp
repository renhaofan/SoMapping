...
Offline_image_loader::Offline_image_loader(const string cal, const string dir, int dm)
{
...
//
bool Offline_image_loader::load_next_frame(double & timestamp, cv::Mat & color_mat, cv::Mat & depth_mat)
{
    // Validate frame index
...
//
//Offline_image_loader::Offline_image_loader(string color_folder, string depth_folder)
//{
//	//
...
void Offline_image_loader::read_calibration_parameters(string cal)
{
    ...
    d2c.m03 = 0.0f; d2c.m13 = 0.0f; d2c.m23 = 0.0f; d2c.m33 = 1.0f;
    f >> d2i.m00 >> d2i.m10 >> d2i.m20 >> d2i.m30;
    f >> d2i.m01 >> d2i.m11 >> d2i.m21 >> d2i.m31;
    f >> d2i.m02 >> d2i.m12 >> d2i.m22 >> d2i.m32;
    d2i.m03 = 0.0f; d2i.m13 = 0.0f; d2i.m23 = 0.0f; d2i.m33 = 1.0f;
    string word; double scale;
    f >> word >> scale;
    SLAM_system_settings::instance()->set_intrinsic(fx, fy, cx, cy, 1.0f/scale);
    SLAM_system_settings::instance()->set_extrinsic(d2c,d2i);

//    cout << SLAM_system_settings::instance()->color_params.sensor_fx << endl;
//    cout << SLAM_system_settings::instance()->depth_params.sensor_fx << endl;
//    cout << SLAM_system_settings::instance()->depth_params.sensor_scale << endl;
...
void Offline_image_loader::detect_images(string associate, string colordir, string depthdir, int dm) {
    ifstream inf;
...
bool Offline_image_loader::jump_to_specific_frame(int frame_id)
{
	...
	//	case ImageLoaderMode::WITH_DEPTH_ONLY:
	//	{
	//		depth_mat = imread(this->depth_path_vector[this->frame_index].c_str(), CV_LOAD_IMAGE_UNCHANGED);
	//		break;
	//	}
	//	case ImageLoaderMode::WITH_COLOR_AND_DEPTH:
	//	{
	//		depth_mat = imread(this->depth_path_vector[this->frame_index].c_str(), CV_LOAD_IMAGE_UNCHANGED);
	//		color_mat = imread(this->color_path_vector[this->frame_index].c_str(), CV_LOAD_IMAGE_UNCHANGED);
	//		break;
	//	}
	//	case ImageLoaderMode::UNEQUAL_COLOR_AND_DEPTH_FRAMES:
	//		return false;
...
//
void Offline_image_loader::detect_images(string color_folder, string depth_folder)
{
    ...
    if((dp  = opendir(color_folder.c_str())) != NULL)
    {
        ...
        for (unsigned int i = 0; i < this->color_path_vector.size(); i++) {
            if (this->color_path_vector[i].at(0) != '/')
                this->color_path_vector[i] = color_folder + this->color_path_vector[i];
        }
    }
    ...
    if((dp1  = opendir(depth_folder.c_str())) != NULL)
    {
        ...
        for (unsigned int i = 0; i < this->depth_path_vector.size(); i++) {
                ...
                double TUMTimeStamp = std::atof(this->depth_path_vector[i].substr(0,pos2-1).c_str());
                this->image_timestamp_vector.push_back(TUMTimeStamp);

                this->depth_path_vector[i] = depth_folder + this->depth_path_vector[i];
    ...
    for(int count = 0; count < this->number_of_frames-1; count++){
        ...
        this->image_timestamp_vector.push_back((double(1.0/30.0) * double(count)));
    }


    // Set mode of data loader
    if (this->depth_path_vector.size() == 0 && this->color_path_vector.size() == 0)
    {
        this->image_loader_mode = ImageLoaderMode::NO_DATA;
        this->number_of_frames = 0;
    }
    else if (this->depth_path_vector.size() > 0 && this->color_path_vector.size() == 0)
    {
...
void Offline_image_loader::read_image_parameters()
{
	...
	if (this->color_path_vector.size() > 0)
	{
		...
		// Read color images
		temp_mat = cv::imread(this->color_path_vector[0].c_str(), CV_LOAD_IMAGE_UNCHANGED);
		...
		//
		//cout << "color_width\t" << this->color_width << "\tcolor_height\t" << this->color_height << endl;
		//cout << "color_element_size\t" << this->color_element_size << endl;
		//cout << "color_channel_num\t" << this->color_channel_num << endl << endl;
	...
	if (this->depth_path_vector.size() > 0)
	{
		...
		// Read depth images
		temp_mat = cv::imread(this->depth_path_vector[0].c_str(), CV_LOAD_IMAGE_UNCHANGED);
		...
		//
		//cout << "depth_width\t" << this->depth_width << "\tdepth_height\t" << this->depth_height << endl;
		//cout << "depth_element_size\t" << this->depth_element_size << endl;
		//cout << "depth_channel_num\t" << this->depth_channel_num << endl << endl;
	}
...
void Offline_image_loader::print_state(bool print_all_pathes) const
{
	switch (this->image_loader_mode)
	{
		...
		case ImageLoaderMode::WITH_DEPTH_ONLY:
		{
			...
			if (print_all_pathes)
			{
				for (size_t path_id = 0; path_id < this->depth_path_vector.size(); path_id++)
				{
		...
		case ImageLoaderMode::WITH_COLOR_AND_DEPTH:
		{
			...
			if (print_all_pathes)
			{
				...
				for (size_t path_id = 0; path_id < this->color_path_vector.size(); path_id++)
				{

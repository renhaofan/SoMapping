...
void Preprocess_engine::init(My_Type::Vector2i raw_color_size,
  ...
  this->aligned_depth_size.width =
      ceil_by_stride(this->raw_depth_size.width, this->image_alignment_width);
...
void Preprocess_engine::copy_previous_intensity_as_model() {
  for (int layer_id = 0; layer_id < this->dev_hierarchy_points.number_of_layers;
    ...
    checkCudaErrors(cudaMemcpy(
        ...
        this->dev_hierarchy_intensity.size[layer_id].width *
            this->dev_hierarchy_intensity.size[layer_id].height * sizeof(float),
        cudaMemcpyDeviceToDevice));
    checkCudaErrors(
        cudaMemcpy(this->dev_hierarchy_model_gradient.data_ptrs[layer_id],
                   this->dev_hierarchy_gradient.data_ptrs[layer_id],
                   this->dev_hierarchy_gradient.size[layer_id].width *
                       this->dev_hierarchy_gradient.size[layer_id].height *
...
void Preprocess_RGBD::init(My_Type::Vector2i raw_color_size,
  ...
  this->dev_hierarchy_points.init_parameters(
      this->raw_depth_size, this->image_alignment_width, number_of_layers);
  this->dev_hierarchy_normals.init_parameters(
      this->raw_depth_size, this->image_alignment_width, number_of_layers);
  this->dev_hierarchy_model_points.init_parameters(
      this->raw_depth_size, this->image_alignment_width, number_of_layers);
  this->dev_hierarchy_model_normals.init_parameters(
      this->raw_depth_size, this->image_alignment_width, number_of_layers);
  this->dev_hierarchy_intensity.init_parameters(
      this->raw_color_size, this->image_alignment_width, number_of_layers);
  this->dev_hierarchy_gradient.init_parameters(
      this->raw_color_size, this->image_alignment_width, number_of_layers);
  this->dev_hierarchy_model_intensity.init_parameters(
      this->raw_color_size, this->image_alignment_width, number_of_layers);
  this->dev_hierarchy_model_gradient.init_parameters(
      this->raw_color_size, this->image_alignment_width, number_of_layers);
  ...
  checkCudaErrors(
      cudaMalloc((void **)&(this->dev_raw_color),
                 this->raw_color_size.width * this->raw_color_size.height *
                     sizeof(RawColorType)));
  checkCudaErrors(
      cudaMalloc((void **)&(this->dev_raw_depth),
                 this->raw_depth_size.width * this->raw_depth_size.height *
                     sizeof(RawDepthType)));
  ...
  checkCudaErrors(
      cudaMemset(this->dev_raw_color, 0x00,
                 this->raw_color_size.width * this->raw_color_size.height *
                     sizeof(RawColorType)));
  checkCudaErrors(
      cudaMemset(this->dev_raw_depth, 0x00,
                 this->raw_depth_size.width * this->raw_depth_size.height *
                     sizeof(RawDepthType)));
  ...
  // HOST
  this->raw_aligned_points = (My_Type::Vector3f *)malloc(
      this->aligned_depth_size.width * this->aligned_depth_size.height *
      sizeof(My_Type::Vector3f));
  this->hierarchy_intensity.init_parameters(
      this->raw_color_size, this->image_alignment_width, number_of_layers);
  this->hierarchy_points.init_parameters(
      this->raw_depth_size, this->image_alignment_width, number_of_layers);
  this->hierarchy_normals.init_parameters(
      this->raw_depth_size, this->image_alignment_width, number_of_layers);
  this->hierarchy_model_points.init_parameters(
      this->raw_depth_size, this->image_alignment_width, number_of_layers);
  this->hierarchy_model_normals.init_parameters(
      this->raw_depth_size, this->image_alignment_width, number_of_layers);
...
void Preprocess_RGBD::preprocess_model_points(
  ...
  checkCudaErrors(cudaMemcpy(
      ...
      this->dev_hierarchy_model_points.size[0].width *
          this->dev_hierarchy_model_points.size[0].height *
          sizeof(My_Type::Vector3f),
      cudaMemcpyDeviceToDevice));
  checkCudaErrors(cudaMemcpy(
      this->dev_hierarchy_model_normals.data_ptrs[0], dev_model_normals,
      this->dev_hierarchy_model_normals.size[0].width *
          this->dev_hierarchy_model_normals.size[0].height *
          sizeof(My_Type::Vector3f),
      cudaMemcpyDeviceToDevice));
  ...
  // Generate hierarchy depth & normal image
  {
    for (int layer_id = 1;
      ...
      thread_rect.z = 1;
      block_rect.x =
          this->dev_hierarchy_points.size[layer_id].width / thread_rect.x;
      block_rect.y =
          this->dev_hierarchy_points.size[layer_id].height / thread_rect.y;
      block_rect.z = 1;
      // Lunch kernel function (points)
      down_sample_hierarchy_layers_CUDA(
          block_rect, thread_rect,
...
void Preprocess_RGBD::filter_image(cv::Mat &raw_color, cv::Mat &raw_depth) {
  ...
  checkCudaErrors(
      cudaMemcpy(this->dev_raw_depth, raw_depth.data,
                 this->raw_depth_size.width * this->raw_depth_size.height *
                     sizeof(RawDepthType),
                 cudaMemcpyHostToDevice));
  //
  checkCudaErrors(
      cudaMemcpy(this->dev_raw_color, raw_color.data,
                 this->raw_color_size.width * this->raw_color_size.height *
                     sizeof(RawColorType),
                 cudaMemcpyHostToDevice));
  ...
  if ((filter_radius & 0x00000001) == 0x00000001) {
    ...
    block_rect.z = 1;
    // Lunch kernel function
    bilateral_filter_5x5_CUDA(block_rect, thread_rect, this->dev_depth_buffer,
  ...
  for (int i = 0; i < repeat_times; i++) {
    ...
    block_rect.z = 1;
    // Lunch kernel function
    bilateral_filter_5x5_CUDA(block_rect, thread_rect, this->dev_filtered_depth,
                              this->dev_depth_buffer);
    // CUDA_CKECK_KERNEL;
    bilateral_filter_5x5_CUDA(block_rect, thread_rect, this->dev_depth_buffer,
                              this->dev_filtered_depth);
  ...
  block_rect.z = 1;
  // Lunch kernel function
  generate_intensity_image_CUDA(block_rect, thread_rect, this->dev_raw_color,
                                this->raw_color_size,
...
void Preprocess_RGBD::generate_hierarchy_image() {
  ...
  // ----- Layer 0 -----
  // Generate aligned points image (From filtered depth image) for camera pose
  // tracking
  {
    thread_rect.x = this->image_alignment_width;
    thread_rect.y = this->image_alignment_width;
    ...
    for (int layer_id = 0;
      ...
      checkCudaErrors(
          cudaMemcpy(this->hierarchy_points.data_ptrs[layer_id],
                     this->dev_hierarchy_points.data_ptrs[layer_id],
                     this->dev_hierarchy_points.size[layer_id].width *
                         this->dev_hierarchy_points.size[layer_id].height *
                         sizeof(My_Type::Vector3f),
  ...
  for (int layer_id = 0; layer_id < this->dev_hierarchy_points.number_of_layers;
    ...
    checkCudaErrors(
        cudaMemcpy(this->hierarchy_points.data_ptrs[layer_id],
                   ...
                   cudaMemcpyDeviceToHost));
    checkCudaErrors(
        cudaMemcpy(this->hierarchy_normals.data_ptrs[layer_id],
                   this->dev_hierarchy_normals.data_ptrs[layer_id],
                   this->dev_hierarchy_normals.size[layer_id].width *
                       this->dev_hierarchy_normals.size[layer_id].height *
                       sizeof(My_Type::Vector3f),
                   cudaMemcpyDeviceToHost));
    checkCudaErrors(
        cudaMemcpy(this->hierarchy_model_points.data_ptrs[layer_id],
                   this->dev_hierarchy_model_points.data_ptrs[layer_id],
                   this->dev_hierarchy_model_points.size[layer_id].width *
                       this->dev_hierarchy_model_points.size[layer_id].height *
                       sizeof(My_Type::Vector3f),
                   cudaMemcpyDeviceToHost));
    checkCudaErrors(
        cudaMemcpy(this->hierarchy_model_normals.data_ptrs[layer_id],
                   this->dev_hierarchy_model_normals.data_ptrs[layer_id],
                   this->dev_hierarchy_model_normals.size[layer_id].width *
                       this->dev_hierarchy_model_normals.size[layer_id].height *
                       sizeof(My_Type::Vector3f),
                   cudaMemcpyDeviceToHost));
  }
}

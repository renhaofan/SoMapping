...
//
__global__ void generate_float_type_depth_KernelFunc(const RawDepthType * raw_depth,
													 ...
													 float * float_type_depth)
{
	...
	// Validate coordinate
	if (u >= raw_depth_size.width || v >= raw_depth_size.height)	is_valid_pixel = false;
	...
	if (is_valid_pixel)
	{
		...
		float_type_depth_value = (float)raw_depth[raw_image_index] / sensor_params.sensor_scale;
		if (float_type_depth_value < sensor_params.min_range)	float_type_depth_value = 0.0f;
		if (float_type_depth_value > sensor_params.max_range)	float_type_depth_value = 0.0f;
									...
									float * float_type_depth)
{
	...
	generate_float_type_depth_KernelFunc << < block_rect, thread_rect >> > (raw_depth, sensor_params, raw_depth_size,
																			float_type_depth);
}


//
__global__ void bilateral_filter_3x3_KernelFunc(const float * src_depth, float dst_depth)
{
	//
	//bool is_valid_pixel = true;
	//// Pixel coordinate
...
//
__global__ void bilateral_filter_5x5_KernelFunc(const float * src_depth, float * dst_depth)
{
	//
	bool is_valid_pixel = true;
	// Image size 
	...
	if (v < RADIUS_5x5 || v >= (image_height - RADIUS_5x5))	is_valid_pixel = false;
	...
	if (is_valid_pixel)
	{
		...
		//float sigma_z = 1.0f / (0.0012f + 0.0019f * (depth_value - 0.4f) * (depth_value - 0.4f)
		//						+ 0.0001f / sqrtf(depth_value) * 0.25f);
		...
		for (int i = -2; i <= 2; i++)	for (int j = -2; j <= 2; j++)
		{
			...
			//
			float weight = expf(-0.5 * ((abs(i) + abs(j)) * SIGMA_L_5x5 * SIGMA_L_5x5
...
void bilateral_filter_5x5_CUDA(dim3 block_rect, dim3 thread_rect,
							   ...
							   float * dst_depth)
{
	bilateral_filter_5x5_KernelFunc << <block_rect, thread_rect >> >(src_depth, dst_depth);
}


//
__global__ void generate_intensity_image_KernelFunc(const RawColorType * raw_color,
													...
													float * aligned_intensity_image)
{
	...
	// Validate coordinate
	if (u >= raw_color_size.width || v >= raw_color_size.height)	is_valid_pixel = false;
	...
	if (is_valid_pixel)
	{
		...
		gray_intensity = (0.299f * (float)pixel_color.r + 0.587f * (float)pixel_color.g + 0.114f * (float)pixel_color.b) / 255.f;
	}
...
void generate_intensity_image_CUDA(dim3 block_rect, dim3 thread_rect, 
								   ...
								   float * aligned_intensity_image)
{
	generate_intensity_image_KernelFunc << <block_rect, thread_rect >> >(raw_color, raw_color_size,
																		 aligned_intensity_image);
}
...
__global__ void generate_gradient_image_KernelFunc(const float * aligned_intensity_image,
												   My_Type::Vector2f * gradient_image)
{
	...
	int image_width = blockDim.x * gridDim.x;
	int image_height = blockDim.y * gridDim.y;
	if (u < GRADIENT_STEP || u >= (image_width - GRADIENT_STEP))	is_valid_pixel = false;
	if (v < GRADIENT_STEP || v >= (image_height - GRADIENT_STEP))	is_valid_pixel = false;
	int image_index = u + v * image_width;
	...
	{
		// unroll
		gray_intensity[0] = aligned_intensity_image[image_index - image_width * GRADIENT_STEP - GRADIENT_STEP];
		gray_intensity[1] = aligned_intensity_image[image_index - image_width * GRADIENT_STEP];
		gray_intensity[2] = aligned_intensity_image[image_index - image_width * GRADIENT_STEP + GRADIENT_STEP];
		...
		gray_intensity[6] = aligned_intensity_image[image_index + image_width * GRADIENT_STEP - GRADIENT_STEP];
		gray_intensity[7] = aligned_intensity_image[image_index + image_width * GRADIENT_STEP];
		gray_intensity[8] = aligned_intensity_image[image_index + image_width * GRADIENT_STEP + GRADIENT_STEP];
		for (int i = 0; i < 9; i++)
		{
	...
	{
		// Gradient X 
		gradient_vec.x = gray_intensity[5] * 0.5 + gray_intensity[2] * 0.25 + gray_intensity[8] * 0.25;
		gradient_vec.x -= gray_intensity[3] * 0.5 + gray_intensity[0] * 0.25 + gray_intensity[6] * 0.25;
		gradient_vec.x /= (float)(GRADIENT_STEP * 2);

		// Gradient Y
		gradient_vec.y = gray_intensity[7] * 0.5 + gray_intensity[6] * 0.25 + gray_intensity[8] * 0.25;
		gradient_vec.y -= gray_intensity[1] * 0.5 + gray_intensity[0] * 0.25 + gray_intensity[2] * 0.25;
								  ...
								  My_Type::Vector2f * gradient_image)
{
	...
	generate_gradient_image_KernelFunc << <block_rect, thread_rect >> >(aligned_intensity_image,
																		gradient_image);
}
...
__global__ void outlier_point_filter_KernelFunc(const float * src_depth, float * dst_depth)
{
    bool enable=false;
    bool is_valid_pixel = true;
    int count=0;
    ...
    if (is_valid_pixel)
        for (int i = -3; i < 3; ++i) {//3
            for (int j = -3; j < 3; ++j) {//3
                ...
                dis = tmpz - z;
                dis =  dis * dis;
                if (dis<0.001f) {//0.005~0.015//多个相邻点深度接近该点深度，该点不是噪点
                    count++;
...
//
__global__ void bilateral_filter_KernelFunc(const float * src_depth, float * dst_depth)
{
    //
    bool is_valid_pixel = true;
    // Image size
    int image_width = blockDim.x * gridDim.x;//TODO remove the img size computation out of the filter, for example, input para Vector2d directly
    int image_height = blockDim.y * gridDim.y;
    // Pixel coordinate
    int u, v;
    u = threadIdx.x + blockDim.x * blockIdx.x;
...
void outlier_point_filter_CUDA(dim3 block_rect, dim3 thread_rect,
                               ...
                               float * dst_depth)
{
    outlier_point_filter_KernelFunc << <block_rect, thread_rect >> >(src_depth, dst_depth);
}
...
void bilateral_filter_CUDA(dim3 block_rect, dim3 thread_rect,
                           ...
                           float * dst_depth)
{
    bilateral_filter_KernelFunc << <block_rect, thread_rect >> >(src_depth, dst_depth);
}
...
//
__global__ void generate_aligned_points_image_KernelFunc(const float * filtered_depth,
														 Sensor_params sensor_params,
														 My_Type::Vector3f * points_image)
{
	...
	{
		// 
		point.x = depth_value * ((float)u - sensor_params.sensor_cx + 0.5f) / sensor_params.sensor_fx;
		point.y = depth_value * ((float)v - sensor_params.sensor_cy + 0.5f) / sensor_params.sensor_fy;
		point.z = depth_value;
	}
...
void generate_aligned_points_image_CUDA(dim3 block_rect, dim3 thread_rect,
										...
										My_Type::Vector3f * points_image)
{
	generate_aligned_points_image_KernelFunc << <block_rect, thread_rect >> >(filtered_depth, sensor_params,
																			  points_image);
}
...
inline __device__ void compute_normal_vector(const My_Type::Vector3f * points_image,
															int & image_width, 
															int & center_index,
...
//
template <int Normal_Stride>
__global__ void compute_normals_image_KernelFunc(const My_Type::Vector3f * points_image,
												 My_Type::Vector3f * normals_image)
{
	...
	u = threadIdx.x + blockDim.x * blockIdx.x;
	v = threadIdx.y + blockDim.y * blockIdx.y;
	if (u < Normal_Stride || u >= image_width - Normal_Stride)	is_valid_pixel = false;
	if (v < Normal_Stride || v >= image_height - Normal_Stride)	is_valid_pixel = false;
	...
	if (is_valid_pixel)
	{
		compute_normal_vector<Normal_Stride>(points_image, image_width, normal_image_index, normal_vec);
	}
...
void compute_normals_image_CUDA(dim3 block_rect, dim3 thread_rect, 
								...
								My_Type::Vector3f * normals_image)
{
	compute_normals_image_KernelFunc<NORMAL_STRIDE> << <block_rect, thread_rect >> >(points_image, normals_image);
}


//
template <typename T>
__global__ void down_sample_hierarchy_layers_KernelFunc(const T * src_layer, My_Type::Vector2i src_layer_size, 
														T * dst_layer)
{
	//
	bool is_valid_pixel = true;
	...
	if ((dst_u << 1) >= (src_layer_size.width - 1) || (dst_v << 1) >= (src_layer_size.height - 1)) is_valid_pixel = false;
	...
	if (is_valid_pixel)
	{
		//
		int src_u, src_v;
		src_u = dst_u << 1 + 0;		src_v = dst_v << 1 + 0;
...
// Specialization T = My_Type::Vector3f (ToDo : copy operator for My_Type::VectorX )
template <>
__global__ void down_sample_hierarchy_layers_KernelFunc<My_Type::Vector3f>(const My_Type::Vector3f * src_layer, 
...
// Specialization T = My_Type::Vector3f (ToDo : copy operator for My_Type::VectorX )
template <>
__global__ void down_sample_hierarchy_layers_KernelFunc<My_Type::Vector2f>(const My_Type::Vector2f * src_layer,
																		   My_Type::Vector2i src_layer_size,
...
void down_sample_hierarchy_layers_CUDA(dim3 block_rect, dim3 thread_rect,
                                       ...
                                       My_Type::Vector2f * dst_layer)
{
    down_sample_hierarchy_layers_KernelFunc<< <block_rect, thread_rect >> >(src_layer, src_layer_size, dst_layer);
}
//
void down_sample_hierarchy_layers_CUDA(dim3 block_rect, dim3 thread_rect,
...
void down_sample_hierarchy_layers_CUDA(dim3 block_rect, dim3 thread_rect,
									   ...
									   T * dst_layer)
{
	down_sample_hierarchy_layers_KernelFunc<T> << <block_rect, thread_rect >> >(src_layer, src_layer_size,
																				dst_layer);
}
...
//
//	down_sample_hierarchy_layers_CUDA<float>(block_rect, thread_rect, NULL, temp_vec, NULL);
//	down_sample_hierarchy_layers_CUDA<My_Type::Vector2f>(block_rect, thread_rect, NULL, temp_vec, NULL);
//	down_sample_hierarchy_layers_CUDA<My_Type::Vector3f>(block_rect, thread_rect, NULL, temp_vec, NULL);
//}
...
__global__ void compute_points_laplacian_KernelFunc(const My_Type::Vector3f * points_image,
...
void compute_points_laplacian_CUDA(dim3 block_rect, dim3 thread_rect,
								   ...
								   float * laplacian_image)
{
	compute_points_laplacian_KernelFunc<LAPLACIAN_STRIDE> << <block_rect, thread_rect >> >(points_image, laplacian_image);
}
